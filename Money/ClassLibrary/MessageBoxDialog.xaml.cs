...
namespace ClassLibrary
{
	...
	{
		internal static void ShowInfo(string title, string text, EventHandler closeHandler)
		{
			new MessageBoxDialog(title, text, MessageBoxButtons.OK, MessageBoxIcon.Info, closeHandler);
		}

		internal static void ShowError(string title, string text, EventHandler closeHandler)
		{
			new MessageBoxDialog(title, text, MessageBoxButtons.OK, MessageBoxIcon.Error, closeHandler);
		}

		internal static void ShowWarning(string title, string text, EventHandler closeHandler)
		{
			new MessageBoxDialog(title, text, MessageBoxButtons.OK, MessageBoxIcon.Warning, closeHandler);
		}

		internal static void ShowOKCancel(string title, string text, MessageBoxIcon icon, EventHandler closeHandler)
		{
			new MessageBoxDialog(title, text, MessageBoxButtons.OKCancel, icon, closeHandler);
		}

		internal static void ShowYesNo(string title, string text, MessageBoxIcon icon, EventHandler closeHandler)
		{
			new MessageBoxDialog(title, text, MessageBoxButtons.YesNo, icon, closeHandler);
		}

		internal static void ShowYesNoCancel(string title, string text, MessageBoxIcon icon, EventHandler closeHandler)
		{
	...
	public partial class MessageBoxDialog : DialogPanel
	{
		...
		private static BitmapImage GetBitmapImage(MessageBoxIcon icon)
		{
			...
			return ResourceFile.GetBitmap("Images/MessageBox" + name + ".png", AssemblySource.This);
		}
	}
}
