...
//			base.DefaultStyleKey = base.GetType();
			...
			foreach (string itemText in items)
			{
				...
				if (MatchMenuItem(item, "cut"))
				{
					...
					item.Icon = new Image() { Source = ResourceFile.GetBitmap("ContextMenuText/Images/Cut.png", AssemblySource.This) };
				}
				else
				if (MatchMenuItem(item, "copy"))
				{
					item.Click += OnCopyClick;
					item.Icon = new Image() { Source = ResourceFile.GetBitmap("ContextMenuText/Images/Copy.png", AssemblySource.This) };
				}
				else
				if (MatchMenuItem(item, "paste"))
				{
					item.Click += OnPasteClick;
					item.Icon = new Image() { Source = ResourceFile.GetBitmap("ContextMenuText/Images/Paste.png", AssemblySource.This) };
				}
				else
				if (MatchMenuItem(item, "delete"))
				{
					item.Click += OnDeleteClick;
					item.Icon = new Image() { Source = ResourceFile.GetBitmap("ContextMenuText/Images/Delete.png", AssemblySource.This) };
				}
				else
				if (MatchMenuItem(item, "select"))
				{
					item.Click += OnSelectAllClick;
					item.Icon = new Image() { Source = ResourceFile.GetBitmap("ContextMenuText/Images/SelectAll.png", AssemblySource.This) };
				}
				else
				if (MatchMenuItem(item, "print"))
				{
					item.Click += OnPrintClick;
					item.Icon = new Image() { Source = ResourceFile.GetBitmap("ContextMenuText/Images/Print.png", AssemblySource.This) };
				}
		...
		private void ShowClipboardError()
		{
			...
			MessageBoxEx.ShowInfo(errorCaption, builder.ToString(), null);
		}
	}
}
