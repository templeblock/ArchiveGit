...
namespace ClassLibrary
{
	internal class MD5 : IDisposable
	{
		...
		private int m_State;
		private readonly uint[] m_state = new uint[4];
		private readonly uint[] m_count = new uint[2];
		private readonly byte[] m_buffer = new byte[64];
		...
		static private readonly byte[] PADDING = new byte[] {
		...
		internal static string GetMd5String(String source)
		{
		...
		protected virtual void HashCore(byte[] input, int offset, int count)
		{
			...
			int index = (int)((m_count[0] >> 3) & 0x3F);

			// Update number of bits
			if ((m_count[0] += (uint)count << 3) < ((uint)count << 3))
				m_count[1]++;
			m_count[1] += ((uint)count >> 29);
			...
			if (count >= partLen)
			{
				Buffer.BlockCopy(input, offset, m_buffer, index, partLen);
			...
			Buffer.BlockCopy(input, offset + i, m_buffer, index, count - i);
		}
		...
		{
			int i = inputOffset;
			for (int j = outputOffset; j < outputOffset + count; j += 4)
			{
		...
		{
			int i = outputOffset;
			for (int j = inputOffset; j < inputOffset + count; j += 4)
			{
		...
		public virtual bool CanReuseTransform { get { return true; } }
		public virtual bool CanTransformMultipleBlocks { get { return true; } }
		public virtual byte[] Hash { get { if (m_State == 0) return (byte[])m_HashValue.Clone(); throw new InvalidOperationException(); } }
		public virtual int HashSize { get { return HashSizeValue; } }
		public virtual int InputBlockSize { get { return 1; } }
		public virtual int OutputBlockSize { get { return 1; } }
		...
		internal byte[] ComputeHash(Stream inputStream)
		{
			...
			int count;
			while ((count = inputStream.Read(buffer, 0, 4096)) > 0)
