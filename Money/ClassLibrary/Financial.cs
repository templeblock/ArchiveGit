...
namespace ClassLibrary
{
	...
	{ // Checkout http://api.kde.org/koffice-api/koffice-apidocs/kspread/html/financial_8cpp-source.html
		...
		internal double EFFECT(double rate, int npery)
		{
			if (npery == 0 || rate == 0)
			{
		...
		internal double SLN(double Cost, double Salvage, double Life)
		{
			if (Life <= 0.0)
		...
		internal double SYD(double Cost, double Salvage, double Life, int Period)
		{
			if (Life <= 0.0)
		...
		internal double DDB(double Cost, double Salvage, double Life, int Period)
		{
			double Factor = 2.0;
		...
		/// <returns></returns>
		internal static double PresentValue(double rate, double numberPeriods, double payment, double futureValue, Compounding compounding)
		{
			if (rate == 0)
		...
		/// <param name="compounding">Specifies when values are accrued, at the beginning or at the end of the period.</param>
		/// <returns></returns>
		internal static double FutureValue(double rate, double numberPeriods, double payment, double presentValue, Compounding compounding)
		{
			if (rate == 0)
		...
		/// <param name="compounding">Specifies when values are accrued, at the beginning or at the end of the period.</param>
		/// <returns></returns>
		internal static double Payment(double rate, double numberPeriods, double presentValue, double futureValue, Compounding compounding)
		{
			if (numberPeriods == 0)
		...
		/// <param name="compounding">Specifies when values are accrued, at the beginning or at the end of the period.</param>
		/// <returns></returns>
		internal static double PrincipalPayment(double rate, int period, double numberPeriods, double presentValue, double futureValue, Compounding compounding)
		{
		...
		/// <param name="compounding">Specifies when values are accrued, at the beginning or at the end of the period.</param>
		/// <returns></returns>
		internal static double InterestPayment(double rate, int period, double numberPeriods, double presentValue, double futureValue, Compounding compounding)
		{
			if (period <= 0 || period >= numberPeriods + 1)
				throw new ArgumentException("Invalid Argument Value", "period");
			...
			if (compounding != Compounding.EndOfPeriod)
				...
				auxiliaryPeriod = 1;
			double internalFutureValue = FutureValue(rate, period - auxiliaryPeriod, payment, presentValue, Compounding.EndOfPeriod);
		...
		/// <param name="compounding">Specifies when values are accrued, at the beginning or at the end of the period.</param>
		/// <returns></returns>
		internal static double NumberOfPeriods(double rate, double payment, double presentValue, double futureValue, Compounding compounding)
		{
			if (rate <= -1)
...
//j				throw new ArgumentException("Cannot Calculate Number of Periods");
		...
		/// <returns></returns>
		internal static double InternalRateOfReturn(double[] cashFlow, double guess)
		{
			if (guess == 0)
				guess = 0.033;
		...
		/// <returns></returns>
		internal static double ReinvestedInternalRateOfReturn(double[] cashFlow, double financeRate, double reInvestRate)
		{
			if (cashFlow.Rank != 1)
				throw new ArgumentException("Invalid Argument Value", "CashFlow");
			...
			double presentValueFinance = InternalNetPresentValue(cashFlow, financeRate, -1);
			if (presentValueFinance == 0)
				throw new DivideByZeroException("Cannot Divide By Zero");

			short arrayUpperBound = (short)cashFlow.GetUpperBound(0);
			double presentValueReInvest = InternalNetPresentValue(cashFlow, reInvestRate, 1);
			double internalReInvestedRate = (-presentValueReInvest * Math.Pow(1 + reInvestRate, arrayUpperBound)) / (presentValueFinance * (1 + financeRate));
			if (internalReInvestedRate < 0)
				throw new ArgumentException("Argument Invalid Value");

			return (Math.Pow(internalReInvestedRate, 1 / (arrayUpperBound - 1)) - 1);
		}
		...
		/// <param name="rate">Periodic Interest Rate.</param>
		/// <returns></returns>
		internal static double NetPresentValue(double[] cashFlow, double rate)
		{
			if (cashFlow == null)
				throw new ArgumentException("Invalid Argument Value", "cashFlow");
			...
			return InternalNetPresentValue(cashFlow, rate, 0);
		}
		...
		/// <returns></returns>
		private static double InternalNetPresentValue(double[] cashFlow, double rate, short paymentDueCollectable)
		{
		...
		/// <returns></returns>
		internal static double Rate(double numberPeriods, double payment, double presentValue, double futureValue, Compounding compounding, double guess)
		{
			if (numberPeriods <= 0)
