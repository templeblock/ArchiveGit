...
IccExpert::IccExpert(void)
{
	...
	_out_data = 0;
	_tableH = 0;
	_intent	= cmPerceptual;
	_dpintent = cmAToB0Tag;
	_pdintent = cmBToA0Tag;
}
...
long IccExpert::_check_connect_to_dev_tags(CsFormat *prof, CM2Header *header)
{
	...
	switch(header->profileConnectionSpace)
	{
		...
		switch(header->profileClass)
		{
			...
			exist = prof->checkTagExist(_pdintent);
			if(exist)	ok = 1;
			break;
...
long IccExpert::_check_dev_to_connect_tags(CsFormat *prof, CM2Header *header)
{
	...
	switch(header->profileConnectionSpace)
	{
		...
		switch(header->profileClass)
		{
			...
			case(cmAbstractClass):
			case(cmColorSpaceClass):
			exist = prof->checkTagExist(_dpintent);
			if(exist)	ok = 1;
			break;
...
//1/13
	_intent = profset->intent;
	switch(profset->intent){
		case(cmPerceptual):
			_dpintent = cmAToB0Tag;
			_pdintent = cmBToA0Tag;
			break;
		...
		case(cmRelativeColorimetric):
			_dpintent = cmAToB1Tag;
			_pdintent = cmBToA1Tag;
			break;

		case(cmSaturation):
			_dpintent = cmAToB2Tag;
			_pdintent = cmBToA2Tag;
			break;
...
//1/13
	...
	switch(_intent){
		...
		case(cmRelativeColorimetric):
		for(i = 0; i < 3; i++){
			rgbtoxyz[i] /= (whitex/0.9642);
			rgbtoxyz[i+6] /= (whitez/0.9642);
		}
		break;
...
McoStatus IccExpert::_load_XYZ_to_XYZ(CsFormat *prof, CM2Header *header, long dev_to_con, McoHandle* h, CMLut16Type* lut)
{
	...
	if(dev_to_con)
		status = _load_lut_tag(prof, header, _dpintent, h,  lut);
	else
		status = _load_lut_tag(prof, header, _pdintent, h,  lut);
...
{
	return _load_lut_tag(prof, header, _dpintent, h,  lut);;
}
