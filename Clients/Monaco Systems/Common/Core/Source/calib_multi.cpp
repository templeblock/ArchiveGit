...
void calib_base::interpolate_3D_n_0(double *lab,double *table)
{
double	a,b,c;
int32	a1,a2,b1,b2,c1,c2;
int32	ai,bi,ci;
...
if (K+300 <= max_ink) // no ink limiting problem
	...
	ink_sum += Y_Lut[cm];
	ink_sum += K_comp;
	if (ink_sum > max_ink)  // check to see if color is completly out of gamut for the ink limit
		{
		cm = 0;
		cp = BT_DIM-1;
		mm = 0;
...
// get the lab value for a cmyk value
double calib_inter4::getLab(double *cmyk, double *lab)
{
register double L,a,b,L2,a2,b2,d,temp;
double	K;
...
int32	Ki1,Ki2;
double  Kv,Kd,One_Kd;
unsigned char  cal_c,cal_m,cal_y,cal_k;
...
cal_k = (unsigned char)linear_data->CnvtLinearInv(K,3);	
...
if (Ki1 == 0)
	...
	do 
		...
		for (i=-1; i<2; i++)
			{
			int_c = cmy[0] + i*speed;
			cal_c = C_Cal_Lut[(long)(int_c*8+0.5)];
			if ((int_c >=0) && (int_c <= max_cal)) for (j=-1; j<2; j++)
				{
				int_m = cmy[1] + j*speed;
				cal_m = M_Cal_Lut[(long)(int_m*8+0.5)];
				if ((int_m >=0) && (int_m <= max_cal)) for (k=-1; k<2; k++) if (i!=0 || j!=0 || k!=0)
					{
					int_y = cmy[2] + k*speed;
					cal_y = Y_Cal_Lut[(long)(int_y*8+0.5)];
					ink_sum = cal_k+cal_c+cal_m+cal_y;
...
if (linear_data)
	...
	bt_1 = max_cal/(double)(800-1);

	for (j=0; j<800; j++)
		{
		C_Cal_Lut[j] = linear_data->CnvtLinearInv(j*bt_1,0);
		M_Cal_Lut[j] = linear_data->CnvtLinearInv(j*bt_1,1);
		Y_Cal_Lut[j] = linear_data->CnvtLinearInv(j*bt_1,2);
		}
	}
