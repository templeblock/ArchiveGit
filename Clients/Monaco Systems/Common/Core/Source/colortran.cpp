...

{
...

	{
		...
		monitor[0] = .0000; // Gamma (not read by newbuildrgbxyz())

		monitor[1] = .3126; // WhiteX

		monitor[2] = .3289; // WhiteY

		monitor[3] = .6584; // RedX

		monitor[4] = .3377; // RedY

		monitor[5] = .2799; // GreenX

		monitor[6] = .7200; // GreenY

		monitor[7] = .1342; // BlueX

		monitor[8] = .0720; // BlueY



		double rgbtoxyz[9];

		newbuildrgbxyz(monitor, xyz2rgb, rgbtoxyz);

	}



	nxyztorgb(xyz2rgb, gamma, xyz, rgb, 1/*num*/);
...

{
	...
	for( i = 0; i < 256; i++)
		...
		gammat[i] = (char)(255.0*pow((double)i/255.0, gamma) + 0.5);
