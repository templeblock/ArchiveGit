...
//testing function
void printCMYK(double *cmyk, char *outname)
{
	FILE *file;
	int i, j;

file = fopen( (char*)outname, "w");
if(!file)	return;


for (i=0; i<33*33*33; i++)
		{
			for (j=0; j<4; j++) fprintf(file,"%f\t",cmyk[i*4+j]);
		fprintf(file,"\n");
	}
...
for (i=0; i<4; i++)
	{
	if (cmyk_out[i] <0) cmyk_out[i] = 0;
	if (cmyk_out[i] >100) cmyk_out[i] = 100;
	inksum += cmyk_out[i];
...
if (inksum > maxInk)
	...
	if (cmyk_out[3] < 0) 
		{
		cmyk_out[3] = 0;
		cmy_down = cmysum-(inksum - cmysum)-maxInk;
		}
	else
		{
...
McoStatus add_black::evalMB(double minGam, double maxGam)
{
	...
	int32	calib_start;
	double		K_goal,K_inc;
	double 		*dist = 0L,*distp;
	double		min_dist;
	int32		min_dist_K;
	double		K_best;
	double		mL;
	double		minK,maxK,absMaxK;
	double		max_dist;
	double    	check_dist;
	short		*gamut = 0L,*gamutp;
	double		avg_err,err_num;
	double		temp,range,kmin,kmax,kinc,tdist;
	...
	absMaxK = 100*findblack->maxK();	
	
	dist = new double[33*1089];
	if (!dist) return MCO_MEM_ALLOC_ERROR;
	distp = dist;
	
	gamut = new short[33*1089];
	if (!gamut) return MCO_MEM_ALLOC_ERROR;
	gamutp = gamut;
			
	if (ucrgcr) check_dist = 0.0001;	
	...
	for( i = 0; i < 33; i++)
		...
		for( j = 0; j < 1089; j++)
			{
			NotInGammut = TRUE;
			calib->getMinMaxK(lab,&minK,&maxK,gamutp);
			if (maxK > absMaxK) maxK = absMaxK;
			K = findblack->eval(lab,minK,maxK);
			if ((K > 2) || (!gamutp[0]))
				{
				distp[0] = calib->getDistance(lab,K,check_dist);
				}
			else distp[0] = check_dist;
			if ((j%66 == 0) && (thermd != 0L)) if (!thermd->DisplayTherm(2000+i*37,Process_BuildingTable)) goto bail;
			cmyk[3] = K;	// set K
			gamutp++;
			distp++;
			lab += 3;
	...
	for( i = 0; i < 33; i++)
		...
		for( j = 0; j < 33; j++)
			...
			for( k = 0; k < 33; k++)
				{
				K = cmyk[3];
				if ((K > 2) && ((distp[0] > check_dist) || (!gamutp[0])))
					{
					if (!ucrgcr)
						{
						// calculate how much error is present
						avg_err = dist[k+33*j+33*33*i];
						err_num = 1;
						if (i>0) 
							{
							avg_err += dist[k+33*j+33*33*(i-1)];
							err_num++;
							}
						if (i<32) 
							{
						...
						avg_err = avg_err/err_num;
						
						temp = (avg_err-check_dist)/3;
						temp = temp*temp;
						if ((distp[0]/avg_err)*(2/(1+exp(-temp))-1) > 2.0)
							{
							range = 1.0;  // the error is significantly different, search all values
							}
						else if (avg_err > check_dist)
							{
							// calculate the search region
							temp = (avg_err - check_dist)/4;
							temp = temp*temp*temp;
							range = 0.05+0.95*(temp/(1+temp));
							}
						else range = 0.05;
						
						// scale range to actual range
						range *= absMaxK;
						...
						// find the correct increment
						temp = (kmax-kmin)/3.5;
						kinc = (kmax-kmin)/(int)temp;
						
						K_best = K;
					
						max_dist = MinVal(distp[0],MAX_DIST);
						min_dist = distp[0];
						for (fi=kmin; fi<=kmax; fi+=kinc)
							{
							tdist = calib->getDistance(lab,fi,max_dist);
							// anything less than five is chance
							if (tdist <5) tdist = 4+(tdist*tdist*tdist)*0.008;
							// add in a k distance factor
							temp = fi-K;
							temp = temp*temp*0.025;
							tdist += 0.1*fabs(fi-K)+ 6 - 6*exp(-temp);
							if (tdist < min_dist)
								{
								K_best = fi;
								min_dist = tdist;
			...
			if (thermd != 0L) if (!thermd->DisplayTherm(3221+i*37,Process_BuildingTable)) goto bail;
			}
		}			
