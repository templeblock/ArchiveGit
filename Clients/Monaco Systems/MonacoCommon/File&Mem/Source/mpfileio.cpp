...
McoStatus Mpfileio::revconvert(MNPatchType *patchtype, PatchFormat *patchdata)
{
	...
	patchdata->steps = new long[patchdata->numCube];
	for(i = 0; i < (long)patchtype->numCube; i++)
		patchdata->steps[i] = patchtype->steps[i];

	patchdata->blacks = new double[patchdata->numCube];
	for(i = 0; i < (long)patchdata->numCube; i++)
	...
	patchdata->cmy = new double[totalcmy];
	m = 0;
	for(i = 0; i < (long)patchdata->numCube; i++)
...
McoStatus	Mpfileio::loadprofile(CsFormat *icc)
{
    ...
    memset ( &patchtypesrc, 0, sizeof(MNPatchType) );
	memset ( &patchtypedest, 0, sizeof(MNPatchType) ); 
	...
	long lutsize, lutoffset, inputsize, outputsize, start, i;
	if( luttag.typeDescriptor == cmSigLut16Type){
		...
		lutoffset = 52 + luttag.inputChannels*luttag.inputTableEntries*2;	
		...
		lutoffset = 48 + luttag.inputChannels*256;
	...
	_printdata->tableH = McoNewHandle(lutsize);	
	...
	status = icc->getPartialTagdata(cmAToB0Tag, lutoffset, lutsize, data);
	...
	if(icc->checkTagExist(MC0P_SETTING_TAG)){
		...
		_printdata->setting.linearDate = setting.linearDate;
		_printdata->setting.srcDate = setting.srcDate;
		_printdata->setting.dstDate = setting.dstDate;
		for(i = 0; i < 6; i++)
...
//setting tag
	...
	setting->linearDate = _printdata->setting.linearDate;
	setting->srcDate = _printdata->setting.srcDate;
	setting->dstDate = _printdata->setting.dstDate;
	for(i = 0; i < 6; i++)
...
void Mpfileio::deletePatchdata(PatchFormat *patchdata)
{
	if(patchdata->steps){
		delete 	[]patchdata->steps;
		patchdata->steps = 0;
	}
	if(patchdata->blacks){
		delete 	[]patchdata->blacks;
		patchdata->blacks = 0;
	}
	if(patchdata->cmy){
		delete []patchdata->cmy;
		patchdata->cmy = 0;
	}	
	if(patchdata->linear){
		delete []patchdata->linear;
		patchdata->linear = 0;
	}	
	if(patchdata->data){
		delete []patchdata->data;
		patchdata->data = 0;
	}	
	if(patchdata->ldata){
		delete []patchdata->ldata;
		patchdata->ldata = 0;
	}
}

McoStatus Mpfileio::createPatchdata(PatchFormat *patchdata, long *steps)
{
	long	i, totalcmy, labpatches;

	deletePatchdata(patchdata);

	patchdata->steps = new long[patchdata->numCube];
	if(!patchdata->steps)	return MCO_FAILURE;

	for(i = 0; i < patchdata->numCube; i++)
		patchdata->steps[i] = steps[i];

	patchdata->blacks = new double[patchdata->numCube];
	if(!patchdata->blacks)	return MCO_FAILURE;
	...
	patchdata->cmy = new double[totalcmy];
	if(!patchdata->cmy)	return MCO_FAILURE;

	if(patchdata->numLinear)
	{
		patchdata->linear = new double[patchdata->numLinear];
		if(!patchdata->linear)	return MCO_FAILURE;
	}
	...
	patchdata->data = new double[labpatches*3];
	if(!patchdata->data)	return MCO_FAILURE;

	if(patchdata->numLinear)
	{
		patchdata->ldata = new double[ patchdata->numLinear*3*patchdata->numComp];
		if(!patchdata->ldata)	return MCO_FAILURE;
	}
...
McoStatus Mpfileio::_loadPatchdata(FILE *fs, long which)
{
	...
	fgets(patchname, 15, fs);

	m = strncmp(patchname, "Monaco Patches", 14);
	if(m)	return MCO_FAILURE;
	...
	if(patchdata->numLinear > 0)
		totallinear = (patchdata->numLinear - 1)*patchdata->numComp;

	for(i = 0; i < (long)patchdata->numCube; i++)
		fscanf(fs, "%ld",&steps[i]);

	status = createPatchdata(patchdata, steps); 
	if(status != MCO_SUCCESS)	return status;
	
	for(i = 0; i < (long)patchdata->numCube; i++)
		fscanf(fs, "%lf",&patchdata->blacks[i]);
	
	m = 0;
	for(i = 0; i < (long)patchdata->numCube; i++)
		for(j = 0; j < (long)patchdata->steps[i]; j++)
			fscanf(fs, "%lf",&patchdata->cmy[m++]);
	...
	for(i=0; i < totallinear; i++)
	{
		j = i*3;
		if (!feof(fs)) fscanf(fs, "%lf \t %lf \t %lf\n", &patchdata->ldata[j], &patchdata->ldata[j+1], &patchdata->ldata[j+2]);
		else 
		{
			patchdata->ldata[j] = 0;
			patchdata->ldata[j+1] = 0; 
			patchdata->ldata[j+2] = 0;
		}
	}
...
McoStatus Mpfileio::_savePatchdata(FILE *fs, long which)
{
	...
	McoStatus 			status;
	long	i, j, m;
	long	totalcmy, calpatch, totallinear = 0;
	PatchFormat		*patchdata;
	...
	for(i = 0; i < (long)patchdata->numCube; i++)
		fprintf(fs, "%lf ",patchdata->blacks[i]);
...
{	
	...
	fgets(junk,50,fs);
	n = strncmp(junk,"DryJet Linearization", 20);
	if (n) return MCO_FILE_DATA_ERROR;

	SettingFormat *setting = &_printdata->setting;
	return _loadCurveData(fs, MAX_LINEAR_HAND, (long *)setting->numLinearHand, setting->linearX, 
				setting->linearY, setting->linearDesc, &numchannel);
}
...
McoStatus Mpfileio::_saveLineardata(FILE *fs)
{
	...
	return _saveCurveData(fs, MAX_LINEAR_HAND, (long *)setting->numLinearHand, setting->linearX, 
				setting->linearY, setting->linearDesc, 4);
}
...
McoStatus Mpfileio::_loadTonedata(FILE *fs)
{
	...
	fgets(junk,50,fs);
	n = strncmp(junk,"DryJet Tone Curve", 17);
	if (n) return MCO_FILE_DATA_ERROR;

	SettingFormat *setting = &_printdata->setting;
	return _loadCurveData(fs, MAX_TONE_HAND, (long *)setting->numToneHand, setting->toneX, 
				setting->toneY, setting->toneDesc, &numchannel);
}
...
McoStatus Mpfileio::_saveTonedata(FILE *fs)
{
	...
	return _saveCurveData(fs, MAX_TONE_HAND, (long *)setting->numToneHand, setting->toneX, 
				setting->toneY, setting->toneDesc, 4);
}

//load linear data from patch
McoStatus Mpfileio::loadLineardata(void)
{
	int i;
	SettingFormat *setting = &_printdata->setting;
	PatchFormat	*patchdata = &(_printdata->destpatch);

	if(patchdata->numLinear < 0)	return MCO_FAILURE;

	for(i = 0; i < 4; i++)
		setting->numLinearHand[i] = patchdata->numLinear;

	for(i = 0; i < patchdata->numLinear; i++)
	{
		setting->linearX[i] = patchdata->linear[i];
		setting->linearX[i+MAX_TONE_HAND] = patchdata->linear[i];
		setting->linearX[i+MAX_TONE_HAND*2] = patchdata->linear[i];
		setting->linearX[i+MAX_TONE_HAND*3] = patchdata->linear[i];
	}

	for(i = 0; i < 4; i++)
		setting->linearY[i*MAX_TONE_HAND] = 0;

	for(i = 0; i < patchdata->numLinear - 1; i++)
	{ 
		setting->linearY[i+1] = patchdata->ldata[i*3];
		setting->linearY[i+1+MAX_TONE_HAND] = patchdata->ldata[(i+patchdata->numLinear-1)*3];
		setting->linearY[i+1+MAX_TONE_HAND*2] = patchdata->ldata[(i+2*(patchdata->numLinear-1))*3];
		setting->linearY[i+1+MAX_TONE_HAND*3] = patchdata->ldata[(i+3*(patchdata->numLinear-1))*3];
	}

	setting->linearDate = setting->dstDate;
	memset(setting->linearDesc, '\0', 100);

	return MCO_SUCCESS;
}

McoStatus Mpfileio::_loadCurveData(FILE *fs, long max, long * numHand, double * x, 
									double * y, char *desc, long *channel)
{
	long	numhand[6];
	double	tonex[6*256], toney[6*256];
	int 				n,i,j,l;
	int					numc = -1;
	...
	{
		fscanf(fs,"%d\n",&numhand[i]);
		if (numhand[i] > max) return MCO_FAILURE;
		for (j = 0; j < numhand[i]; j++)
			fscanf(fs,"%lf %lf\n", &tonex[i*max+j], &toney[i*max+j]);
	}
	...
	*channel = numc;
	for( i = 0; i < numc; i++){
		numHand[i] = numhand[i];
		for( j = 0; j < numhand[i]; j++){
			x[i*max+j] = tonex[i*max+j];
			y[i*max+j] = toney[i*max+j];
		}
	}
...
McoStatus Mpfileio::_saveCurveData(FILE *fs, long max, long * numHand, double * x,
									double * y, char *desc, long channel)
{
	int i,j;
