...
{	
	...
	printerData->setblack(_blacktableH);
...
// initialize the PrintData structure to default values and copy
// this data into printerData structure
...
printData->setting.linearDate = 0;	// set to Jan 1, 1970
printData->setting.srcDate = 0;	// set to Jan 1, 1970
printData->setting.dstDate = 0; // set to Jan 1, 1970
		
printData->A2B0.grid_points	= _rev_table_grid;		
		
// copy the tone and linearization curves
status = createCalCurves();
if (status) return status;
	
return copyIntoPrintData();

/*
//P
printData->setting.saturation = 80;			
printData->setting.colorBalance = 100;		
printData->setting.linear = 80;			
printData->setting.smoothSimu = 0;			
printData->setting.kOnly = 1;	
...
// create the calCurves structure, and delete if necessary
McoStatus ProfileDocFiles::createCalCurves(void)
{
if (calCurves) delete calCurves;
calCurves = new CalCurves(printData);
if (calCurves) return MCO_SUCCESS;
return MCO_MEM_ALLOC_ERROR;
}


// copy the data in profileDoc into PrintData
// do this before a save for example
...
status = createCalCurves(); // need to create a Mac specific object if on a Mac
if (status) return status;
//calCurves = new CalCurves(printData);
...
printData->setting.srcDate = simupatchD->date;
printData->setting.dstDate = patchD->date;
...
// copy data outof printData into prfileDoc
// do this after a load for example
...
simupatchD->date = printData->setting.srcDate;
patchD->date = printData->setting.dstDate;
...
// produce a lut for the output table that correct for linearization in the table
...
// this code is used for expanding the data within the 
// table neutral values. It put a S curve in the a & b luts.
...
// produce a lut for the  table that corrects for linearization in the table
// used for cmyk links
...
if ((simupatchD->linearDataH != 0L) && (simupatchD->format.numLinear > 0))
	...
	for(i = 0; i <CT->input_channels; i++)
		...
		for (j=0; j<CT->input_entries; j++)
			{
				lut[j] = 100*(double)j/(CT->input_entries-1.0);   //clut[j]/2.55;
			}
		...
		for(i = 0; i <CT->input_channels; i++)
			...
			for (j=0; j<CT->input_entries; j++)
				{
				lut[j] = 100*(double)j/(CT->input_entries-1.0); //slut[j]/655.35;
				}
			slinear_data->CnvtLinear(lut,CT->input_entries,i);
			for (j=0; j<CT->input_entries; j++)
				{
...
// the one function that is called when a table is to be generated
McoStatus ProfileDocFiles::_handle_calibrate(void)
{
	...
	if ((numbits_icc != 8) && (numbits_icc != 16)) numbits_icc = 8;
...
// reinint the gammut compression
	...
	switch(_condata.tabletype){
		...
		case(5):
		_initicclab();
		if (K_only_cmyk) delete K_only_cmyk;
...
void ProfileDocFiles::SetUpForCancel()
{
	last_changedLinearTone = changedLinearTone;
	last_changedProfile = changedProfile;
}

// restore various parameters after 

void ProfileDocFiles::HandleCancel()
{
	changedLinearTone = last_changedLinearTone;
	changedProfile = last_changedProfile;
}
...
if ((!patchDcopy)) return FALSE;
if ((patchDcopy->type)) return TRUE;
return FALSE;
#else
...
// copy the date from the patch date
printData->setting.linearDate = patchD->date;
...
McoStatus ProfileDocFiles::buildgamut(void)
{
	McoStatus status;
	...
	gammutExpand = (_condata.saturation/100)+1.2*(1-_condata.saturation/100);

	gammutcomp = new GammutComp(patchD,_labtableH, _labtableCopyH,_out_of_gamutH,
		_gammutSurfaceH,_gammutSurfaceBlackH,NULL,NULL,NULL,
		_condata.saturation,_condata.col_balance, _condata.col_bal_cut, l_linear,
		100,0,100,0,0,NULL,thermd);
	
	if (!gammutcomp) return MCO_MEM_ALLOC_ERROR;
