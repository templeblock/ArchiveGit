...
// ty is type of data (cmyk or rgb)
// numc is the number of channels (3 or 4)
// slow is the number of K channels (K changes the slowest within the data)
// linear is the number of linear values
// dims is the dimensions of the cubes
...
// smooths the edges and center of the data, but not the corners
...
// the function that combines averging with median filtering
// This function removes outliers with the median filter and the averages
// the remaing data
void median_avg(double *out,double *in,long *index,double *w,int num);
void median_avg(double *out,double *in,long *index,double *w,int num)
{
int i,n,s;
double d,p;
...
n = num-num/2;
s = (num-n)/2;
new_w = new double[num];

for (i=0; i<num; i++)
	{
	d = fabs((double)(i-(num/2)));
	p = pow((6*d/num),2.2);
	new_w[i] = w[index[i]]*exp(-p);
	}
...
// Smooth the lab space of the patches, smooth L, a, & b
McoStatus RawData::median_patch(void)
{
int32	c,f,h,i,j,k;
int32	im,jm,km;
int32	l,m,n;
	 			...
	 			for (k=1; k<format.cubedim[f]-1; k++)
					...
					for (q=-1; q<=1; q++)
						for (r=-1; r<=1; r++)
							...
							values_1[c] = labh[indexl];
							values_2[c] = labh[indexl+1];
							values_3[c] = labh[indexl+2];
					...
					sort2(9,values_1-1,values_1i-1);
					sort2(9,values_2-1,values_2i-1);
					sort2(9,values_3-1,values_3i-1);
					if (fabs(labh[index]-values_1[5]) > 0.1)
						{
						median_avg(&labh2[index],values_1,values_1i,b,9);
						}
					else labh2[index] = labh[index];
					
					if (fabs(labh[index+1]-values_2[5]) > 0.1)
						{
						median_avg(&labh2[index+1],values_2,values_2i,b,9);
						}
					else labh2[index+1] = labh[index+1];
					
					if (fabs(labh[index+2]-values_3[5]) > 0.1)
						{
						median_avg(&labh2[index+2],values_3,values_3i,b,9);
						}
					else labh2[index+2] = labh[index+2];
	 	...
	 	for (i=1; i<format.cubedim[f]-1; i++)
	 		...
	 		for (j=1; j<format.cubedim[f]-1; j++)
	 			...
	 			for (k=1; k<format.cubedim[f]-1; k++)
					...
					sort2(27,values_1-1,values_1i-1);
					sort2(27,values_2-1,values_2i-1);
					sort2(27,values_3-1,values_3i-1);
					if (fabs(labh[index]-values_1[13]) > 0.1)
						{
						median_avg(&labh2[index],values_1,values_1i,a,27);
						}
					else labh2[index] = labh[index];
					
					if (fabs(labh[index+1]-values_2[13]) > 0.1)
						{
						median_avg(&labh2[index+1],values_2,values_2i,a,27);
						}
					else labh2[index+1] = labh[index+1];
					
					if (fabs(labh[index+2]-values_3[13]) > 0.1)
						{
						median_avg(&labh2[index+2],values_3,values_3i,a,27);
						}
					else labh2[index+2] = labh[index+2];
...
// Do four dimensional interpolation with the data
// 
...
// Do three dimensional interpolation (RGB)
// 
...
// copy one raw data structure into another
...
// copy the patchFormat structure into rawdata
...
// copy raw data into patchFormat
...
tocopy->steps = new long[format.numslow];
if (!tocopy->steps) return MCO_MEM_ALLOC_ERROR;
tocopy->blacks = new double[format.numslow];
if (!tocopy->blacks) return MCO_MEM_ALLOC_ERROR;
tocopy->cmy = new double[dimCount];
