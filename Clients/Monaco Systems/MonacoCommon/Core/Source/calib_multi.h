...
//	virtual McoStatus evalCMY(double *lab,double *cmyk,int32 num,double *m_dist) {return MCO_SUCCESS;}
	...
	virtual double getLab(double *cmyk, double *lab) {return -1;}
};
...
public:
	...
	// look up tables used for mak ink calculations'
	
	// these are used when the table is searched
	double		C_Lut[BT_DIM];
	double		M_Lut[BT_DIM];
	double		Y_Lut[BT_DIM];
	
	// these are used when the actual 
	unsigned char		C_Cal_Lut[800];
	unsigned char		M_Cal_Lut[800];
	unsigned char		Y_Cal_Lut[800];
...
//	McoStatus evalCMY(double *lab,double *cmyk,int32 num,double *m_dist);
	...
	double getLab(double *cmyk, double *lab);
