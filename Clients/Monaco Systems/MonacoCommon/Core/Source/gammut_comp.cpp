		...
		for(j = 0; j < 72; j++){
			...
			for(i = 0; i < 50; i++){
				...
				if (gammutSurfaceManyBlack != 0L)
					...
					if (yes) 
						{
						gammutSurfaceManyBlack[c] = sqrt((boundlab[1]*boundlab[1]) + (boundlab[2]*boundlab[2]));
						gammutSurfaceManyBlackL[c] = boundlab[0];
						}
					else 
						{
						gammutSurfaceManyBlack[c] = 0.0;
						gammutSurfaceManyBlackL[c] = 0.0;
						}
					}
				if (yes)
					{
				...
				for (k=1; k<GAM_DIM_K; k++)
					...
					if (gammutSurfaceManyBlack != 0L)
						...
						if (yes) 
							{
							gammutSurfaceManyBlack[c+3600*k] = sqrt((boundlab[1]*boundlab[1]) + (boundlab[2]*boundlab[2]));
							gammutSurfaceManyBlackL[c+3600*k] = boundlab[0];
							}
						else 
							{
							gammutSurfaceManyBlack[c+3600*k] = 0.0;
							gammutSurfaceManyBlackL[c+3600*k] = 0.0;
							}
						}
...
//	saveGammutSurfaceAsTiff(gammutSurfaceBlack);
			...
			*labcopy++ = B;
			...
			if ((lchin[1] > lchin2[1]) || (fabs(lchin[0]-labin2[0]) > 2.5))// || (L > maxgrayL) || (L < mingrayL))// out of gammut
				...
				*out_of_gamut++ = 255;
				...
				else if (lchin[2] > 305) s = (lchin[2] - 305)/80;
