...
public:
	...
	McoHandle	_out_of_gamutH;  // the 33x33x33 table of out of gamut flags 255 = out of gamut, 0 = in gamut
...
//end of add	
	...
	void 	_applyLabTableCurve(void);
	void 	_buildLabTableLut(double in,double *out);
	...
	// the number of bits used by the icc table
	int		numbits_icc;
	...
	virtual McoStatus saveSettings(void) {return MCO_UNSUPPORTED_OPERATION;}
	...
	McoStatus _build_small_invers(McoHandle BigH,McoHandle Table1H,McoHandle Table2H,double *maxTL,double *minTL,calib_inter4 *calib);
