...
bool CDongle::IsApplication(int which, LPWORD pVersions, LPWORD pFeatures)
{
	...
	*pFeatures = applock->features;
	...
	if (applock->active && applock->count > 0 && applock->count < 100)
	{
		if (applock->date)
		{
			if (MessageConfirm(MESSAGE_DAYS_LEFT, applock->count) == IDNO)
				return true;
		...
		else
		{
			...
			if (MessageConfirm(MESSAGE_COUNT_DECREMENTED, applock->count) == IDNO)
				return true;
	...
	{
		if (MessageConfirm(MESSAGE_APP_INACTIVE) == IDNO)
			return false;
