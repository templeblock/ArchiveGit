...
McoStatus McoXformIcc::buildCW(McoColorSync **colsynList,int32 numProf)
{
	int i;
	OSErr	iErr;
	CMError cmErr;
...
McoStatus McoXformIcc::eval(McoLayout *in, McoLayout *out)
 {
...
McoStatus McoXformIcc::evalPixMap(PixMap *pixmap)
 {
 McoStatus status = MCO_SUCCESS;
 unsigned char *inPtr8, *outPtr8, *inLine8, *outLine8;
 int32 i,j,temp;
...
McoStatus McoXformIcc::evalColors(unsigned char *in, unsigned char *out,int32 num)
 {
 unsigned char *inPtr8, *outPtr8, *inLine8, *outLine8;
 int32 i,j,temp;
 CMBitmap srcmap, desmap;
