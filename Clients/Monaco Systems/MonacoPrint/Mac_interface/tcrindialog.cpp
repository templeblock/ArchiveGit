...
#define	TCR_IDS { 	2,4,5,8,6,7,10,15,16,-1,17,-1,19,400,401,21,22,23,24,26}
/*		2,	\	// Ok_Box
		4,  \	// Save As
		5,  \	// Strip_rect
		8,  \	// Message
...
TCRInDialog::TCRInDialog(ProfileDoc *dc,RawData *pD):TableDialog(dc,pD)
{
	short		iType;
	Handle		iHandle;
	Handle		sHandle;
	...
	int32 		i,wi,hi;
	double w,h;
	short		tcr_ids[] = TCR_IDS;
	PixMapHandle	bigPixMap;
	McoStatus 	state;
	int 		papernum;
	
	for (i=0; i<NumInputIDS; i++) ids[i] = tcr_ids[i];
	...
	// initialize comminications with the tcr
	...
	if (doc->tcom != NULL) ((TCR*)(doc->tcom))->setUpPatches(patchD->patches.columns,patchD->patches.rows,w,h,patchD->patches.starting,patchD->patches.direction,papernum);
...
//	SetPort(oldP);
	...
	GetDItem(dp, ids[Lift], &iType, (Handle*)&iHandle, &pRect);
	SetCtlValue((ControlHandle)iHandle,1);

	((TCR*)(doc->tcom))->doPenDown = 1;
	
	checkFinished();
	
	//McoMessAlert(MCO_WARN_TCR,0L);
	DrawWindow();
}	
...
count = 0;
do {
	state = ((TCR*)(doc->tcom))->getNextPatch(current_patch,&patch_data[current_patch*3]);
...
if (doc->tcom != NULL) tcom->setUpPatches(patchD->patches.columns,patchD->patches.rows,w,h,patchD->patches.starting,patchD->patches.direction,papernum);
...
if (McoMessAlert(MCO_FIND_PATCHS,0L) == MCO_SUCCESS)
	...
	if (state != MCO_SUCCESS) return state;
	tcom->setPoints_2(tablePoints,patchPoints);
	...
	tcom->moveToPatch(current_patch,0);
	_waiting=1;	
	return MCO_SUCCESS;
	}
...
McoStatus TCRInDialog::DoEvents(short item, Point clickPoint, WindowCode *wc, int32 *numwc, void **data, Boolean &changed)
{
...
double 		m=1;
	...
	*numwc = 0;
	...
	if (item != -1) {
		...
		else if (item == ids[Lift])
			{
			((TCR*)(doc->tcom))->doPenDown = 1-((TCR*)(doc->tcom))->doPenDown;
			GetDItem(dp, ids[Lift], &itemtype, (Handle*)&sliderControl, &r);
			SetCtlValue((ControlHandle)sliderControl,((TCR*)(doc->tcom))->doPenDown);		
			}		
		else if (item == OK)
			{
			StorePatches();
