...
void Table::translateToTable(double *patch,int32 *table)
{
double x,y;
...
if ((nx <= max_x) && (nx >= min_x) && (ny <= max_y) && (ny >= min_y))
	...
	if ((nx > max_x) || (nx < min_x))
		...
		a *= -1;
		nx = a*x+b*y+tx+0.5;
		if ((nx > max_x) || (nx < min_x)) a *= -1;
		}
	if ((ny > max_y) || (ny < min_y))
		{
		d *= -1;
		ny = c*x+d*y+ty+0.5;
		if ((ny > max_y) || (ny < min_y)) d *= -1;
		}
	Tx = tx;
	Ty = ty;
	}
...
McoStatus Table::setUpPatches(int32 numCol,int32 numRow,double w,double h,int first, int direction,int papernum)
{
