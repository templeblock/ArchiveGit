...
void CViewGamut::DoFlagCommand(unsigned long flag)
{
	...
	flags &= ~kQ3ViewerShowBadge;
	flags &= ~kQ3ViewerButtonCamera;
	flags &= ~kQ3ViewerDraggingInOff;
	flags &= ~kQ3ViewerButtonReset;
	flags &= ~kQ3ViewerOutputTextMode;
	flags &= ~kQ3ViewerButtonZoom;

	flags |=  kQ3ViewerActive;
	flags |=  kQ3ViewerControllerVisible;
	flags |=  kQ3ViewerButtonDolly;
	flags |=  kQ3ViewerButtonOrbit;
	flags |=  kQ3ViewerButtonTruck;

	// Toggle the flag
	TQ3Status status = Q3WinViewerSetFlags(m_pViewer, flags);
