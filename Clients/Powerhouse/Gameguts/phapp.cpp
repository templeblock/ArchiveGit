...
//***********************************************************************
{
	// Make sure that the Indeo driver is referenced properly in system.ini
	WritePrivateProfileString("drivers32", "VIDC.IV32", "ir32_32.dll",  "system.ini");
...
//***********************************************************************
BOOL CPHApp::PumpMessage( LPMSG pMsg )
//***********************************************************************
{
	if (!pMsg)
		return(TRUE);
	...
	if (TranslateTitleBar( pMsg ))
		return(TRUE);

	// handle any entries in our keymap
	if (TranslateKeyMap( pMsg ))
		return(TRUE);

	// handle any accelerators
	if (m_hAccel && TranslateAccelerator(m_hMainWnd, m_hAccel, pMsg))
		return(TRUE);

	// See if a child window wants the message
	if (DialogMsg( pMsg ))
		return(TRUE);
...
//***********************************************************************
{
	if ( !m_hMainWnd ) // has no main window!
		PostQuitMessage(0);

	// Acquire and dispatch messages until a WM_QUIT message is received.
	while( TRUE )
	{
		// Process all pending messages; fall through when queue is empty
		while ( PeekMessage(&m_msgCur, NULL, NULL, NULL, PM_REMOVE) )
		{
			if (!PumpMessage(&m_msgCur))
				return(TRUE);
		}

		// No more messages; do any idle or background processing

		LONG lIdleCount = 0;
		if ( OnIdle(lIdleCount++) )	// If more idle work to do...
			continue;				// skip the wait for new messages...
									// but go process whatever is there...

		// Wait for any message sent or posted to this queue 
		// or for any of the event handles to be signaled.
		DWORD result = MsgWaitForMultipleObjects(m_iEvents, m_hEvent, 
				 FALSE/*bWaitAll*/, INFINITE/*NoTimeout*/, QS_ALLINPUT); 

		// Something is available; The result tells us the type of event we have
		if (result == (WAIT_OBJECT_0 + m_iEvents))
		{ // A Windows Message is available; loop back up and process it
		} 
		else
		{ 
			// One of the handles became signaled. 
            if( result == WAIT_OBJECT_0 && m_pEventProc[0] )
				(*m_pEventProc[0])(m_dwEventData[0]);
		}
	}
...
//***********************************************************************
{
	return(FALSE); // No more idle work to do
}

//***********************************************************************
BOOL CPHApp::EventAdd( HANDLE hEvent, EVENTCALLBACK pEventProc, DWORD dwEventData )
//***********************************************************************
{
	if ( m_iEvents >= MAX_EVENTS || m_iEvents < 0 )
		return( FALSE );
	m_hEvent[m_iEvents] = hEvent;
	m_pEventProc[m_iEvents] = pEventProc;
	m_dwEventData[m_iEvents] = dwEventData;
	m_iEvents++;
	return( TRUE );
}

//***********************************************************************
BOOL CPHApp::EventRemove( HANDLE hEvent )
//***********************************************************************
{
	int j = 0;
	for ( int i=0; i<MAX_EVENTS; i++ )
	{
		if ( m_hEvent[i] == hEvent )
		...
		{ // Move all others up in the list
			if ( j < i )
			{
				m_hEvent[j] = m_hEvent[i];
				m_pEventProc[j] = m_pEventProc[i];
				m_dwEventData[j] = 	m_dwEventData[i];
				m_hEvent[i] = NULL;
				m_pEventProc[i] = NULL;
				m_dwEventData[i] = NULL;
			}
...
//***********************************************************************
{
	...
	if ( !hPrevInstance && !pApp->InitApplication() )
		return( -1 );
