...
Repeat:
	...
	if ( lNextShot == SHOT_CONT )	// Continue where we left off.
	{
		lNextShot = lCurrentShot;
		if (lpVideo->pAMovie)
			lPrevFrame = lpVideo->pAMovie->GetPosition();
		else
			lPrevFrame = 0;
		goto Repeat;
	}
...
/***********************************************************************/
{
	...
	if ( !lFrame )
	{
		if ( !lpVideo->pAMovie || !(lFrame = lpVideo->pAMovie->GetPosition()) )
			return( NULL );
	}
...
/***********************************************************************/
{
	...
	{ // The shot table entries treats shots as 1-n, not 0-(n-1)
		...
		for ( long idx2 = 1; idx2 <= lpVideo->iNumShots; idx2++ )
		{
			if ( !(lpShot2 = Video_GetShotPtr( lpVideo, idx2 )) )
				continue;

			SHOTID lCurShot = lpShot2->lShotID;
...
/***********************************************************************/
{
	...
	lpVideo->pAMovie->Stop( YES/*bWait*/ );
	lpVideo->pAMovie->Close();
	delete lpVideo->pAMovie;
	lpVideo->pAMovie = NULL;
}
...
/***********************************************************************/
{
	...
	// Just in case a movie is already open
	...
	if ( !(lpVideo->pAMovie = new CAMovie) )
	{
		HourGlass( NO );
		return( NO );
	}

	if ( !(lpVideo->pAMovie->Open( szExpFileName, hWindow, lStartFrame )) )
	{
		HourGlass( NO );
		return( NO );
	}

	lpVideo->lFrames = lpVideo->pAMovie->GetLength();
	lpVideo->wDisk = wDisk;

	lpVideo->pAMovie->SetTimeFormat(YES/*bFrames*/);
...
/***********************************************************************/
{
	...
	{ // if the shot is changing, or a NULL event code...
		...
		if ( lpShot->bGoesPrev )
		{
			...
			{
				lpVideo->lPrevFrame = lpVideo->pAMovie->GetPosition();
...
/***********************************************************************/
{
	LPVIDEO lpVideo = (LPVIDEO)AllocX( sizeof(VIDEO), GMEM_ZEROINIT|GMEM_SHARE );
	if ( !lpVideo )
		return( FALSE );
...
/***********************************************************************/
{
	...
	RECT SrcRect;
	lpVideo->pAMovie->GetSrcRect( &SrcRect );
	RECT DstRect = SrcRect;
	ScaleRect( &DstRect, lpVideo->iZoomFactor );
	lpVideo->pAMovie->SetDstRect( &DstRect );

	int cx = RectWidth(&DstRect);
	int cy = RectHeight(&DstRect);
...
{ // Timer went off, so jump to the desired shot
	...
	if (id == lpVideo->idPromptTimer )
	{
		if (lpVideo->lPromptShot)
		{
...
/***********************************************************************/
{
	...
	if ( lpShot->bLeftIsSubShot )
	{
		...
		{
			if ( !(lFrame = lpVideo->pAMovie->GetPosition()) )
				goto TryRight;
		}
		if ( !(lpShot = Video_FindSubShot( lpVideo,
...
/***********************************************************************/
{
	...
	int x = (pt.x * 10) / RectWidth( &rWindow );
	int y = (pt.y * 10) / RectHeight( &rWindow );
...
/***********************************************************************/
{
	...
	switch ( vk )
	{
		...
		case VK_PAUSE:
		{
			if ( !lpVideo->pAMovie )
				break;
 			if ( lpVideo->pAMovie->IsPaused() )
				lpVideo->pAMovie->Resume();
			else
			if ( lpVideo->pAMovie->IsPlaying() )
				lpVideo->pAMovie->Pause();
			break;
		}
...
//jm		if ( !lpVideo->lpEventProc || !(*lpVideo->lpEventProc)( hWindow, lpVideo, EVENT_END ) )
...
//jm		if ( !lpVideo->lpEventProc || !(*lpVideo->lpEventProc)( hWindow, lpVideo, EVENT_LEFT ) )
...
//jm		if ( !lpVideo->lpEventProc || !(*lpVideo->lpEventProc)( hWindow, lpVideo, EVENT_UP ) )
...
//jm		if ( !lpVideo->lpEventProc || !(*lpVideo->lpEventProc)( hWindow, lpVideo, EVENT_RIGHT ) )
...
//jm		if ( !lpVideo->lpEventProc || !(*lpVideo->lpEventProc)( hWindow, lpVideo, EVENT_DOWN ) )
...
//jm		if ( !lpVideo->lpEventProc || !(*lpVideo->lpEventProc)( hWindow, lpVideo, EVENT_HOME ) )
...
//jm		if ( !lpVideo->lpEventProc || !(*lpVideo->lpEventProc)( hWindow, lpVideo, EVENT_INS ) )
...
//jm		if ( !lpVideo->lpEventProc || !(*lpVideo->lpEventProc)( hWindow, lpVideo, EVENT_DEL ) )
		...
		{
			if ( lpVideo->pAMovie )
 				lpVideo->pAMovie->SetAudioOnOff( OFF );
			break;
		}
		...
		{
			if ( lpVideo->pAMovie )
				lpVideo->pAMovie->SetAudioOnOff( ON );
			break;
		}
		...
		{
			if ( lpVideo->pAMovie )
				lpVideo->pAMovie->SetVideoOnOff( OFF );
			break;
		}
		...
		{
			if ( lpVideo->pAMovie )
				lpVideo->pAMovie->SetVideoOnOff( ON );
			break;
		}
		...
		case VK_MULTIPLY:
		{
			iSpeed = 0;
			if ( lpVideo->pAMovie )
				lpVideo->pAMovie->SetVideoSpeed( 1000L + iSpeed );
			break;
		}
...
/***********************************************************************/
{
	...
	if ( iEventCode )
	{
//jm	if ( !lpVideo->lpEventProc || !(*lpVideo->lpEventProc)( hWindow, lpVideo, iEventCode ) )
...
/***********************************************************************/
{
	...
	lpVideo->pAMovie->SetAudioOnOff( bOn );
}
...
/***********************************************************************/
void Video_HookMouseButtons( LPLBUTTONHOOK lpLeftProc, LPRBUTTONHOOK lpRightProc )
/***********************************************************************/
{
	if (lpLeftProc)
		lpLButtonDown = lpLeftProc;
	if (lpRightProc)
