...
//************************************************************************
HPALETTE CreateIdentityPalette( LPRGBQUAD pColorTable, int nColors, BOOL bMakeIdentity )
//************************************************************************
{
	if ( !pColorTable )
		return( NULL );
	...
	if ( !(lpPalette = (LPLOGPALETTE)AllocX(sizeof(LOGPALETTE)+ (256 * sizeof(PALETTEENTRY)), GMEM_ZEROINIT)) )
		return(NULL);
	...
	for ( int i = 0; i < nColors; ++i)
	{
		...
		lpPalette->palPalEntry[i].peFlags = 0/*PC_NOCOLLAPSE*/;
	}
...
{
	UINT nColors;
	GetObject(hPal, 2, (LPSTR)&nColors);
	if (nColors <= 0)
		return(FALSE);
	
	LPPALETTEENTRY lpEntry;
	if ( !(lpEntry = (LPPALETTEENTRY)AllocX(256 * sizeof(PALETTEENTRY), GMEM_ZEROINIT)) )
		return(FALSE);
	
	GetPaletteEntries(hPal, 0, nColors, lpEntry);

	if (!(nColors = DoCreateIdentityPalette(lpEntry, nColors)))
	{
		FreeUp(lpEntry);
		return(FALSE);
	}
	...
	SetPaletteEntries(hPal, 0, nColors, lpEntry);
	FreeUp(lpEntry);
	return(TRUE);
}
...
//************************************************************************
{
	...
	if (GetDeviceCaps(hdc, RASTERCAPS) & RC_PALETTE)
		...
		if (GetSystemPaletteUse(hdc) == SYSPAL_NOSTATIC)
			...
			if ( nColors > 256 - nStaticColors ) nColors = 256 - nStaticColors;
			for (; i<nColors; i++)
...
/***********************************************************************/
{
	...
	lpEntry1 = (LPPALETTEENTRY)AllocX(sizeof(PALETTEENTRY)*256, GMEM_ZEROINIT);
	lpEntry2 = (LPPALETTEENTRY)AllocX(sizeof(PALETTEENTRY)*256, GMEM_ZEROINIT);
