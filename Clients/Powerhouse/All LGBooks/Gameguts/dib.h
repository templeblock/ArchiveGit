...
public:
	...
	static CDib FAR *LoadDibFromFile(LPCSTR szFile, HPALETTE hPal, BOOL bMapToLevels = FALSE, BOOL fDecodeRLE = FALSE, BOOL bBrowse = TRUE);
	...
	BOOL ConvertData( LPTR lpData, UINT biBits, DWORD biStyle, UINT wUsage );
	class CDib FAR * DibCopy(BOOL fCopyBits = TRUE);
	HBITMAP CreateBitmap( HPALETTE hpal, LPTR lpData );
	class CDib FAR * HalftoneDIB( HPALETTE hPal );

	// DIB manipulation
	int MapToPalette( HPALETTE hpal, LPTR lpInUse = NULL ); 
