...
//************************************************************************
{
	...
	{
		int j;
		for( j=i+1; j<nNumVideoTimers; j++, i++ )
		{
...
//***************************************************************************
{
	static PALETTEENTRY PalEntries[256];
	UINT uNumEntries = 256;

	if (!pinst)
		return FALSE;

	HPALETTE hPal = DrawDibGetPalette(pinst->hdd);

	uNumEntries = GetPaletteEntries(hPal, 0, 256, PalEntries);

	for(UINT i=0; i<uNumEntries; i++)
	{
		//PalEntries[i].peRed += (255 - PalEntries[i].peRed / 2);
		//PalEntries[i].peGreen += (255 - PalEntries[i].peGreen / 2);
		//PalEntries[i].peBlue += (255 - PalEntries[i].peBlue / 2);
		PalEntries[i].peRed = (PalEntries[i].peRed + 255) / 3;
		PalEntries[i].peGreen = (PalEntries[i].peGreen + 255) / 2;
		PalEntries[i].peBlue = (PalEntries[i].peBlue + 255) / 3;
	}

	//Debug("ChangePal\n");
	//BOOL bRet = DrawDibSetPalette(pinst->hdd, hPal);
	DrawDibChangePalette(pinst->hdd, 0, uNumEntries, PalEntries );
...
//***************************************************************************
{
	...
	*/
	...
	//lTimerId =  timeSetEvent(66, 0, TimeProc, 0, 0);
...
//***************************************************************************
{
	// delete temporary bitmap data
	//Debug("VideoEndInterim\n");
...
#ifdef PROFILE
char debug[81];
wsprintf(debug, "AVI Draw time = %ld\n", lTotalAVIDraw / lCountAVIDraw);
OutputDebugString(debug);
wsprintf(debug, "Notify time = %ld\n", lTotalNotify / lCountNotify);
OutputDebugString(debug);
wsprintf(debug, "Dib Draw time = %ld\n", lTotalDibDraw / lCountDibDraw);
OutputDebugString(debug);
#endif
	// If a timer has been setup
	if (uTimerId)
	{
...
//***************************************************************************
{
	...
	case ICM_DRAW_BEGIN: // Prepare to draw the video data
		...
		{
			//char debug[81];
			//long lEndTime = timeGetTime() - lStartTime;
			//wsprintf(debug,"time = %ld\n", lEndTime);
			//Debug(debug);
			//lStartTime = 0;
		}
