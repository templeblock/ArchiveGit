...
/***********************************************************************/
{
	...
	ATOM atom;
	if ( !(atom = GlobalFindAtom(lpString)) )
		return( NULL );
...
/***********************************************************************/
{
	...
	STRING szString;
	GlobalGetAtomName((ATOM)lShot, szString, sizeof(STRING) );
...
/***********************************************************************/
{
	...
	FNAME szFileName, szExpFileName;
	...
	if ( !GetApp()->FindContent( szFileName, szExpFileName, NO ) )
	{
	...
	if ( !(lpVideo->hDevice = MCIOpen( GetApp()->m_hDeviceAVI, szFileName, szAlias, hWindow, 
		lStartFrame )) )
	{
		HourGlass( NO );
		return( NO );
...
/***********************************************************************/
{
	...
	if ( !Video_Open( hWindow, lpVideo, (lpShot ? lpShot->wDisk : 1), (lpShot ? lpShot->lStartFrame : 0) ) )
...
/***********************************************************************/
{
	...
	for (int i=0; i < lpVideo->iNumShots; i++)  {
		ATOM atm = lpVideo->lpAllShots[i].aShotID;
...
/***********************************************************************/
{
	...
	switch ( vk )
	{
		...
		case VK_END:
		{
			...
			GlobalGetAtomName( lpShot->aShotID, szString, sizeof(STRING) );
