...
/**********************************************************************************************/
{
	...
	InitWeapons(hWnd);
		...
		Weapons.SetAmmo(1, -1); // laser
		Weapons.SetAmmo(2, AMMO_MGUN); // machine gun;
		Weapons.SetAmmo(3, AMMO_MISSILES); // missiles
		Weapons.SetAmmo(4, AMMO_MINES);
		Weapons.SetAmmo(5, AMMO_BOMBS);
		Weapons.SetAmmo(6, AMMO_JACKS);
		Weapons.SetAmmo(7, AMMO_LEROYS);
		Weapons.SetAmmo(8, AMMO_THUMPER);
	#else
		Weapons.SetAmmo(0, -1); // scrap gun
		Weapons.SetAmmo(1, -1); // laser
		Weapons.SetAmmo(2, 0); // machine gun;
		Weapons.SetAmmo(3, 0); // missiles
		Weapons.SetAmmo(4, 0);
		Weapons.SetAmmo(5, 0);
		Weapons.SetAmmo(6, 0);
		Weapons.SetAmmo(7, 0);
...
//************************************************************************
{
	...
	switch (id)
	{
		...
		case '9':
		{
			int iCurrentWeapon = id - '1';
			Weapons.SetCurWeapon(iCurrentWeapon);
		...
		{	
			if (Weapons.GetCurWeapon() == 9)
				...
				Weapons.SetCurWeapon( Weapons.GetCurWeapon() + 1 );
...
{ // return whether or not the item was removed from the portal
	...
	// Check for missiles
	else if (cTest == 'R' || cTest == 'r')
	{
		nSetWeapon = 3;
		nAmmo = AMMO_MISSILES/5;
		nMaxAmmo = AMMO_MISSILES;
	}

	// Check for mines
	else if (cTest == 'S' || cTest == 's')
	{
		nSetWeapon = 4;
		nAmmo = AMMO_MINES/5;
		nMaxAmmo = AMMO_MINES;
	...
	// Check for bombs (spine killers)
	else if (cTest == 'K' || cTest == 'k')
	{
		nSetWeapon = 5;
		nAmmo = AMMO_BOMBS/5;
		nMaxAmmo = AMMO_BOMBS;
	...
	if (nSetWeapon > -1)
	{
		Weapons.AddAmmo(nSetWeapon, nAmmo);
		if ( Weapons.GetAmmo(nSetWeapon) > nMaxAmmo) 
			Weapons.SetAmmo(2, nMaxAmmo);

		// Change weapons
		if (Weapons.GetCurWeapon() < nSetWeapon)
...
/***********************************************************************/
{
	...
	if (iWeapon == 2)
		SetWeaponDigitDisplay(hWnd, Weapons.GetAmmo(iWeapon)/ROUNDS_PER_CLIP);
	else
		SetWeaponDigitDisplay(hWnd, Weapons.GetAmmo(iWeapon));

	SendDlgItemMessage( hWnd, IDC_WEAPON, SM_SETICONID, IDC_WEAPON + iWeapon + 1, 0L );
	SendDlgItemMessage( hWnd, IDC_AMMOTYPE, SM_SETICONID, IDC_AMMOTYPE + iWeapon + 1, 0L );
}
...
//************************************************************************
{
	...
	int iCurrentWeapon = Weapons.GetCurWeapon();
	...
	{			
		if (!Weapons.GetAmmo(iCurrentWeapon))
			return;
	}	
	...
	{
		Weapons.DecAmmo(iCurrentWeapon, 1);

		int iAmmo = Weapons.GetAmmo(iCurrentWeapon);
		if (iCurrentWeapon == 2)
		...
		if (App.bEnergyCheat)
		{
			...
			if ( iCurrentWeapon == 2)
				...
				iMax = AMMO_THUMPER;
			Weapons.SetAmmo(iCurrentWeapon, iMax);
		}
	}
}
