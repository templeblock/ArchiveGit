...
//************************************************************************
PDIB CDib::LoadDibFromFile(LPCTSTR szFile, HPALETTE hPal, BOOL bMapToLevels, BOOL fDecodeRLE)
//************************************************************************
{
	PDIB pHTdib;
	PDIB pdib = new CDib();
	if (!pdib)
		return(NULL);
	if (!pdib->LoadFromFile(szFile, fDecodeRLE))
	{
		delete pdib;
		return(NULL);
	}
...
//************************************************************************
BOOL CDib::LoadFromFile(LPCTSTR szFile, BOOL fDecodeRLE)
//************************************************************************
{
	HFILE fh;
	DWORD dwBits;
	...
	if ( (fh = OpenFile(szFile, &of, OF_READ)) < 0 )
		return(FALSE);
	...
	if ( !(m_lp = (LPTR)calloc(1, dwBits = GetSizeImage() )) )
		{
		_lclose(fh);
		return( FALSE );
		}
...
/***********************************************************************/
PDIB CDib::LoadDibFromResource(HINSTANCE hInstance, LPCTSTR lpResource, HPALETTE hPal, BOOL bMapToLevels, BOOL fDecodeRLE)
/***********************************************************************/
{
	PDIB pHTdib;
	PDIB pdib = new CDib();
...
//************************************************************************
BOOL CDib::LoadFromResource(HINSTANCE hInstance, LPCTSTR szResource, BOOL fDecodeRLE)
//************************************************************************
{
	HRSRC hDibRes;
	HGLOBAL hDib;
	...
	// Can we get enough memory to hold the DIB bits
	if ( (m_lp = (LPTR)calloc(1, dwBits = GetSizeImage() )) != NULL )
...
//************************************************************************
BOOL CDib::WriteFile( LPCTSTR szFile )
//************************************************************************
{
	...
	if ( (fh = OpenFile( szFile, &of, OF_CREATE|OF_READWRITE )) < 0 )
		return FALSE;
...
BOOL CDib::DibFlip()
{
	...
	if (cLine >= 2) 
	{
		if (cbLine > 640)
		{
			if ((lpTemp = (HPTR)malloc(cbLine)) == NULL)	
				return FALSE;
		}
		else
		{
...
//************************************************************************
{
	...
	if ( !(lp = (LPTR)calloc(1, GetSizeImage() )) )
	{
		return( NULL );
	}
...
//************************************************************************
{
	...
	if ( !(m_lp = (LPTR)calloc(1, dwSizeImage + 1024 )) )
		{
		return FALSE;
		}
...
//************************************************************************
{
	...
	if ( !(m_lp = (LPTR)calloc(1, GetSizeImage() )) )
		{
		goto exit;
		}
