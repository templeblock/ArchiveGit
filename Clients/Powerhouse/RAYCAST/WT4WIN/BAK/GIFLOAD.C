...
** by Chris Thomas (GCT).  You'll see a GCT or WIN32 next to my changes.
** Largest change is loading palette from WT's path.
** Also didn't have M_PI_2 declared (at least back when I was experimenting
** with WT using Watcom 9.0...).
*/
...
//GCT: don't have M_PI_2.  define it.
#ifndef M_PI_2
#define M_PI_2 (3.141589*2.0)
#endif
...
Color palette[PALETTE_ENTRIES];


int   ReadCode( void );
//int   log2( int );    //GCT: WC 9.0 pukes on this declaration...
...
/*
 * Calculate with colors as vectors
 */
#define sqr(x) ((x)*(x))
#define Norm(V) (sqrt(sqr(V.red) \
		      +sqr(V.green) \
		      +sqr(V.blue)))

#define vminus(V1,V2,V3) \
{ \
    (V3).red=(V1).red-(V2).red; \
    (V3).green=(V1).green-(V2).green; \
    (V3).blue=(V1).blue-(V2).blue; \
}
    
#define VectorDot(V1,V2) ((V1).red*(V2).red \
			 +(V1).green*(V2).green \
			 +(V1).blue*(V2).blue)

#define Winkel(V1,V2) (acos(fabs((double)(VectorDot((V1),(V2)))) \
			    /(Norm(V1)*Norm(V2))))
...
#ifdef WIN32
	//GCT load palette file from WT's path.
