...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="drawicons" select="/linelist/variables/var[@name='drawicons']/@value" />
	<xsl:variable name="filename" select="'Thumbnails'" />
	<xsl:variable name="folder-indent" select="'12'" />
	<xsl:variable name="line-height" select="'20'" />
	<xsl:variable name="pad-filename" select="'30'" />
	...
	<!--xsl:variable name="thumbslist-path" select="''" /-->
	...
	<xsl:template match="/linelist">
		<html lang="en" xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
			<head>
				...
				<style type="text/css">
					body
						{ font-family:Verdana,sans-serif; font-size:8pt; margin:0px; }
					body, div
				...
				<script language="javascript">
					...
					function OnClickFolder(LinkNode)
					{
					...
					function OnClickFile(LinkNode, strName)
					{
						var FolderNode = LinkNode.parentNode;
						OnAutoFile(LinkNode.parentNode, strName);
					}
					...
					//j	CollapsePrevious(LastActiveFolderNode, FolderNode);
					...
					function HiliteNode(FolderNode, bOn)
					{
						...
						Node.style.backgroundColor = (!bOn ? DefaultBGColor : '#f0f0f0');
					}
					...
					function SetFirstThumbsURLHere()
					{
						var x1 = '<xsl:value-of select="$thumbslist-path" />' + '<xsl:value-of select="descendant::file[1]/@name"/>';
						mfilename.value = x1;
					}
			...
			<body onload="OnLoad()">
				<input type="hidden" name="mfilename" id="mfilename" value=''/>
				<xsl:apply-templates select="folder">
					<xsl:sort select="@name" />
					<xsl:with-param name="depth" select="0" />
				</xsl:apply-templates>
				<script>
					SetFirstThumbsURLHere();
				</script>
			</body>
		</html>
	</xsl:template>
	...
	<xsl:template match="folder">
		...
		<div>
			...
			<span class="clickable" onclick="OnClickFolder(this)" style="margin-left:{$indent}; ">
	...
	<xsl:template match="file">
		...
		<div id="{@type}" class="{@name}">
			...
			<span class="clickable" onclick="OnClickFile(this, '{@name}')" style="margin-left:{$indent}; ">
				<xsl:if test="$drawicons='1'">
					<img class="icon" src="{$img-folder}/{@type}.gif" />
				</xsl:if>
