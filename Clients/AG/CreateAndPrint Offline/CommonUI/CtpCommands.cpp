...
const int WORKSPACETOP_OFFSET = 51;
/////////////////////////////////////////////////////////////////////////////
...
LRESULT CDocWindow::OnDocEnvelopeWiz(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	return LaunchEnvelopeWizard(m_pAGDoc->GetDocType(), false/*bAtPrintTime*/);
}

/////////////////////////////////////////////////////////////////////////////
LRESULT CDocWindow::LaunchEnvelopeWizard(AGDOCTYPE DocType, bool bAtPrintTime)
{
	if (!m_pAGDoc)
		return E_FAIL;
...
/////////////////////////////////////////////////////////////////////////////
LRESULT CDocWindow::Close(bool bExit)
{
	if (!m_pAGDoc)
		return E_FAIL;
	...
	if (m_HTMLModelessDlg.m_hWnd && ::IsWindow(m_HTMLModelessDlg.m_hWnd))
		m_HTMLModelessDlg.SendMessage(WM_CLOSE);
...
//j	if (m_Undo.CanUndo()) // If there have been any edits made, prompt to save the document
	...
	{  // If there have been any edits made, prompt to save the document
		...
		int iResponse = CMessageBox::Message(
			String("The contents of '%s' has changed.\n\nDo you want to save the changes?\n", strFileName),
			(bExit ? MB_YESNO | MB_SYSTEMMODAL : MB_YESNOCANCEL));
...
//LRESULT CDocWindow::OnRotateBitmap(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
//{
//	//CString strFileName("C:\\Documents and Settings\\BPendekanti\\Desktop\\3078364T.bmp");
//
...
//
//	//CImageRotator ImgRot;
//	//ImgRot.Rotate(10);
//	//sizeDest = ImgRot.CalcDestSize(sizeDest);
...
//	//bi.bmiHeader.biSizeImage = Image.GetWidth() * 4 * Image.GetHeight(); // 32 bit
...
//	//HDC hdc = GetDC(NULL);
//
//	//if (!GetDIBits(hdc, (HBITMAP)Image, 0, nHeight, pBits, &bi, DIB_RGB_COLORS))
//	//{
//	//	delete pBits;
//	//	pBits = NULL;
//	//}
...
//	//BOOL bRes = ImgRot.ProcessPixels(pSrcPixels, sizeSrc, pDestPixels, sizeDest);
//
...
//j				RECT DestRect = pSym->GetDestRect();
				...
				if (pSym->IsImage())
				{
					...
					CString strNativeType = pSymImage->GetNativeType();
					CString strFileName = "Unknown";
					BYTE* pMemory = NULL;
					DWORD dwSrcSize = 0;

					// Try to read in the archived file data
					HGLOBAL hMemory = pSymImage->RestoreNativeData();
					if (hMemory)
					{
...
/////////////////////////////////////////////////////////////////////////////
void CDocWindow::CloseModelessDlg()
{
	if (m_HTMLModelessDlg.m_hWnd && ::IsWindow(m_HTMLModelessDlg.m_hWnd))
		m_HTMLModelessDlg.PostMessage(WM_CLOSE);
}
...
LRESULT CDocWindow::OnDocPrint(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	//Handle online users
	CString strURL;
	CString strPrintStatus;
	bool bAllowLocalPrint = false;

	if (m_HTMLModelessDlg.m_hWnd && ::IsWindow(m_HTMLModelessDlg.m_hWnd))
	{
		m_HTMLModelessDlg.DoModeless(NULL);
		return S_OK;
	}
	...
	bool bAllowRemotePrint = (!bBlueMountain && !bDisney && !m_bMetricMode && !m_pCtp->IsAIM() && m_pAGDoc->NeedsCardBack() && !m_pCtp->IsApplication() && !m_pAGDoc->HasLockedImages());

	// Need to display this only for americangreetings.com 
	CString strRetval = "LOCAL";
	if (m_pCtp->IsAmericanGreetings())
	{
		if ( (bAllowRemotePrint) || 
			 (!bAllowRemotePrint && !bFreeContent && !bPaidMember))
		{
	...
	if (strRetval == "REMOTE")
	{
		...
		m_HTMLModelessDlg.SetWindowSize(720, 600);
		m_HTMLModelessDlg.SetDispatch(pDisp);
		m_HTMLModelessDlg.SetURL(strURL, false);
		m_HTMLModelessDlg.DoModeless(hWndParent);
...
//	CString strPrintStatus;
//	bool bAllowLocalPrint = false;
//	if (!m_pCtp->IsApplication())
//	{
//		if (m_HTMLPrintdlg.m_hWnd && ::IsWindow(m_HTMLPrintdlg.m_hWnd))
//		{
//			m_HTMLPrintdlg.DoModeless(NULL);
//			return S_OK;
//		}
//
//		// Need a fix for applications and offline guys
//		// Member Number
//		CString strMemNum = "memnum=";
//		strMemNum += m_pCtp->GetContextMemberNumber();
//
//		// Member Status
//		CString strMemStatus = "memstat=";
//		strMemStatus += m_pCtp->GetContextMemberStatus();
//
//		// Item Info
//		CString strItemInfo = "iteminfo=";
//		strItemInfo += m_pCtp->GetContextItemInfo();
//
//		// Allow Remote
//		bool bBlueMountain = m_pCtp->IsBlueMountain();
//		bool bDisney = (m_pCtp->GetContextLicense() == 283);
//		bool bAllowRemotePrint = (!bBlueMountain && !bDisney && !m_bMetricMode && !m_pCtp->IsAIM() && !m_pCtp->IsApplication() && m_pAGDoc->NeedsCardBack() && !m_pAGDoc->HasLockedImages());
//		CString strRemotePrint = "remoteprint=";
//		strRemotePrint += (bAllowRemotePrint?"YES":"NO");
//
//		strURL = "HTTP://localhost/DataUpload/PrintMethodAg.htm?";
//		strURL += strMemNum + "&";
//		strURL += strMemStatus + "&";
//		strURL += strItemInfo + "&";
//		strURL += strRemotePrint + "&";
//		
...
//			return S_OK;
//
//		CString strControlName = "PrintStatus";
//		dlg.GetControlValue(strControlName, strPrintStatus);
//
//		strControlName = "URL";
//		strURL.Empty();
//		dlg.GetControlValue(strControlName, strURL);
//	}
//	else
//		bAllowLocalPrint = true;
//
//	if (strPrintStatus == "REMOTE")
//	{
//		bool bDevelopment = m_pCtp->IsDevelopmentServer();
//		if (bDevelopment)
//			strURL = "http://sitetest.4yoursoul.com/31/ClientInterface/MemberCnP.asp";
...
//
//		#ifdef NOTUSED
//		//strURL = "HTTP://localhost/DataUpload/FYS_Home_1.htm";
//		strURL = "http://localhost/DataUpload/MemberCnP.asp";
//		#endif NOTUSED
//
//		// build the product number
//		strURL += "?prodnum=";
...
//
//		HWND hWndParent = ::GetActiveWindow();
//		m_HTMLPrintdlg.SetWindowSize(720, 600);
//		m_HTMLPrintdlg.SetDispatch(pDisp);
//		m_HTMLPrintdlg.SetURL(strURL, false);
//		m_HTMLPrintdlg.DoModeless(hWndParent);
//
//		return S_OK;
//	}
//
//	if (strPrintStatus == "SIGNIN" || strPrintStatus == "JOINNOW")
//	{
//		CComQIPtr<IDispatch> pDisp = m_pCtp;
//
//		HWND hWndParent = ::GetActiveWindow();
...
//
//	HRESULT hr;
//	if (strPrintStatus == "LOCAL" || bAllowLocalPrint)
//		hr = (m_Print.Start() ? S_OK : E_FAIL);
//
//	if (m_pCtp->AutoPrintEnabled())
//		m_pCtp->AutoCloseBrowser();
...
//	CWaitCursor Wait;
//
//	if (m_FYSPrintdlg.m_hWnd && ::IsWindow(m_FYSPrintdlg.m_hWnd))
//	{
//		m_FYSPrintdlg.DoModeless(NULL);
//		m_FYSPrDlghWnd = m_FYSPrintdlg.m_hWnd;
//
//		return S_OK;
//	}
//
//	bool bAllowLocalPrint = true;
//
//#ifdef _ENABLE_SIGNIN
//	bAllowLocalPrint = m_pCtp->AllowLocalPrint();
//	if (!bAllowLocalPrint)
//	{
//		CHtmlDialog dlg("SignInAg.htm", true, ::GetActiveWindow());
//		dlg.SetWindowSize(600, 300);
//		dlg.SetResizable(false);
//		dlg.SetScrollbars(false);
//		if (!dlg.DoModal())
//			return S_OK;
//
//		bAllowLocalPrint = (dlg.GetReturnString() == "TRUE");
//	}
//#endif
//
//	bool bBlueMountain = m_pCtp->IsBlueMountain();
...
//	if (bAllowRemotePrint)
//	{
//		CHtmlDialog dlg("PrintMethodAg.htm", true, ::GetActiveWindow());
//		dlg.SetWindowSize(600, 300);
//		dlg.SetResizable(false);
//		dlg.SetScrollbars(false);
//		if (!dlg.DoModal())
//			return S_OK;
//
//		bAllowRemotePrint = (dlg.GetReturnString() == "REMOTE");
//	}
//
//	if (bAllowRemotePrint)
//	{
//		// build the product number
//		CString strProduct("?prodnum=");
//		strProduct += m_pCtp->GetContextProductNumber();
//
//		CString strURL;
//		bool bDevelopment = m_pCtp->IsDevelopmentServer();
//		if (bDevelopment)
...
//
//#ifdef _HTMLMODALDLG
//		CHTMLDialogEx FYSPrintdlg;
//
//		FYSPrintdlg.SetWindowSize(720, 600);
//		FYSPrintdlg.SetDispatch(pDisp);
//		FYSPrintdlg.SetURL(strURL, false);
//		FYSPrintdlg.SetDocWindow(this);
//		FYSPrintdlg.DoModal();
//#else
//		HWND hWndParent = ::GetActiveWindow();
//		m_FYSPrintdlg.SetWindowSize(720, 600);
//		m_FYSPrintdlg.SetDispatch(pDisp);
//		m_FYSPrintdlg.SetURL(strURL, false);
//		m_FYSPrintdlg.DoModeless(hWndParent);
//		m_FYSPrDlghWnd = m_FYSPrintdlg.m_hWnd;
//#endif _HTMLMODALDLG
//
//		return S_OK;
...
//		hr = (m_Print.Start() ? S_OK : E_FAIL);
//
//	// If the plugin is only running to print and exit, let's exit now
//	if (m_pCtp->AutoPrintEnabled())
//		m_pCtp->AutoCloseBrowser();
//
//	return hr;
...
LRESULT CDocWindow::AddImage(LPCSTR strFileName, int iOffset, int x, int y, bool bImageDropped)
{
	...
	if (strExtension == ".png")
	{
		...
		lResult = AddImage(hMemory, true/*bIsDIB*/, iOffset, x, y, bImageDropped);
		::GlobalFree(hMemory);
	}
	else
	if (strExtension == ".jpg" || strExtension == ".wmf" || strExtension == ".emf" || strExtension == ".bmp")
	{
		...
		lResult = AddImage(hMemory, (strExtension == ".bmp")/*bIsDIB*/, iOffset, x, y, bImageDropped);
	}
	else
...
/////////////////////////////////////////////////////////////////////////////
LRESULT CDocWindow::AddImage(HANDLE hMemory, bool bIsDIB, int iOffset, int x, int y, bool bImageDropped)
{
	CWaitCursor Wait;
	...
	//bpp 01/26/05
	CAGSym* pAddAPhotoSym = NULL;
	int iAddAPhotoOffset = iOffset;
	POINT pt = {x, y};

	::ScreenToClient(m_hWnd, &pt);
	m_pClientDC->GetViewToDeviceMatrix().Inverse().Transform(pt);
	pAddAPhotoSym = pActiveLayer->FindAddAPhotoSymByPoint(pt, GetHiddenFlag());
	...
	CToolsCtlPanel* pCtlPanel = m_pCtp->GetCtlPanel(SELECTED_PANEL);
	if (pCtlPanel)
...
LRESULT CDocWindow::AddTextbox(SYMTYPE SymType)
{
	...
	CToolsCtlPanelText *pToolsTxtPanel = (CToolsCtlPanelText*)m_pCtp->GetCtlPanel(TEXT_PANEL);
	if (!pToolsTxtPanel)
		return E_FAIL;
	...
	if (!bAddAPhoto)
	{
		int nItem = pToolsTxtPanel->SendDlgItemMessage(IDC_FONT, CB_GETCURSEL, 0, 0);
		if (nItem >= 0)
		{
			int nFont = pToolsTxtPanel->SendDlgItemMessage(IDC_FONT, CB_GETITEMDATA, nItem, 0);
			if (nFont >= 0)
			{
	...
	COLORREF FillColor = (!bAddAPhoto ? pToolsTxtPanel->GetCurrentFillColor() : RGB(0,0,0));
	COLORREF FillColor2 = (!bAddAPhoto ? pToolsTxtPanel->GetCurrentFillColor2() : RGB(0,0,0));
	COLORREF LineColor = (!bAddAPhoto ? pToolsTxtPanel->GetCurrentLineColor() : RGB(0,0,0));
	FillType Fill = (!bAddAPhoto ? pToolsTxtPanel->GetCurrentFillType() : FT_Solid);
	int LineWidth = (!bAddAPhoto ? pToolsTxtPanel->GetCurrentLineWidth() : LT_None);

	// Get the size
	int TextSize = (!bAddAPhoto ? pToolsTxtPanel->GetCurrentTextSize() : POINTUNITS(16));
	...
	if (!bAddAPhoto)
	{
		if (pToolsTxtPanel->IsDlgButtonChecked(IDC_BOLD))
			Font.lfWeight = FW_BOLD;
		if (pToolsTxtPanel->IsDlgButtonChecked(IDC_ITALIC))
			Font.lfItalic = true;
		if (pToolsTxtPanel->IsDlgButtonChecked(IDC_UNDERLINE))
	...
	if (!bAddAPhoto)
	{
		if (pToolsTxtPanel->IsDlgButtonChecked(IDC_TOP))
			eVertJust = eVertTop;
		else
		if (pToolsTxtPanel->IsDlgButtonChecked(IDC_MIDDLE))
			eVertJust = eVertCentered;
		else
		if (pToolsTxtPanel->IsDlgButtonChecked(IDC_BOTTOM))
	...
	if (!bAddAPhoto)
	{
		if (pToolsTxtPanel->IsDlgButtonChecked(IDC_LEFT))
			eHorzJust = eFlushLeft;
		else
		if (pToolsTxtPanel->IsDlgButtonChecked(IDC_CENTER))
			eHorzJust = eRagCentered;
		else
		if (pToolsTxtPanel->IsDlgButtonChecked(IDC_RIGHT))
...
LRESULT CDocWindow::OnDocAddCalendar(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	CToolsCtlPanelCal* pCtlPanel = (CToolsCtlPanelCal*)m_pCtp->GetCtlPanel(CALENDAR_PANEL);
	if (!pCtlPanel)
		return E_FAIL;

	// Create the object at the default size
	SIZE Size = {INCHES(5.0), INCHES(5.0)};
...
LRESULT CDocWindow::AddShape(LPCSTR strShapeName, bool bFilled)
{
	...
	CToolsCtlPanelGraphics* pToolsPanel = (CToolsCtlPanelGraphics*)m_pCtp->GetCtlPanel(GRAPHICS_PANEL);
	if (!pToolsPanel)
		return E_FAIL;

	// Create the object at the default size
	SIZE Size = {INCHES(2.0), INCHES(2.0)};

	CAGSymGraphic* pSym = NULL;
	if (!strcmpi(strShapeName, "Line"))
	{
	...
	if (!strcmpi(strShapeName, "Square"))
	{
	...
	if (!strcmpi(strShapeName, "Circle"))
	{
	...
	// Initialize the symbol
	pSym->SetLineColor(pToolsPanel->GetCurrentLineColor());
	pSym->SetFillColor(pToolsPanel->GetCurrentFillColor());
	pSym->SetFillColor2(pToolsPanel->GetCurrentFillColor2());

	if (bFilled)
	{
		pSym->SetLineWidth(pToolsPanel->GetCurrentLineWidth());
		pSym->SetFillType(FT_Solid);
	}
	else
	{
		int LineWidth = pToolsPanel->GetCurrentLineWidth();
		pSym->SetLineWidth(LineWidth == LT_None ? LT_Hairline : LineWidth);
	...
	pToolsPanel->UpdateControls();

	return S_OK;
}
/////////////////////////////////////////////////////////////////////////////
LRESULT CDocWindow::OnDocFreeRotate(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
    CWaitCursor Wait;
...
LRESULT CDocWindow::UndoCropRect(CAGSym* pSym, RECT& CropRect)
{
	...
	pImageSym->SetCropRect(CropRect);
...
LRESULT CDocWindow::PasteText(HANDLE hMemory)
{
	...
	CToolsCtlPanelText* pToolsPanel = (CToolsCtlPanelText*)m_pCtp->GetCtlPanel(TEXT_PANEL);
	if (!pToolsPanel)
		return E_FAIL;
	...
	// Get the font
	int nItem = pToolsPanel->SendDlgItemMessage(IDC_FONT, CB_GETCURSEL, 0, 0);
	if (nItem >= 0)
	{
		int nFont = pToolsPanel->SendDlgItemMessage(IDC_FONT, CB_GETITEMDATA, nItem, 0);
		if (nFont >= 0)
		{
			LOGFONTEXLIST& FontList = m_pCtp->GetFontList().GetInstalledFonts();
			LOGFONT NewFont = FontList[nFont].lf;
	...
	// Get the fill and line attributes
	COLORREF FillColor = pToolsPanel->GetCurrentFillColor();
	COLORREF FillColor2 = pToolsPanel->GetCurrentFillColor2();
	COLORREF LineColor = pToolsPanel->GetCurrentLineColor();
	FillType Fill = pToolsPanel->GetCurrentFillType();
	int LineWidth = pToolsPanel->GetCurrentLineWidth();

	// Get the size
	int TextSize = pToolsPanel->GetCurrentTextSize();
	if (TextSize < POINTUNITS(6))
		TextSize = POINTUNITS(6);
	...
	// Get the emphasis
	if (pToolsPanel->IsDlgButtonChecked(IDC_BOLD))
		Font.lfWeight = FW_BOLD;
	if (pToolsPanel->IsDlgButtonChecked(IDC_ITALIC))
		Font.lfItalic = true;
	if (pToolsPanel->IsDlgButtonChecked(IDC_UNDERLINE))
		Font.lfUnderline = true;

	// Get the vertical justification
	TSVertJustifications eVertJust = eVertCentered;
	if (pToolsPanel->IsDlgButtonChecked(IDC_TOP))
		eVertJust = eVertTop;
	else
	if (pToolsPanel->IsDlgButtonChecked(IDC_MIDDLE))
		eVertJust = eVertCentered;
	else
	if (pToolsPanel->IsDlgButtonChecked(IDC_BOTTOM))
		eVertJust = eVertBottom;

	// Get the horizontal justification
	eTSJust eHorzJust = eRagCentered;
	if (pToolsPanel->IsDlgButtonChecked(IDC_LEFT))
		eHorzJust = eFlushLeft;
	else
	if (pToolsPanel->IsDlgButtonChecked(IDC_CENTER))
		eHorzJust = eRagCentered;
	else
	if (pToolsPanel->IsDlgButtonChecked(IDC_RIGHT))
		eHorzJust = eFlushRight;
...
LRESULT CDocWindow::DeleteSymbol(bool bCut, UINT nCommand)
{
	...
	if (m_pEnvelopeWizard)
	{
		...
		if (SymID == IDR_ENVELOPE_SEND || SymID == IDR_ENVELOPE_RETURN)
		{
			CMessageBox::Message(String("Sorry.  You cannot delete this object while using the envelope wizard."));
			return E_FAIL;
		}

		//if (m_pSym->GetType() == ST_IMAGE)
		//	m_pEnvelopeWizard->DeleteGraphicSymbol((CAGSymImage*)m_pSym);
	}
...
LRESULT CDocWindow::MirrorFlip(CAGSym* pSym, UINT nCommand, bool bFlipX, bool bFlipY)
{
	...
	if (pSym->IsImage())
	{
		pDIB = ((CAGSymImage*)pSym)->GetDib();
		if (!pDIB)
			return E_FAIL;

		pDIBNew = CDib::Orient(pDIB, 0/*iRotateDirection*/, bFlipX, bFlipY);
		if (!pDIBNew)
			return E_FAIL;
		if (!((CAGSymImage*)pSym)->CanChangeOrientation())
			((CAGSymImage*)pSym)->OrientChangeState(0/*iRotateDirection*/, bFlipX, bFlipY);
	}
	...
	if (pDIBNew)
	{
		((CAGSymImage*)pSym)->SetDib(pDIBNew);
		free(pDIB);
	}
	else
	{
...
LRESULT CDocWindow::Rotate(CAGSym* pSym, UINT nCommand, bool bCW)
{
	...
	if (pSym->IsImage())
	{
		...
		pDIBNew = CDib::Orient(pDIB, iRotateDirection, false/*bFlipX*/, false/*bFlipY*/);
		if (!pDIBNew)
			return E_FAIL;
		if (!((CAGSymImage*)pSym)->CanChangeOrientation())
			((CAGSymImage*)pSym)->OrientChangeState(iRotateDirection, false/*bFlipX*/, false/*bFlipY*/);
	}
...
{
	CMenu Menu;
	bool bOK = !!Menu.LoadMenu(IDR_POPUPMENU_FILE);
	if (!bOK)
		return E_FAIL;
...
/////////////////////////////////////////////////////////////////////////////
LRESULT CDocWindow::MaximizeSymbolSize(bool bWidth, bool bHeight)
{
	CWaitCursor Wait;
	...
	if (pSym->IsImage() && bWidth && bHeight)
	{
		if (fScaleX > fScaleY)
			fScaleX = fScaleY;
		else
...
LRESULT CDocWindow::OnDocChangeFillType(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	//Get the fill type
	FillType Fill = ((CToolsCtlPanelCommon*)pCtlPanel)->GetCurrentFillType();
...
LRESULT CDocWindow::OnDocChangeCalendarMonth(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	CToolsCtlPanelCal* pCtlPanel = (CToolsCtlPanelCal*)m_pCtp->GetCtlPanel(SELECTED_PANEL);
	if (!pCtlPanel)
		return E_FAIL;
	...
	if (m_Select.SymIsCalendar())
	{
		// Change the symbol
		CAGSymCalendar* pCalendarSym = m_Select.GetCalendarSym();
		int iCalendarMonth = pCtlPanel->GetCurrentCalendarMonth();
...
LRESULT CDocWindow::OnDocChangeCalendarStyle(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	if (m_Select.SymIsCalendar())
	{
		...
		CToolsCtlPanelCal* pCtlPanelCal = (CToolsCtlPanelCal*)pCtlPanel;

		if (pCtlPanelCal)
		{
			UINT iYrStyle = pCtlPanelCal->GetYearStyle();
			UINT iWkStyle = pCtlPanelCal->GetWeekStyle();
			UINT iTtlSize = pCtlPanelCal->GetTitleSize();
			UINT iWkSize = pCtlPanelCal->GetDayOfWkSize();
			bool iHideBoxes = pCtlPanelCal->GetHideBoxes();
			bool iHideAllBoxes = pCtlPanelCal->GetHideAllBoxes();
			bool iWkStart = pCtlPanelCal->GetWeekStart();
...
LRESULT CDocWindow::OnDocChangeCalendarYear(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	if (m_Select.SymIsCalendar())
	{
		// Change the symbol
		CAGSymCalendar* pCalendarSym = m_Select.GetCalendarSym();
		int iCalendarYear = pCtlPanel->GetCurrentCalendarYear();
...
LRESULT CDocWindow::OnDocChangeTextSize(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	if (m_Select.SymIsText() || (m_Select.SymIsAddAPhoto() && SpecialsEnabled()))
	{
		int TextSize = ((CToolsCtlPanelText*)pCtlPanel)->GetCurrentTextSize();
	...
	if (m_Select.SymIsCalendar())
	{
		CToolsCtlPanelText* pTxtPanel = (CToolsCtlPanelText*)m_pCtp->GetCtlPanel(TEXT_PANEL);
		int TextSize = pTxtPanel->GetCurrentTextSize();
		if (TextSize < POINTUNITS(1))
			TextSize = POINTUNITS(1);
...
LRESULT CDocWindow::OnDocChangeLineWidth(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	CToolsCtlPanelCommon* pCtlPanel = (CToolsCtlPanelCommon*)m_pCtp->GetCtlPanel(SELECTED_PANEL);
	if (!pCtlPanel)
		return E_FAIL;
	...
	if (m_Select.SymIsCalendar())
	{
		...
		else
		{
			...
			COLORREF LineColor = ((LineWidth == LT_None)?CLR_NONE:pCtlPanel->GetCurrentLineColor());
...
LRESULT CDocWindow::OnDocChangeShapeName(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	if (m_Select.SymIsShape())
	{
		...
		iShapeIndex = ((CToolsCtlPanelGraphics*)pCtlPanel)->GetCurrentShape();
...
LRESULT CDocWindow::OnSelectAll(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	CToolsCtlPanel* pCtlPanel = m_pCtp->GetCtlPanel(TEXT_PANEL);
	if (pCtlPanel)
		pCtlPanel->UpdateControls();
...
LRESULT CDocWindow::OnConvertToJPG(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	if (m_Select.GetImageSym()->GetNativeType() == "JPG")
		return E_FAIL;
...
LRESULT CDocWindow::OnArtGallery(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	CMonitorDirectory monitor(m_hWnd, IDC_FILEDOWNLOAD);

	CHtmlDialog dlg("http://www.facetofacesoftware.com/ArtGallery/index.html", false/*bRes*/, m_hWnd);
	dlg.SetWindowSize(640, 480);
	dlg.SetResizable(true);
	dlg.DoModal();
...
/////////////////////////////////////////////////////////////////////////////
LRESULT CDocWindow::OnFileDownload(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	CWaitCursor Wait;
	...
	static bool fDownloadInProgress;

	if (fDownloadInProgress)
		return E_FAIL;
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		CString szNewFileName = szPath + szFile.Left(szFile.GetLength() - 4);
		if (!::MoveFile(szFileName, szNewFileName)) // if the rename doesn't work...
			::DeleteFile(szFileName); // get rid of the file
		else
		{
			AddImage((LPCSTR)szNewFileName, 0);
