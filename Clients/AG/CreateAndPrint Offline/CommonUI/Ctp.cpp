...
void CCtp::CreateBackPage()
{
	...
	int idSiteLogo = (bHpUI ? IDR_HPAGLOGO : (bBlueMountain ? IDR_BMLOGO : IDR_AGLOGO));
	...
	if (bHpUI)
	{
		...
		pSymSponsorLogo = GetImageFromResource(IDR_HPLOGO, &ImageRect);
		if (pSymSponsorLogo)
		{
...
void CCtp::ConfigureWindows(int iWidth, int iHeight)
{
	...
	for (int nIndex = 0; nIndex < NUM_PANELS; nIndex++)
	{
		if (m_pToolsPanel[nIndex])
		{
			...
			::GetWindowRect(m_pToolsPanel[nIndex]->m_hWnd, &rToolsPanel);
	...
	{ // Maintained to sit to the left of the control panel (when shown)
		...
		RECT rect = {0, rToolsPanel.bottom, WIDTH(rPagePanel), rToolsPanel.bottom + HEIGHT(rPagePanel)};
...
bool CALLBACK CCtp::MyFontDownloadCallback(LPARAM lParam, void* pDownloadVoid)
{
	...
	if (!pDownload) // The final callback when there are no more files to download
	{
		...
		if (pCtp->GetCtlPanel(TEXT_PANEL))
			((CToolsCtlPanelText*)pCtp->GetCtlPanel(TEXT_PANEL))->SetFonts();
...
{
	// The license option can either be None (0) or Disney (283)
	return atoi(GetOptionStringValue(m_strContext, CString(_T("license"))));
}
