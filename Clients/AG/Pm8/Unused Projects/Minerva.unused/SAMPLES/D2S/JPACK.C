/*.T JPTEST.C
 * $Header: /PEGASUS/D2J/D2SEWWW/jpack.c 11    11/11/98 10:23a Jimbean $Nokeywords: $
 */
...
PRIVATE long  pack (void)
{
  ...
  BITMAPINFOHEADER    BMI;
	...
	if(!test_strips)
		...
		get_buff_size = 10000;
	get_buff_temp = malloc(get_buff_size + 16); /* big enough for query */
	check(get_buff_temp != NULL, "Out of memory for query buffer");
	get_buff = ((DWORD)(get_buff_temp + 16)) & (-16);  /* align 16 */
...
/*		case BI_YVYU : */
	...
	if(test_strips)
  {
    ...
    STRIP_SIZE, AND THE DATA MUST BE PUT IN IN MULTIPLES OF STRIP_SIZE,
	  ...
	  free(get_buff_temp); /* was used for just xquery */
  ...
  /* write out put buffer, if anything left there */
	...
	free(get_buff_temp);   p.Get.Start = NULL;
...
PRIVATE void    print_help (void)
{
...
#endif
    "     Makes either PIC2 or JPEG images (default is JPEG).\n"
    "         For PIC2 images, the default color palette behavior is to put a\n"
    "     palette into the header of the PIC2 file if the input is a\n"
    ...
    "     (These defaults can be modified with some of options below).\n"
		...
		getchar();
  printf(
		"    -J     Make JPEG rather than PIC2 file"
#if PC_WATCOM
		...
		"           a PIC2 file.\n"
    "               For a PIC2 output file, this flag means to put any palette\n"
    "           into a APP1 JPEG marker INSTEAD of putting it into the PIC2\n"
    "           header.\n"
		"    -X     Don't make any NEW color palettes for PIC2 or JPEG files.\n"
    "           Thus for a PIC2 or JPEG file, if the input has no palette,\n"
    "           neither will the output(even if -M is set for a JPEG file).\n"
    "           Note that if -X is set and the output is a PIC2 file, the\n"
    ...
    "           makes it possible for PIC2 output files to have no palette.\n"
