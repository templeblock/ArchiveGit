/******************************************************************************
 *
 * MOPRGJPG.C - Minerva MDI progressive JPEG expand module
 *
 ...
 *
 * Global data:
 *     OpenDefProgressiveJpeg - opcode definition data to open progressive jpeg image
 *
 * Local functions:
 *
 *   ** call Pegasus to open image **
 *     ExpandImageInWindow       - all the open progressive jpeg interesting stuff
 ...
 *     OpenProgressiveJpeg        - expand image to dib
 *     ExtractDibOpenProgressiveJpeg - extract full-size, full-detail dib with no interaction
 *     OptionsDialogOpenProgressiveJpeg - image options dialog 
 *     DefaultsDialogOpenProgressiveJpeg - default image options dialog
 *     PropertiesDialogOpenProgressiveJpeg - image properties dialog
 *     CleanupOpenProgressiveJpeg - image cleanup (dib, settings, picparm)
 *     DupPicParmOpenProgressiveJpeg - duplicate picparm for new window
 *
 *   ** misc **
...
extern SAVEOP SaveDefProgressiveJpeg;
...
static BOOL OpenProgressiveJpeg(
    HWND                       hWnd,
    const BYTE PICHUGE*        pSrc,
    DWORD                      dwSrcLen,
    const PIC_PARM PICFAR*     pSrcPicParm,
...
static BOOL ExtractDibOpenProgressiveJpeg(
    const BYTE PICHUGE*        pSrc,
    DWORD                      dwSrcLen,
    const PIC_PARM PICFAR*     pSrcPicParm,
    BYTE PICHUGE* PICFAR*      ppDib,
    DWORD PICFAR*              pdwDibLen,
    PIC_PARM PICFAR*           pDibPicParm);
static BOOL OptionsDialogOpenProgressiveJpeg(
    HWND                       hwnd,
    const BYTE PICHUGE*        pSrc,
    DWORD                      dwSrcLen,
    const PIC_PARM PICFAR*     pSrcPicParm,
...
static void PropertiesDialogOpenProgressiveJpeg(
    HWND                   hwnd,
    LPCSTR                 pszFilename,
    const BYTE PICHUGE*    pSrc,
    DWORD                  dwSrcLen,
...
static void DefaultsDialogOpenProgressiveJpeg(void);
static BOOL DupPicParmOpenProgressiveJpeg(const PIC_PARM PICFAR* pSrc, PIC_PARM PICFAR* pDup);
static void CleanupOpenProgressiveJpeg(
    BYTE PICHUGE *pDib,
    PIC_PARM PICFAR *pDibPicParm,
    VOID PICFAR* pOpenData);
...
static BOOL ReqExecYield(PIC_PARM PICFAR* p);
...
static CTRLINITDATA InitDisplayTiming[] =
    ...
    */
    ...
    { 0, eText, IDC_OPRGJPGTIME_TIMERRES, 0, sizeof("999.999") },
    { 0, eText, IDC_OPRGJPGTIME_INIT,     0, sizeof("999.999") },
    { 0, eText, IDC_OPRGJPGTIME_EXEC,     0, sizeof("999.999") },
    { 0, eText, IDC_OPRGJPGTIME_TERM,     0, sizeof("999.999") },
    { 0, eText, IDC_OPRGJPGTIME_ALLOC,    0, sizeof("999.999") },
    { 0, eText, IDC_OPRGJPGTIME_MEMCPY,   0, sizeof("999.999") },
    { 0, eText, IDC_OPRGJPGTIME_YIELD,    0, sizeof("999,999") },
    { 0, eText, IDC_OPRGJPGTIME_OPTOTAL,  0, sizeof("999.999") },
    { 0, eText, IDC_OPRGJPGTIME_DISPLAYPALETTE, 0, sizeof("999.999") },
    { 0, eText, IDC_OPRGJPGTIME_PAINT,    0, sizeof("999.999") },
    { 0, eText, IDC_OPRGJPGTIME_TOTAL,    0, sizeof("999.999") },
...
static char szOptionsSection[] = "Progressive JPEG Expand Options";
...
static char szDisplay[] = "Progressive Display";
...
static CTRLINITDATA InitOptionsData[] =
    ...
    */
    ...
    { "Make Gray",             eCheckBox, IDC_OPRGJPGOPT_CHECK_MAKEGRAY,            FALSE },
    { "Dither",                eCheckBox, IDC_OPRGJPGOPT_CHECK_DITHER,              TRUE  },
    { "Cross-Block Smoothing", eCheckBox, IDC_OPRGJPGOPT_CHECK_ENHANCEDDECOMPRESS,  TRUE  },
    { "Prompt After Scans",    eCheckBox, IDC_OPRGJPGOPT_CHECK_PROMPTSCAN,          TRUE  },

    { szImageSize,    eRadioButton, IDC_OPRGJPGOPT_RADIO_FULLSIZE,        TRUE,  0, "100%" },
    { szImageSize,    eRadioButton, IDC_OPRGJPGOPT_RADIO_FOURTH,          FALSE, 0, "25%"  },
    { szImageSize,    eRadioButton, IDC_OPRGJPGOPT_RADIO_SIXTEENTH,       FALSE, 0, "1/16" },
    { szImageSize,    eRadioButton, IDC_OPRGJPGOPT_RADIO_SIXTYFOURTH,     FALSE, 0, "1/64" },
    { szBitsPerPixel, eRadioButton, IDC_OPRGJPGOPT_RADIO_32BITS,          FALSE, 0, "32"   },
    { szBitsPerPixel, eRadioButton, IDC_OPRGJPGOPT_RADIO_24BITS,          TRUE,  0, "24"   },
    { szBitsPerPixel, eRadioButton, IDC_OPRGJPGOPT_RADIO_16BITS,          FALSE, 0, "16"   },
    { szBitsPerPixel, eRadioButton, IDC_OPRGJPGOPT_RADIO_8BITS,           FALSE, 0, "8"    },
    { szBitsPerPixel, eRadioButton, IDC_OPRGJPGOPT_RADIO_4BITS,           FALSE, 0, "4"    },
    { szColorPalette, eRadioButton, IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE, TRUE,  0, "Embedded" },
    { szColorPalette, eRadioButton, IDC_OPRGJPGOPT_RADIO_MAKEPALETTE,     FALSE, 0, "Make",    },
    { szColorPalette, eRadioButton, IDC_OPRGJPGOPT_RADIO_EXTERNALPALETTE, FALSE, 0, "External" },
    { szDisplay,      eRadioButton, IDC_OPRGJPGOPT_RADIO_DISPLAYFINALONLY,     TRUE,  0, "Final Only"  },
    { szDisplay,      eRadioButton, IDC_OPRGJPGOPT_RADIO_PROGRESSIVEDISPLAY,   FALSE, 0, "Progressive" },

    { "External Palette Filename",   eEdit, IDC_OPRGJPGOPT_EDIT_EXTERNALPALETTE, FALSE, _MAX_PATH },
    { "Make Gray Grays To Make",     eEdit, IDC_OPRGJPGOPT_EDIT_GRAYSTOMAKE,     FALSE, 3, "256" },
    { "Make Palette Colors To Make", eEdit, IDC_OPRGJPGOPT_EDIT_COLORSTOMAKE,    FALSE, 3, "256" },
    { "Password",                    eEdit, IDC_OPRGJPGOPT_EDIT_PASSWORD,        TRUE,  8 },
    { "Image Left",                  eEdit, IDC_OPRGJPGOPT_EDIT_IMAGELEFT,       FALSE, 4 },
    { "Image Top",                   eEdit, IDC_OPRGJPGOPT_EDIT_IMAGETOP,        FALSE, 4 },
    { "Image Width",                 eEdit, IDC_OPRGJPGOPT_EDIT_IMAGEWIDTH,      FALSE, 4 },
    { "Image Height",                eEdit, IDC_OPRGJPGOPT_EDIT_IMAGEHEIGHT,     FALSE, 4 },
    { "Window Left",                 eEdit, IDC_OPRGJPGOPT_EDIT_WINDOWLEFT,      FALSE, 4 },
    { "Window Top",                  eEdit, IDC_OPRGJPGOPT_EDIT_WINDOWTOP,       FALSE, 4 },
    { "Scans To Decode",             eEdit, IDC_OPRGJPGOPT_EDIT_SCANSTODECODE,   FALSE, 4 },
    
    { 0, ePushButton, IDC_OPRGJPGOPT_BUTTON_BROWSE          },
    { 0, ePushButton, IDTEST                                },
    { 0, eGroupBox,   IDC_OPRGJPGOPT_STATIC_COLORPALETTE    },
    { 0, eText,       IDC_OPRGJPGOPT_STATIC_COLORS          },
    { 0, eText,       IDC_OPRGJPGOPT_STATIC_EXTERNALPALETTE },
    { 0, eText,       IDC_OPRGJPGOPT_STATIC_GRAYS           },
    { 0, eCheckBox,   IDC_OPRGJPGOPT_CHECK_SETDEFAULT       },
...
static CTRLINITDATA InitPropertiesData[] =
    {
    { 0, eText, IDC_PRGJPGPROP_FILENAME,        0, _MAX_PATH },
    { 0, eText, IDC_PRGJPGPROP_FILESIZE,        0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_IMAGEWIDTH_SOURCE,         0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_IMAGEHEIGHT_SOURCE,        0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_BITSPERPIXEL_SOURCE,       0, 2 },
    { 0, eText, IDC_PRGJPGPROP_IMAGESIZE_SOURCE,          0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_COLORTABLESIZE_SOURCE,     0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_NUMBERIMPORTANT_SOURCE,    0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_IMAGEWIDTH_DISPLAYED,      0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_IMAGEHEIGHT_DISPLAYED,     0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_BITSPERPIXEL_DISPLAYED,    0, 2 },
    { 0, eText, IDC_PRGJPGPROP_IMAGESIZE_DISPLAYED,       0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_COLORTABLESIZE_DISPLAYED,  0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_NUMBERIMPORTANT_DISPLAYED, 0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_PICVERSION,                0, sizeof("4,000,000,000") },
    { 0, eText, IDC_PRGJPGPROP_VISUALORIENTATION,         0, sizeof("270* clockwise, then inverted") },
    { 0, eText, IDC_PRGJPGPROP_COMPRESSIONRATIO, 0, sizeof("4,000,000,000:1") },
    { 0, eText, IDC_PRGJPGPROP_SCANSDISPLAYED,  0, 3 },
    { 0, eText, IDC_PRGJPGPROP_LUMINANCE,       0, 3 },
    { 0, eText, IDC_PRGJPGPROP_CHROMINANCE,     0, 3 },
    { 0, eText, IDC_PRGJPGPROP_SUBSAMPLING,     0, sizeof("unknown") },
    { 0, eEdit, IDC_PRGJPGPROP_COMMENT,         FALSE, MAX_COMMENT },
    { 0, eEdit, IDC_PRGJPGPROP_APP2,            FALSE, MAX_COMMENT },
    { 0, eText, IDC_PRGJPGPROP_ENTROPYCODING,   0, sizeof("Els Arithmetic Entropy Coding") },
...
static DWORD arTypes[] = { BI_PJPG, BI_PICP, (DWORD)-1 };
...
OPENOP OpenDefProgressiveJpeg =
    {
    OP_P2D,
    "Progressive JPEG Expand",
    "Progressive JPEG Defaults",
    "Progressive JPEG (*.pic,*.jpg)|*.PIC;*.JPG|",
    arTypes,
    0,
    OpenProgressiveJpeg,
    ExtractDibOpenProgressiveJpeg,
    OptionsDialogOpenProgressiveJpeg,
    PropertiesDialogOpenProgressiveJpeg,
    DefaultsDialogOpenProgressiveJpeg,
    CleanupOpenProgressiveJpeg,
    DupPicParmOpenProgressiveJpeg,
 ...
 *  OnCommand()
 *
 *  WM_COMMAND event processing for Next Scan Dialog while progressively
 *  displaying a progressive jpeg image in ExpandImageInWindow
 ******************************************************************************/
static BOOL OnCommand(HWND hwndDlg, int nCtrlID, HWND hwndCtrl, UINT wNotify)
{
    NOREFERENCE(hwndCtrl);
    NOREFERENCE(wNotify);
    if ( nCtrlID == IDC_PRGJPG_BUTTON_NEXTSCAN || nCtrlID == IDC_PRGJPG_BUTTON_FINALSCAN )
    {
 ...
 *  DlgProc()
 *
 *  dialog procedure for Next Scan Dialog while progressively displaying a
 *  progressive jpeg image in ExpandImageInWindow
 ******************************************************************************/
static BOOL CALLBACK DlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    LONG nScans;
    char sz[sizeof("999 scans displayed.") + 30]; /* current string + padding for changes */
    char szx[sizeof(sz)];
    int n;
    ...
    {
    case WM_INITDIALOG:
        nScans = *(LONG PICFAR*)lParam;
        n = GetDlgItemText(hwndDlg, IDC_PRGJPG_STATIC_NUMSCANS, sz, sizeof(sz));
        assert(n != 0 && n < sizeof(sz) - 1);
        wsprintf(szx, sz, (int)nScans, (LPCSTR)( nScans == 1 ? "" : "s" ));
        SetDlgItemText(hwndDlg, IDC_PRGJPG_STATIC_NUMSCANS, szx);
 ...
 *      VOID PICFAR*           pOpenData)
 * 
 *  all the open progressive jpeg interesting stuff
 *
 *  parameters:
    ...
    VOID PICFAR*           pOpenData)
{
    ...
    */
    ...
    pp->ParmVerMinor = 1;
    pp->Op = OP_P2D;        /* Progressive JPEG to DIB */
    ...
    bFinalDibOnly = CtrlDataIsPushed(ar, IDC_OPRGJPGOPT_RADIO_DISPLAYFINALONLY);
    nScans = (int)CtrlDataGetLong(ar, IDC_OPRGJPGOPT_EDIT_SCANSTODECODE, FALSE);
    bPromptScan = CtrlDataIsPushed(ar, IDC_OPRGJPGOPT_CHECK_PROMPTSCAN);
    ...
    if ( dwGetQSize == 0 || dwGetQSize >= dwSrcLen )
        ...
        pp->Get.Start  = (BYTE PICHUGE*)pSrc;
            /* cast away constness to eliminate the warning --
                ...
                practice, (although it is currently safe with Progressive JPEG expand). */
    ...
    */
    ...
    while ( ( response = PegasusReqInit(pp) ) == RES_ERR )
        ...
        CleanupOpenProgressiveJpeg(0, pp, 0);
        if ( pp->Status == ERR_MISSING_KEY || pp->Status == ERR_INVALID_KEY )
            ...
            CtrlDataSetText(ar, IDC_OPRGJPGOPT_EDIT_PASSWORD, sz);
            }
        else
            {
            MiscGlobalFreePtr(&pGetQ);
    ...
    *ppDib = (BYTE PICHUGE*)MiscGlobalAllocPtr(*pdwDibLen, IDS_IMAGEOUTOFMEMORY);
    ...
    /**************************************************************************
        Pegasus(REQ_EXEC)
    */
    pp->u.P2D.NumScansReq = 1;
    nLastScanDone = 0;
    do
        {
        if ( bFinalDibOnly )
            pp->u.P2D.NumScansReq = nScans;
        if ( App.pPut0 != 0 )
        {
        ...
        if ( response == RES_ERR )
            ...
            if ( pp->Status != ERR_BAD_DATA ||
                ...
                {
                /* either error is not BAD_DATA or nothing was output */
                MiscGlobalFreePtr(&pGetQ);
        ...
        if ( pp->u.P2D.NumScansDone != 0 )
            {
            nLastScanDone = (int)pp->u.P2D.NumScansDone;
            if ( pp->u.P2D.NumScansDone != nScans && pp->u.P2D.NumScansReq != 0 )
                {
                if ( !bYield )
                {
                    App.dwTime[PAINT] -= MiscTickCount();    
                    InvalidateRect(App.hWnd, 0, FALSE);
                    ChildSetDibHeight(App.hWnd, pp->Head.biHeight);
                    UpdateWindow(App.hWnd);
                    App.dwTime[PAINT] += MiscTickCount();    
                }
                pp->u.P2D.NumScansReq++;
                App.dwTime[TOTAL] += MiscTickCount();
                if ( bPromptScan && DialogBoxParam(
                        hinstThis,
                        MAKEINTRESOURCE(IDD_PRGJPGNEXTSCAN),
                        hwndFrame,
                        (DLGPROC)DlgProc,
                        (LPARAM)&pp->u.P2D.NumScansDone) != IDC_PRGJPG_BUTTON_NEXTSCAN )
                    bFinalDibOnly = TRUE;
                App.dwTime[TOTAL] -= MiscTickCount();    
                }
            }
    ...
    while ( nLastScanDone == 0 ||
            ( pp->u.P2D.NumScansDone != 0 &&
              pp->u.P2D.NumScansDone != nScans &&
              pp->u.P2D.NumScansReq != 0 ) );
    if ( pp->u.P2D.NumScansDone == 0 )
        pp->u.P2D.NumScansDone = nLastScanDone;
    ...
    */
    ...
    else if ( bDisplayTiming )
        MiscDisplayTimingDialog(hWnd,
            MAKEINTRESOURCE(IDD_OPRGJPGTIME),
            App.dwTime,
            InitDisplayTiming);
...
static void InitPicParm(PIC_PARM PICFAR* p, const PIC_PARM PICFAR* pSrcPicParm, LPCTRLINITDATA ar)
{
    ...
    /* image password -- if one is required and this is null or wrong, we'll get an
        ...
        on our own */
    CtrlDataGetText(ar, IDC_OPRGJPGOPT_EDIT_PASSWORD, sz, sizeof(sz));
    ...
    switch ( CtrlDataWhichRadio(ar, IDC_OPRGJPGOPT_RADIO_FULLSIZE) )
        {
        case IDC_OPRGJPGOPT_RADIO_FULLSIZE:
            p->u.J2D.Thumbnail = THUMB_NONE;
            break;
        case IDC_OPRGJPGOPT_RADIO_FOURTH:
            p->u.J2D.Thumbnail = THUMB_4;
            break;
        case IDC_OPRGJPGOPT_RADIO_SIXTEENTH:
            p->u.J2D.Thumbnail = THUMB_16;
            break;
        case IDC_OPRGJPGOPT_RADIO_SIXTYFOURTH:
    ...
    if ( !CtrlDataIsPushed(ar, IDC_OPRGJPGOPT_CHECK_ENHANCEDDECOMPRESS) )
        p->u.J2D.PicFlags |= PF_NoCrossBlockSmoothing;

    /* the remaining settings depend on DIB size */
    switch ( CtrlDataWhichRadio(ar, IDC_OPRGJPGOPT_RADIO_32BITS) )
        {
        case IDC_OPRGJPGOPT_RADIO_32BITS:
            p->u.J2D.DibSize = 32;
            break;
        case IDC_OPRGJPGOPT_RADIO_24BITS:
            p->u.J2D.DibSize = 24;
            break;
        case IDC_OPRGJPGOPT_RADIO_16BITS:
            p->u.J2D.DibSize = 16;
            break;
        case IDC_OPRGJPGOPT_RADIO_8BITS:
            p->u.J2D.DibSize = 8;
            break;
        case IDC_OPRGJPGOPT_RADIO_4BITS:
            p->u.J2D.DibSize = 4;
            break;
        default:
            assert(FALSE);
    ...
    wImageLeft   = (WORD)CtrlDataGetLong(ar, IDC_OPRGJPGOPT_EDIT_IMAGELEFT, FALSE);
    wImageTop    = (WORD)CtrlDataGetLong(ar, IDC_OPRGJPGOPT_EDIT_IMAGETOP, FALSE);
    wImageWidth  = (WORD)CtrlDataGetLong(ar, IDC_OPRGJPGOPT_EDIT_IMAGEWIDTH, FALSE);
    wImageHeight = (WORD)CtrlDataGetLong(ar, IDC_OPRGJPGOPT_EDIT_IMAGEHEIGHT, FALSE);
    ...
    if ( wImageLeft != 0 || wImageTop != 0 || wImageWidth != 0 || wImageHeight != 0 )
        ...
        if ( wImageWidth == 0 )
            wImageWidth = (WORD)( pSrcPicParm->Head.biWidth - wImageLeft );
        if ( wImageHeight == 0 )
            wImageHeight = (WORD)( labs(pSrcPicParm->Head.biHeight) - wImageTop );
    ...
    if ( p->u.J2D.DibSize > 8 )
        {
        if ( p->u.J2D.DibSize == 16 && CtrlDataIsPushed(ar, IDC_OPRGJPGOPT_CHECK_DITHER) )
        ...
        assert(!CtrlDataIsPushed(ar, IDC_OPRGJPGOPT_CHECK_MAKEGRAY));
        }
    else /* p->u.J2D.DibSize <= 8 */
        {
        /* dither is independent of gray or color choice */
        if ( CtrlDataIsPushed(ar, IDC_OPRGJPGOPT_CHECK_DITHER) )
            p->u.J2D.PicFlags |= PF_Dither;

        if ( CtrlDataIsPushed(ar, IDC_OPRGJPGOPT_CHECK_MAKEGRAY) )
            {
            ...
            p->u.J2D.GraysToMake = CtrlDataGetLong(ar, IDC_OPRGJPGOPT_EDIT_GRAYSTOMAKE, FALSE);
            }
            ...
            switch ( CtrlDataWhichRadio(ar, IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE) )
                {
                case IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE:
                ...
                case IDC_OPRGJPGOPT_RADIO_MAKEPALETTE:
                    p->u.J2D.PicFlags |=
                    ...
                    p->u.J2D.PrimClrToMake =
                        CtrlDataGetLong(ar, IDC_OPRGJPGOPT_EDIT_COLORSTOMAKE, FALSE);
                    break;

                case IDC_OPRGJPGOPT_RADIO_EXTERNALPALETTE:
                    CtrlDataGetText(ar, IDC_OPRGJPGOPT_EDIT_EXTERNALPALETTE, sz, sizeof(sz));
                    p->Head.biClrUsed =
...
/******************************************************************************
 *  BOOL ReqExecYield(PIC_PARM PICFAR *p)
 * 
 *  handle all of REQ_EXEC's RES_*_DATA_YIELD events, also called for each
 *  RES_PUT_NEED_SPACE and at RES_DONE if there are any data left in the put queue
 *
 *  updates the display according to what's available in the put queue
 *  yields to windows
 *
 *  parameters:
 *      p  - pointer to initialized PIC_PARM
 *
 *  returns:
 *      TRUE if the application is closed while yielding to windows, else FALSE
 ******************************************************************************/
BOOL ReqExecYield(PIC_PARM PICFAR *p)
{
    APPDATA PICFAR* pApp = (APPDATA PICFAR*)p->App;
    ...
    lHeight = p->Head.biHeight * p->u.J2D.WidthPad;
    if ( pApp->pPut0 == 0 )
        {
        if ( p->Put.QFlags & Q_REVERSE )
            lHeight -= p->Put.Rear - p->Put.Start;
        else
            lHeight -= p->Put.End - p->Put.Rear;
        }
    else
        lHeight -= pApp->dwPutSize;
...
{    
            ...
            case RES_GET_NEED_DATA:
                if ( ( p->Get.QFlags & Q_EOF ) != 0 )
                    ...
                    if ( pApp->dwGetLen == 0 )
                        ...
                        {
                        pApp->dwTime[MEMCPY] -= MiscTickCount();
                        if ( p->Get.Front <= p->Get.Rear )
                        {
                            if ( p->Get.End - p->Get.Start - ( p->Get.Rear - p->Get.Front ) != 0 )
                                MiscCopyToQueue(&pApp->pGet, &pApp->dwGetLen, &p->Get, 0);
                        }
                        else
                        {
                            if ( p->Get.Front - p->Get.Rear - 1 != 0 )
                                MiscCopyToQueue(&pApp->pGet, &pApp->dwGetLen, &p->Get, 0);
                        }
                        pApp->dwTime[MEMCPY] += MiscTickCount();
                        if ( ( p->u.J2D.PicFlags & PF_YieldGet ) != 0 )
    ...
    if ( response == RES_DONE )
        {
        if ( pApp->pPut != 0 && p->Put.Front != p->Put.Rear )
            {
            pApp->dwTime[MEMCPY] -= MiscTickCount();
            MiscCopyFromQueue(&pApp->pPut, &pApp->dwPutSize, &p->Put, p->u.J2D.StripSize);
            pApp->dwTime[MEMCPY] += MiscTickCount();
...
/******************************************************************************
 *  BOOL ExtractDibOpenProgressiveJpeg(
 *      const BYTE PICHUGE*    pSrc,
 *      DWORD                  dwSrcLen,
 *      const PIC_PARM PICFAR* pSrcPicParm,
 *      BYTE PICHUGE* PICFAR*  ppDib,
    ...
    VOID PICFAR* PICFAR*       ppOpenData)
{
    ...
    if ( pSrcPicParm->Head.biBitCount == 8 )
        {
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_STATIC_COLORPALETTE);
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_RADIO_32BITS);
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_RADIO_24BITS);
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_RADIO_16BITS);

        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE);
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_RADIO_MAKEPALETTE);
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_RADIO_EXTERNALPALETTE);
        
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_EDIT_COLORSTOMAKE);
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_STATIC_COLORS);
        
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_STATIC_EXTERNALPALETTE);
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_EDIT_EXTERNALPALETTE);
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_BUTTON_BROWSE);

        /* and make sure either 4-bit or 8-bit bit depth is pushed and
            that make gray is pushed */
        if ( !CtrlDataIsPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_4BITS) )
            CtrlDataSetPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_8BITS, TRUE);
        CtrlDataSetPushed(*ppOpenData, IDC_OPRGJPGOPT_CHECK_MAKEGRAY, TRUE);
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_CHECK_MAKEGRAY);
        }
    else
        {
        ...
        if ( ( CtrlDataIsPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_8BITS) ||
               CtrlDataIsPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_4BITS) ) &&
             !CtrlDataIsPushed(*ppOpenData, IDC_OPRGJPGOPT_CHECK_MAKEGRAY) )
            {
            if ( CtrlDataIsPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_8BITS) )
                nColors = 256;
            else
            ...
            CtrlDataGetText(*ppOpenData, IDC_OPRGJPGOPT_EDIT_EXTERNALPALETTE, sz, sizeof(sz));
            ...
            if ( ( CtrlDataIsPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE) &&
                   ( lPrimaryColors < 2 || lPrimaryColors > nColors ) &&
                   ( lSecondaryColors < 2 || lSecondaryColors > nColors ) ) ||
                 ( CtrlDataIsPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_EXTERNALPALETTE) &&
                ...
                CtrlDataSetPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_MAKEPALETTE, TRUE);
                CtrlDataSetLong(*ppOpenData, IDC_OPRGJPGOPT_EDIT_COLORSTOMAKE, nColors, FALSE);
                }
            }
        if ( CtrlDataIsInactive(*ppOpenData, IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE) &&
             CtrlDataIsPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE) )
            CtrlDataSetPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_MAKEPALETTE, TRUE);
        }                            
...
/******************************************************************************
 *  BOOL OptionsDialogOpenProgressiveJpeg(
 *      HWND                   hWnd,
 *      const BYTE PICHUGE*    pSrc,
 *      DWORD                  dwSrcLen,
 *      const PIC_PARM PICFAR* pSrcPicParm,
 ...
 *
 *  notes:
 *      The processing specific to the Open Progressive Jpeg image options is contained
 *      in the WM_COMMAND handler OnCmdOPrgJpgOpt.  See also mdlg.c and mdlgctrl.c.
 *
    ...
    VOID PICFAR* PICFAR*       ppOpenData)
{
    ...
    result = DlgDoModal(hwndFrame, MAKEINTRESOURCE(IDD_OPRGJPGOPT), ImageOnCommand, *ppData, &oud);
        ...
        if ( oud.bTested )
            ...
            *ppData = pOldData;
            ...
            if ( ExpandImageInWindow(     
                    ...
                    pDibPicParm,
                    *ppData) )
                CleanupOpenProgressiveJpeg(pOldDib, &OldDibPicParm, 0);
            else                
                assert(FALSE);
    ...
    if ( CtrlDataIsPushed(*ppData, IDC_OPRGJPGOPT_CHECK_SETDEFAULT) )
...
/******************************************************************************
 *  void DefaultsDialogOpenProgressiveJpeg(void)
 *
...
static void DefaultsDialogOpenProgressiveJpeg(void)
{
    ...
    CtrlDataSetPushed(pOpenData, IDC_OPRGJPGOPT_CHECK_SETDEFAULT, TRUE);
    CtrlDataSetInactive(pOpenData, IDC_OPRGJPGOPT_CHECK_SETDEFAULT);
    CtrlDataSetInactive(pOpenData, IDTEST);
    result = DlgDoModal(
        hwndFrame,
        MAKEINTRESOURCE(IDD_OPRGJPGOPT),
        DefaultsOnCommand,
        pOpenData,
        NULL);
    if ( result != IDOK )
...
/******************************************************************************
 *  void PropertiesDialogOpenProgressiveJpeg
 *      LPCSTR                 pszFilename,
 *      const BYTE PICHUGE*    pSrc,
 *      DWORD                  dwSrcLen,
 *      const PIC_PARM PICFAR* pSrcPicParm,
...
static void PropertiesDialogOpenProgressiveJpeg(
    ...
    const VOID PICFAR*     pOpenData)
{
    char sz[_MAX_PATH + sizeof(" Progressive JPEG Properties")];
    int result;
    ...
    */
    ...
    CtrlDataSetText(pInitData, IDC_PRGJPGPROP_FILENAME, sz);
    CtrlDataSetLong(pInitData, IDC_PRGJPGPROP_FILESIZE, dwSrcLen, FALSE);
    CtrlDataSetLong(pInitData, IDC_PRGJPGPROP_IMAGEWIDTH_SOURCE, pSrcPicParm->Head.biWidth, FALSE);
    CtrlDataSetLong(pInitData, IDC_PRGJPGPROP_IMAGEHEIGHT_SOURCE, labs(pSrcPicParm->Head.biHeight), FALSE);
    CtrlDataSetLong(
        pInitData,
        IDC_PRGJPGPROP_BITSPERPIXEL_SOURCE,
    ...
    CtrlDataSetLong(
        pInitData,
        IDC_PRGJPGPROP_IMAGESIZE_SOURCE,
        dwSrcWidthPad * labs(pSrcPicParm->Head.biHeight),
        FALSE);
    CtrlDataSetLong(pInitData, IDC_PRGJPGPROP_COLORTABLESIZE_SOURCE, pSrcPicParm->Head.biClrUsed, FALSE);
    CtrlDataSetLong(
        pInitData,
        IDC_PRGJPGPROP_NUMBERIMPORTANT_SOURCE,
        pSrcPicParm->Head.biClrImportant,
        FALSE);

    CtrlDataSetLong(
        pInitData,
        IDC_PRGJPGPROP_IMAGEWIDTH_DISPLAYED,
        pDibPicParm->Head.biWidth,
        FALSE);
    CtrlDataSetLong(
        pInitData,
        IDC_PRGJPGPROP_IMAGEHEIGHT_DISPLAYED,
        pDibPicParm->Head.biHeight,
        FALSE);
    CtrlDataSetLong(
        pInitData,
        IDC_PRGJPGPROP_BITSPERPIXEL_DISPLAYED,
        pDibPicParm->Head.biBitCount,
        FALSE);
    CtrlDataSetLong(pInitData, IDC_PRGJPGPROP_IMAGESIZE_DISPLAYED, dwDibLen, FALSE);
    CtrlDataSetLong(
        pInitData,
        IDC_PRGJPGPROP_COLORTABLESIZE_DISPLAYED,
        pDibPicParm->Head.biClrUsed,
        FALSE);
    CtrlDataSetLong(
        pInitData,
        IDC_PRGJPGPROP_NUMBERIMPORTANT_DISPLAYED,
        pDibPicParm->Head.biClrImportant,
        FALSE);

    CtrlDataSetLong(pInitData, IDC_PRGJPGPROP_PICVERSION, pSrcPicParm->PicVer, FALSE);
    CtrlDataSetText(
        pInitData,
        IDC_PRGJPGPROP_VISUALORIENTATION,
    ...
    CtrlDataSetLong(
        pInitData,
        IDC_PRGJPGPROP_COMPRESSIONRATIO,
        ( dwSrcBmpSize + dwSrcLen / 2 ) / dwSrcLen,
        FALSE);
    CtrlDataGetText(pInitData, IDC_PRGJPGPROP_COMPRESSIONRATIO, sz, sizeof(sz));
    strcat(sz, ":1");
    CtrlDataSetText(pInitData, IDC_PRGJPGPROP_COMPRESSIONRATIO, sz);

    if ( pSrcPicParm->Head.biCompression == BI_JPGE ||
         pSrcPicParm->Head.biCompression == BI_PC2E )
        CtrlDataSetText(pInitData, IDC_PRGJPGPROP_ENTROPYCODING, "Els Arithmetic Entropy Coding");
    else
        CtrlDataSetText(pInitData, IDC_PRGJPGPROP_ENTROPYCODING, "Huffman");
        
    CtrlDataSetLong(pInitData, IDC_PRGJPGPROP_SCANSDISPLAYED, pDibPicParm->u.J2D.NumScansDone, FALSE);
    CtrlDataSetLong(pInitData, IDC_PRGJPGPROP_LUMINANCE, pDibPicParm->u.J2D.LumFactor, FALSE);
    CtrlDataSetLong(pInitData, IDC_PRGJPGPROP_CHROMINANCE, pDibPicParm->u.J2D.ChromFactor, FALSE);
    ...
    CtrlDataSetText(pInitData, IDC_PRGJPGPROP_SUBSAMPLING, sz);
    if ( pDibPicParm->CommentLen != 0 )
        ...
        CtrlDataSetText(pInitData, IDC_PRGJPGPROP_COMMENT, psz);
    ...
    if ( pDibPicParm->u.J2D.AppFieldLen != 0 )
        ...
        CtrlDataSetText(pInitData, IDC_PRGJPGPROP_APP2, psz);
        MiscGlobalFreePtr(&psz);
        }
    ...
    */
    GetWindowText(hWnd, sz, sizeof(sz) - ( sizeof(" Progressive JPEG Properties") - 1 ));
    strcat(sz, " Progressive JPEG Properties");
    result = DlgDoModal(
        hwndFrame,
        MAKEINTRESOURCE(IDD_PRGJPGPROP),
        PropertiesOnCommand,
        pInitData,
        sz);
    CtrlDataFree(&pInitData);
 ...
 *      pUserData   - as defined by OpenProgressiveJpeg
 *
 *  returns:
...
/******************************************************************************
 * void CleanupOpenProgressiveJpeg(
 *      BYTE PICHUGE *pDib,
 *      PIC_PARM PICFAR *pDibPicParm,
 *      VOID PICFAR* pOpenData)
 *
...
/******************************************************************************
 *  BOOL DupPicParmOpenProgressiveJpeg(const PIC_PARM PICFAR* pSrc, PIC_PARM PICFAR* pDup);
 *
...
static BOOL GetCtrlDefaults(LPCTRLINITDATA PICFAR* ppOpenData)
{
    ...
    if ( MiscIsPaletteDisplay(&num) )
        ...
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_RADIO_32BITS);
        CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_RADIO_16BITS);
        if ( CtrlDataIsPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_32BITS) ||
             CtrlDataIsPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_16BITS) )
            CtrlDataSetPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_24BITS, TRUE);
        if ( ToolDefUtility.nFoundParmVer == 0 )
            {
            CtrlDataSetInactive(*ppOpenData, IDC_OPRGJPGOPT_RADIO_24BITS);
            if ( CtrlDataIsPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_24BITS) )
                CtrlDataSetPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_4BITS, TRUE);
            if ( num < 8 )                
                CtrlDataSetPushed(*ppOpenData, IDC_OPRGJPGOPT_RADIO_4BITS, TRUE);
            }
        else if ( num < 8 )
...
static BOOL ImageOnCommand(
    ...
    void PICFAR*    pUserData)
{
    char sz[_MAX_PATH + sizeof(" Progressive JPEG Open Options")];
    ...
    switch ( nCtrlID )
        ...
        case -1:    /* for WM_INITDIALOG - the virtue is it saves us having to have a separarte
            ...
            GetWindowText(poud->hWnd, sz, sizeof(sz) - ( sizeof(" Progressive JPEG Open Options") - 1 ));
            strcat(sz, " Progressive JPEG Open Options");
            SetWindowText(hwndDlg, sz);
            ImageGrayedCtrls(hwndDlg, arOpenData, poud);
            break;
            
        case IDC_OPRGJPGOPT_CHECK_MAKEGRAY:
            if ( wNotifyCode == BN_CLICKED )
                ...
                if ( poud->pSrcPicParm->Head.biBitCount != 8 )
                    ...
                    CheckDlgButton(
                        hwndDlg,
                        IDC_OPRGJPGOPT_CHECK_MAKEGRAY,
                        !IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_CHECK_MAKEGRAY));
                    }                    

                /* make sure the bit depth is 8 bits or 4 bits */
                if ( !IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_4BITS) )
                    CheckRadioButton(
                        hwndDlg,
                        IDC_OPRGJPGOPT_RADIO_32BITS,
                        IDC_OPRGJPGOPT_RADIO_4BITS,
                        IDC_OPRGJPGOPT_RADIO_8BITS);
        ...
        case IDC_OPRGJPGOPT_RADIO_16BITS:
            if ( wNotifyCode == BN_CLICKED )
                ...
                CheckDlgButton(hwndDlg, IDC_OPRGJPGOPT_CHECK_MAKEGRAY, FALSE);
                ImageGrayedCtrls(hwndDlg, arOpenData, poud);
                DlgSetModified(hwndDlg);
                }
            break;
        ...
        case IDC_OPRGJPGOPT_RADIO_DISPLAYFINALONLY:
        case IDC_OPRGJPGOPT_RADIO_PROGRESSIVEDISPLAY:
            if ( wNotifyCode == BN_CLICKED )
                {
        ...
        case IDC_OPRGJPGOPT_BUTTON_BROWSE:
            if ( wNotifyCode == BN_CLICKED )
                {
                /* browse for a suitable external palette file.  If the file doesn't have
                    ...
                    error */
                if ( MiscPaletteBrowse(hwndDlg, IDC_OPRGJPGOPT_EDIT_EXTERNALPALETTE, 256) )
                    DlgSetModified(hwndDlg);
                }
            break;

        case IDC_OPRGJPGOPT_CHECK_SETDEFAULT:
        ...
        case IDHELP:
            WinHelp(hwndFrame, APPLICATION_HELPFILENAME, HELP_CONTENTS, IDD_OPRGJPGOPT);
            break;
...
static BOOL ImageOptionsValid(HWND hwndDlg, OPTIONSUSERDATA PICFAR* poud)
{
    ...
    wImageLeft = (WORD)GetDlgItemInt(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGELEFT, &fValid, FALSE);
    if ( ( !fValid &&
           GetDlgItemText(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGELEFT, sz, sizeof(sz)) != 0 ) ||
        ...
        SetFocus(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGELEFT));
        Edit_SetSel(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGELEFT), 0, -1);
        return ( FALSE );
        }
    wImageTop  = (WORD)GetDlgItemInt(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGETOP,  &fValid, FALSE);
    if ( ( !fValid &&
           GetDlgItemText(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGETOP, sz, sizeof(sz)) != 0 ) ||
        ...
        /* "The number is invalid or out of range.  Please enter a number from %d to %d." */
        SetFocus(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGETOP));
        Edit_SetSel(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGETOP), 0, -1);
        return ( FALSE );
          }
     GetDlgItemInt(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGEWIDTH,  &fValid, FALSE);
     if ( !fValid && GetDlgItemText(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGEWIDTH, sz, sizeof(sz)) != 0 )
          {
          ErrorMessage(
                STYLE_ERROR,
                IDS_INVALIDNUM,
        ...
        /* "The number is invalid or out of range.  Please enter a number from %d to %d." */
        SetFocus(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGEWIDTH));
        Edit_SetSel(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGEWIDTH), 0, -1);
        return ( FALSE );
        }
     GetDlgItemInt(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGEHEIGHT,  &fValid, FALSE);
     if ( !fValid && GetDlgItemText(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGEHEIGHT, sz, sizeof(sz)) != 0 )
        {
        ErrorMessage(
            STYLE_ERROR,
            IDS_INVALIDNUM,
          ...
          /* "The number is invalid or out of range.  Please enter a number from %d to %d." */
        SetFocus(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGEHEIGHT));
        Edit_SetSel(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGEHEIGHT), 0, -1);
        return ( FALSE );
        }

    if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_8BITS) )
        maxColors = 256;
    else if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_4BITS) )
        maxColors = 16;
    else
        {
        /* unless the # bits/pixel <= 8, there can't be an inconsistency in the settings
            unless there's a bug */
          assert(!IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_CHECK_MAKEGRAY) &&
               poud->pSrcPicParm->Head.biBitCount > 8);
        return ( TRUE );
        }

    if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_CHECK_MAKEGRAY) )
        {
        /* validate number of grays to make */
        n = GetDlgItemInt(hwndDlg, IDC_OPRGJPGOPT_EDIT_GRAYSTOMAKE, &fValid, FALSE);
        if ( !fValid || n < 2 || n > maxColors )
            ...
            SetFocus(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_GRAYSTOMAKE));
            Edit_SetSel(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_GRAYSTOMAKE), 0, -1);
            return ( FALSE );
            }
        }
        ...
        if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE) )
            {
        ...
        else if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_MAKEPALETTE) )
                {
            /* validate number of colors to make */
            n = GetDlgItemInt(hwndDlg, IDC_OPRGJPGOPT_EDIT_COLORSTOMAKE, &fValid, FALSE);
            if ( !fValid || n < 2 || n > maxColors )
                {
                ErrorMessage(STYLE_ERROR, IDS_INVALIDNUM, 2, maxColors);
                /* "The number is invalid or out of range.  Please enter a number from 2 to %d." */
                SetFocus(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_COLORSTOMAKE));
                Edit_SetSel(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_COLORSTOMAKE), 0, -1);
                return ( FALSE );
                }
            }
        else
            ...
            assert(IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_EXTERNALPALETTE));
            GetDlgItemText(hwndDlg, IDC_OPRGJPGOPT_EDIT_EXTERNALPALETTE, sz, sizeof(sz));
            n = MiscPaletteColors(sz, 0, maxColors);
            if ( n == 0 )
                {
                SetFocus(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_EXTERNALPALETTE));
                Edit_SetSel(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_EXTERNALPALETTE), 0, -1);
                return ( FALSE );
                }
            }
        }
...
static BOOL DefaultOptionsValid(HWND hwndDlg)
{
    ...
    int maxColors;
     ...
     GetDlgItemInt(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGELEFT, &fValid, FALSE);
     if ( !fValid && GetDlgItemText(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGELEFT, sz, sizeof(sz)) != 0 )
          {
     ...
     GetDlgItemInt(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGETOP,  &fValid, FALSE);
     if ( !fValid && GetDlgItemText(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGETOP, sz, sizeof(sz)) != 0 )
          {
          ErrorMessage(STYLE_ERROR, IDS_INVALIDNUM, 0, 9999);
          /* "The number is invalid or out of range.  Please enter a number from %d to %d." */
          SetFocus(GetDlgItem(hwndDlg, IDC_OPRGJPGOPT_EDIT_IMAGETOP));
    ...
    if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_CHECK_MAKEGRAY) )
        ...
        {
        if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_MAKEPALETTE) )
            {
            /* validate number of colors to make */
            n = GetDlgItemInt(hwndDlg, IDC_OPRGJPGOPT_EDIT_COLORSTOMAKE, &fValid, FALSE);
            if ( !fValid || n < 2 || n > maxColors )
        ...
        else if ( !IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE) )
            {
            /* make sure external palette actually has a suitable palette */
            assert(IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_EXTERNALPALETTE));
            GetDlgItemText(hwndDlg, IDC_OPRGJPGOPT_EDIT_EXTERNALPALETTE, sz, sizeof(sz));
...
static void ImageGrayedCtrls(HWND hwndDlg, LPCTRLINITDATA ar, OPTIONSUSERDATA PICFAR* poud)
{
    long lColors;
    long lSecondaryColors;
    BOOL fMakeGray = IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_CHECK_MAKEGRAY);

    CtrlEnable(
        hwndDlg,
        ar,
        IDC_OPRGJPGOPT_CHECK_PROMPTSCAN,
        IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_PROGRESSIVEDISPLAY));
    if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_8BITS) ||
         IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_4BITS) )
        {
        /* target bit depth <= 8 */

        /* enable make gray options */
        CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_CHECK_DITHER, TRUE);
        CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_EDIT_GRAYSTOMAKE, fMakeGray);
        CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_STATIC_GRAYS, fMakeGray);
        ...
        /* disable bit depths > 8 if makegray is checked,
            ...
            (or hurt) */
        CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_RADIO_32BITS, !fMakeGray);
        CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_RADIO_24BITS, !fMakeGray);
        CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_RADIO_16BITS, !fMakeGray);

        if ( fMakeGray )
            {
            EnableColorPalette(hwndDlg, ar, FALSE);
            CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_RADIO_4BITS, TRUE);
            return;
            }
        }
    else
        ...
        CtrlEnable(
            hwndDlg,
            ar,
            IDC_OPRGJPGOPT_CHECK_DITHER,
            IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_16BITS));
        CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_EDIT_GRAYSTOMAKE, FALSE);
        CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_STATIC_GRAYS,     FALSE);
        CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_RADIO_4BITS, TRUE);
    ...
    if ( lColors > 16 )
        ...
        if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE) )
            ...
            if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_4BITS) )
                {
                CheckDlgButton(hwndDlg, IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE, FALSE);
                CheckDlgButton(hwndDlg, IDC_OPRGJPGOPT_RADIO_MAKEPALETTE, TRUE);
                SetGraysToMake(hwndDlg);
                SetColorsToMake(hwndDlg);
                ...
                CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_RADIO_4BITS, FALSE);
            }
        else if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_4BITS) )
            CtrlEnable(hwndDlg, 
                ar,
                IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE,
                FALSE);
        else
            {
            CtrlEnable(
...
{
    /* group box */
    CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_STATIC_COLORPALETTE,   bEnabled);

    /* radio buttons */
    CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_RADIO_EMBEDDEDPALETTE, bEnabled);
    CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_RADIO_MAKEPALETTE,     bEnabled);
    CtrlEnable(hwndDlg, ar, IDC_OPRGJPGOPT_RADIO_EXTERNALPALETTE, bEnabled);

    /* make palette options */
    CtrlEnable(
        hwndDlg,
        ar,
        IDC_OPRGJPGOPT_EDIT_COLORSTOMAKE,
        bEnabled && IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_MAKEPALETTE));
    CtrlEnable(
        hwndDlg,
        ar,
        IDC_OPRGJPGOPT_STATIC_COLORS,
        bEnabled && IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_MAKEPALETTE));
    ...
    CtrlEnable(
        hwndDlg,
        ar,
        IDC_OPRGJPGOPT_STATIC_EXTERNALPALETTE,
        bEnabled &&
        IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_EXTERNALPALETTE));
    CtrlEnable(
        hwndDlg,
        ar,
        IDC_OPRGJPGOPT_EDIT_EXTERNALPALETTE,
        bEnabled &&
        IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_EXTERNALPALETTE));
    CtrlEnable(
        hwndDlg,
        ar,
        IDC_OPRGJPGOPT_BUTTON_BROWSE,
        bEnabled &&
        IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_EXTERNALPALETTE));
}
...
static void SetGraysToMake(HWND hwndDlg)
{
    if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_CHECK_MAKEGRAY) )
        {
        if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_8BITS) )
            SetNumToMake(hwndDlg, IDC_OPRGJPGOPT_EDIT_GRAYSTOMAKE, 256);
        else
            {
            assert(IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_4BITS));
            SetNumToMake(hwndDlg, IDC_OPRGJPGOPT_EDIT_GRAYSTOMAKE, 16);
            }
        }
}
...
{
    if ( !IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_CHECK_MAKEGRAY) &&
         IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_MAKEPALETTE) )
        {
        if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_8BITS) )
            SetNumToMake(hwndDlg, IDC_OPRGJPGOPT_EDIT_COLORSTOMAKE, 256);
        else if ( IsDlgButtonChecked(hwndDlg, IDC_OPRGJPGOPT_RADIO_4BITS) )
            SetNumToMake(hwndDlg, IDC_OPRGJPGOPT_EDIT_COLORSTOMAKE, 16);
        }
}
