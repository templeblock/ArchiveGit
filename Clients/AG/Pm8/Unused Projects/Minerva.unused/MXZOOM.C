...
static void InitPicParm(PIC_PARM PICFAR* pPicParm, LPCTRLINITDATA ar)
{
    if ( CtrlDataIsPushed(ar, IDC_XZOOMOPT_CHECK_PRESERVEASPECT) &&
        ...
        DWORD dw = CtrlDataGetLong(ar, IDC_XZOOMOPT_EDIT_PERCENT, FALSE);
        if ( dw == 0 || dw > 999 )
            dw = 100;
        if ( dw >= 100 )
        {
            pPicParm->u.ZOOM.NewWidth = (long)( sqrt(dw / 100.0) * 100 + 0.5 );
        }
        else
        {
            pPicParm->u.ZOOM.NewWidth = (long)( 100 / sqrt(100.0 / dw) + 0.5 );
        }
        ...
        case -1:
            ...
            if ( IsDlgButtonChecked(hwndDlg, IDC_XZOOMOPT_CHECK_PRESERVEASPECT) )
                ...
                if ( !fValid || percent == 0 || percent > 999 )
                    percent = 100;
                if ( percent >= 100 )
                    percent = (int)( sqrt(percent / 100.0) * 100 + 0.5 );
                else
                    percent = (int)( 100 / sqrt(100.0 / percent) + 0.5 );
                SetImageDimensions(
                    hwndDlg,
                    arZoomData,
            ...
            if ( wNotifyCode == EN_CHANGE )
                ...
                if ( fValid &&
                    ...
                    SetImageDimensions(
                        ...
                        MulDiv(percent, (int)poud->pDibPicParm->Head.biHeight, 100),
                        percent);
                    CtrlDataSetLong(arZoomData, IDC_XZOOMOPT_EDIT_PERCENT, xpercent, FALSE);
                    }
                }
            break;
    ...
    int percent)
{
    ...
    if ( percent >= 100 )
    {
        xpercent = (int)( ( percent / 100.0 ) * ( percent / 100.0 ) * 100 + 0.5 );
    }
    else
    {
        xpercent = (int)( 100.0 / ( ( 100.0 / percent ) * ( 100.0 / percent ) ) + 0.5 );
    }
    ...
    CtrlDataSetLong(ar,    IDC_XZOOMOPT_EDIT_PERCENT,    xpercent,    FALSE);                        
    ...
    SetDlgItemInt(hwndDlg, IDC_XZOOMOPT_EDIT_PERCENT,    xpercent,    FALSE);
}
