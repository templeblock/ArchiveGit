    ...
    int       nCmdShow)         /* see ShowWindow() documentation */
{
    ...
    PegasusLibInit((unsigned int)hinstCurrent);
    ...
    InitOpList();
#ifdef _MT
    /* unload PIC opcode DLL's in MT case -- the threads we spawn to load images
        can't use these because they're owned by the primary thread.  In the MT
        case we just load them initially to make sure that they're there.  An even
        nicer approach would load them in another thread.  When that thread exited
        they would be unloaded into the DLL pool -- there to be available for the
        first thread that need them */
    CleanupOpList();
#endif
...
#ifndef _MT
    /* unload PIC opcode DLL's in the non-MT case */
    CleanupOpList();
#endif
    PegasusLibTerm();
...
static BOOL RegisterWindowClasses(void)
{
    ...
    wcFrame.style         = 0;  
    wcFrame.lpfnWndProc   = (WNDPROC)FrameWndProc;       /* defined in mframe.c */
    ...
    wcChild.style         = 0;  
    wcChild.lpfnWndProc   = (WNDPROC)ChildWndProc;       /* defined in mchild.c */
...
static void LoadOpDlls(void)
{
    ...
    for ( i = 0; pOpenOpTable[i] != 0; i++ )
        ...
        if ( pOpenOpTable[i]->pszAbout )
            {
                /* load from resource, if present, else load from disk */
            result = PegasusLoadFromRes(pOpenOpTable[i]->lOpcode, CURRENT_PARMVER, "", (unsigned int)hinstThis);
                /*  ^ casting to unsigned int eliminates a 16-bit warning about casting a near pointer
                    to a long (if it were cast to long or dword instead) and is effectively
                    the same in 32 bit as casting to dword */
            if ( result == ERR_NONE )
    ...
    for ( i = 0; pSaveOpTable[i] != 0; i++ )
        ...
        if ( pSaveOpTable[i]->pszAbout != 0 )
            {
                /* load from resource, if present, else load from disk */
            result = PegasusLoadFromRes(pSaveOpTable[i]->lOpcode, CURRENT_PARMVER, "", (unsigned int)hinstThis);
            if ( result == ERR_NONE )
                {
    ...
    for ( i = 0; pTransformOpTable[i] != 0; i++ )
        ...
        if ( pTransformOpTable[i]->pszAbout != 0 )
            {
                /* load from resource, if present, else load from disk */
            result = PegasusLoadFromRes(pTransformOpTable[i]->lOpcode, CURRENT_PARMVER, "", (unsigned int)hinstThis);
            if ( result == ERR_NONE )
                {
    ...
    for ( i = 0; pToolsOpTable[i] != 0; i++ )
        ...
        if ( pToolsOpTable[i]->pszAbout != 0 )
            {
                /* load from resource, if present, else load from disk */
            result = PegasusLoadFromRes(pToolsOpTable[i]->lOpcode, CURRENT_PARMVER, "", (unsigned int)hinstThis);
            if ( result == ERR_NONE )
                {
