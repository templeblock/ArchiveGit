...
BOOL ChildQueryCloseAll(void)
{
    ...
    return ( EnumChildWindows(hwndMDIClient, (BOOL (CALLBACK*)(HWND, LPARAM))EnumChildProcQuery, 0) );
}
        ...
        switch ( uMsg )
            ...
            case WM_CREATE:
                ...
                /*#### TODO: change this so a new window gets a number larger than
                    ...
                    but different paths */
                for ( pszName = sz + lstrlen(sz) - 1; pszName >= sz && *pszName != '\\'; pszName-- )
                    ;
                assert(pszName >= sz);
                if ( pszName >= sz )
                    pszName++;
                ...
                while ( !EnumChildWindows(
                            hwndMDIClient,
                            (BOOL (CALLBACK*)(HWND, LPARAM))EnumChildProcFind,
