// PSToolbar.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE PSToolbarDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("PSTOOLBAR.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(PSToolbarDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(PSToolbarDLL);
		m_hInstance = hInstance;
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("PSTOOLBAR.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(PSToolbarDLL);
	}
	return 1;   // ok
}
