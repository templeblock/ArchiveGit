// CraftToolbar.cpp : Defines the initialization routines for the DLL.
//

#include "stdafx.h"
#include <afxdllx.h>
#include "craftToolbar.h"
...
static AFX_EXTENSION_MODULE CraftToolbarDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("CRAFTTOOLBAR.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(CraftToolbarDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(CraftToolbarDLL);
		m_hInstance = hInstance;
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("CRAFTTOOLBAR.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(CraftToolbarDLL);
	}
	return 1;   // ok
}
