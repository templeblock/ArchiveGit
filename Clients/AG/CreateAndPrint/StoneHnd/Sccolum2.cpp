							   ...
							   size_t		numRecords )
{
	...
	spacePtr->opt				= opt;
	spacePtr->max				= max;
	spacePtr->upperBound		= (double)max / opt;
}

/*===================================================================*/
/* Return the minimum line space multiplier allowed 				 */
/* by any line in the column.										 */
static double MaxSpaceStretch( VJSpace*	space,
							 size_t 	numRecords )
{
	double			maxStretch;
...
//j				maxlead = scCachedStyle::GetMaxParaSpace( paraH, tLine->GetPara() );
		...
		if ( maxTotalParaStretch >= vDiff && currParaSpace > 0 ) {
				...
				/* do it. -- double / MicroPoint				*/
			paraStretchFactor = 1 + ((double)vDiff) / currParaSpace;
		}
		else {
			if ( currParaSpace > 0 ) {
			...
			if ( maxTotalLineStretch >= vDiff && currLineSpace > 0 ) {
					...
					/* max line spacing, do it --  double / MicroPoint */
				lineStretchFactor = 1 + ((double)vDiff) / currLineSpace;
			}
			else {
				if ( currLineSpace > 0 ) {
				...
				if ( exceedMaxValues ) {
					if ( currParaSpace > 0 && extraPPspacing ) {
							...
							/* to cover the excess. 					*/
						paraStretchFactor = 1 + ( ((double)vDiff)
									+ maxTotalParaStretch ) / currParaSpace;

					}		/* (double + MicroPoint) / MicroPoint 		*/
					else {
						...
						MicroPoint	paraDiff
							= scRoundMP( ((double)totalParaSpace) / totalSpace * vDiff );

						MicroPoint	lineDiff
							= scRoundMP( ((double)totalLineSpace) / totalSpace * vDiff );

						/* double / MicroPoint / MicroPoint	*/

						if ( currParaSpace )
							paraStretchFactor	= 1 + ( ((double)paraDiff)
										+ maxTotalParaStretch ) / currParaSpace;
						if ( currLineSpace )
							lineStretchFactor	= 1 + ( ((double)lineDiff)
