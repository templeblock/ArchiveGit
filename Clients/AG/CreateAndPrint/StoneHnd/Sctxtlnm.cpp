							   ...
							   const scFlowDir& fd )
{
	...
	if ( fd.IsHorizontal() ) {
		fTop	= org.y - scRoundMP( (double)ptsize * RLU_BASEfmTop / scBaseRLUsystem );
		fMiddle = org.y - scRoundMP( (double)ptsize * ( RLU_BASEfmTop - (scBaseRLUsystem/2)) / scBaseRLUsystem );
		fRoman	= org.y;
		fBottom = org.y + scRoundMP( (double)ptsize * RLU_BASEfmBottom / scBaseRLUsystem );
...
void scDrawLine::SetRubiCharData( const scRubiData& rd, 
								 ...
								 MicroPoint&		letterspace )
{
	double	rubifactor = ( (double)TSRubiSizeFactor( rd.fRubiSpec ) ) / 10000;
...
void scDrawLine::DrawRenMoji()
{
	...
	for ( i = 0; i < fRenMojiArray.GetNumItems(); i++ ) {
		...
		if ( doMakimotoRenMoji && microMeasure > maxMeasure ) {
			...
			setwidthAdjust = (double)maxMeasure / microMeasure;
...
double scTextline::Select( scMuPoint& 		charLoc,
						 long&				count,
						 const scMuPoint&	hitPt,
						 eContentMovement,
						 BOOL&				endOfLine )
{
	...
	if ( vertical ) {
		x = (double)(hitPt.y - curPos.x);
		y = (double)(hitPt.x - curPos.y);
	}
	else {
		x = (double)(hitPt.x - curPos.x);
		y = (double)(hitPt.y - curPos.y);
	}
	x = x * x;
	y = y * y;
	return( x + y );
