...
{ 
	return scRoundMP( (double)size * rlu / scBaseRLUsystem ); 
}

inline GlyphSize SCRLUCompGS( GlyphSize size, RLU rlu )
{ 
	return scRoundGS( (double)size * rlu / scBaseRLUsystem ); 
}
...
public:
	...
	GlyphSize		GetLeftHangValue( UCS2 ch )
						{ return scRoundGS( (double)GetEscapement( ch ) * GetLeftHang() / 10000 ); }
						
	GlyphSize		GetRightHangValue( UCS2 ch )
						{ return scRoundGS( (double)GetEscapement( ch ) * GetRightHang() / 10000 );	}
