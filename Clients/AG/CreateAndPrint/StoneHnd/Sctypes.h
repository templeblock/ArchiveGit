...
inline MicroPoint	scRoundMP( double mp )
					{
						return (MicroPoint)( mp>=0 ? mp+0.5 : mp-0.5 );
					}
inline GlyphSize	scRoundGS( double mp )
					{
						return (GlyphSize)( mp>=0 ? mp+0.5 : mp-0.5 );	
					}

inline double 		AngleToRadians( scAngle ang )	{ return (double)ang * 3.14159 / 18000; }
...
inline double muInches( MicroPoint mp )	{ return (double)mp / one_inch; }
inline long muPoints( MicroPoint mp )	{ return mp / one_point; }
inline long muQs( MicroPoint mp )		{ return scRoundMP( (double)mp / one_millimeter / 4 ); }
