...
namespace ImageEffects
{
	...
	class MinimumFilter : WholeImageFilter
	{
					...
					for (int dy = - 1; dy <= 1; dy++)
					{
						...
						if (0 <= iy && iy < height)
						{
							...
							for (int dx = - 1; dx <= 1; dx++)
							{
								...
								if (0 <= ix && ix < width)
								{
									pixel = PixelUtils::combinePixels(pixel, inPixels->GetAt(ioffset + ix), PixelUtils::MIN);
								}
							}
						}
					}
	...
	public:
		...
		{
			return new CString("Blur/Minimum");
		}
	};
}