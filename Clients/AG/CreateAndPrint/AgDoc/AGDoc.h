...
void SetAddressText(CAGSymText* pText, LPCTSTR strText, bool bClearText=true, COLORREF clrFillColor=RGB(0,0,0), bool bFirstTime=true);
...
public:
	static const CString Name(AGDOCTYPE DocType)
	{
		switch (DocType)
		{
			...
			case DOC_AVERYPOSTER:	return "Avery Poster";
...
public:
	...
	bool AllowEMDlg() const;
	bool NeedsCardBack() const;
	void Free();
	CAGDoc* Duplicate();
	bool IsContextRestricted() const
		{ return m_bContextRestricted; }
	...
	void SetDocType(AGDOCTYPE DocType)
		{ m_AGDocType = DocType; }
	void SetContextLicense(int iValue)
		{ m_iContextLicense = iValue; }
    ...
    bool IsTempFile(CString& szTempdir) const;
	bool GetClipArtProductInfo(CString &szCAInfo);

protected:
	bool m_bContextRestricted;
	bool m_bModified;
	bool m_bPortrait;
	AGDOCTYPE m_AGDocType;
	int m_nPages;
	int m_nCurPage;
	int m_iContextLicense;
