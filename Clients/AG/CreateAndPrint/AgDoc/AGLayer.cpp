...
void CAGLayer::OrderSymbol(CAGSym* pSym, int iOffset)
{
	...
	// Move the affected symbols down one in the list
	for (int i = m_nSymbols; i > iNewPosition; i--)
		m_pSymbols[i] = m_pSymbols[i-1];
...
{
	// If there are AddAPhoto symbols on the page, run through all the
	// symbols and mark the first transparent image as bCoverAllowed
	...
	for (int i = m_nSymbols - 1; i >= 0; i--)
	{
		...
		// If this is an image, call SetCoverDraw
		if (pSym->IsImage())
		{
			bool bCoverDraw = (bCoverAllowed && ((CAGSymImage*)pSym)->IsTransparent());
			((CAGSymImage*)pSym)->SetCoverDraw(bCoverDraw);
			if (bCoverDraw)
				bCoverAllowed = false;
		}

		pSym->Draw(dc, dwFlags);
	}
}
