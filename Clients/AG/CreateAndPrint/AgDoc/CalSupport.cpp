...
void CCalSupport::Draw()
{
	...
	DrawBackgroundRectangle(TitleRect, TitleFillColor);
	DrawBackgroundRectangle(WeekRect, WeekFillColor);
	DrawBackgroundRectangle(DateRect, DateFillColor);
	...
	DrawRectangle(TitleRect, LineColor, LineWidth, CLR_NONE, GetTitleStr(), m_pCalSym->m_SpecMonth, m_pCalSym->m_uCalStyles1.TitleVertJust, nLineWidth, nLineHeight);
	...
	for (int i=0; i<7; i++)
	{
		...
		DrawRectangle(WeekRect, LineColor, LineWidth, CLR_NONE, GetWeekStr(iDay), m_pCalSym->m_SpecWeek, m_pCalSym->m_uCalStyles1.WeekVertJust, nLineWidth, nLineHeight);
	...
	for (int y=0; y<6; y++)
	{
		...
		for (int x=0; x<7; x++)
		{
			...
			if (bDraw)
				DrawRectangle(DateRect, LineColor, LineWidth, CLR_NONE, strText, m_pCalSym->m_SpecDays, m_pCalSym->m_uCalStyles1.DateVertJust, nLineWidth, nLineHeight);
			iDay++;
...
//////////////////////////////////////////////////////////////////////
void CCalSupport::DrawBackgroundRectangle(const RECT& Rect, COLORREF FillColor)
{
	HRGN hClipRegion = m_dc.SaveClipRegion();
	CRect DestRect = Rect;
	m_dc.SetClipRectangle(&DestRect);
...
//////////////////////////////////////////////////////////////////////
void CCalSupport::DrawRectangle(const RECT& Rect, COLORREF LineColor, int LineWidth, COLORREF FillColor, const CString& strText, const CAGSpec& Spec, int VertJust, int nLineWidth, int nLineHeight)
{
	HRGN hClipRegion = m_dc.SaveClipRegion();
	CRect DestRect = Rect;
	m_dc.SetClipRectangle(&DestRect);
