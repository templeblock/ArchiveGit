...
CAGSpec::CAGSpec(const CAGSpec& Spec)
{
	...
	m_sCharSpacePct = Spec.m_sCharSpacePct;
...
//////////////////////////////////////////////////////////////////////
void CAGSpec::DrawTextAligned(CAGDC* pDC, LPCSTR pString, int* pWidths, CRect& RectAlign, eVertJust VertJust) const
{
	int iHorzAlign = 0;
...
//////////////////////////////////////////////////////////////////////
void CAGSpec::DrawText(CAGDC* pDC, LPCSTR pString, int* pWidths, int x, int y, CRect* pRectAlign) const
{
	if (!pDC)
		return;
...
/////////////////////////////////////////////////////////////////////////////
bool CALLBACK CAGSpec::CallbackChangeLineSpacing(CAGSpec* pSpec, CAGSpec* pNewSpecTemplate)
{
	if (!pSpec || !pNewSpecTemplate)
		return false;

	pSpec->m_sLineLeadPct += pNewSpecTemplate->m_sLineLeadPct;
	return true;
}

/////////////////////////////////////////////////////////////////////////////
bool CALLBACK CAGSpec::CallbackChangeCharSpacing(CAGSpec* pSpec, CAGSpec* pNewSpecTemplate)
{
	if (!pSpec || !pNewSpecTemplate)
		return false;

	pSpec->m_sCharSpacePct += pNewSpecTemplate->m_sCharSpacePct;
	return true;
}
...
void CAGText::Invalidate(scXRect& rRect)
{
	...
	CRect rect(rRect.x1, rRect.y1, rRect.x2, rRect.y2);
...
//////////////////////////////////////////////////////////////////////
void CAGText::Create(const CRect& DestRect)
{
	m_DestRect = DestRect;
...
//////////////////////////////////////////////////////////////////////
void CAGText::SetDestRect(const CRect& DestRect)
{
	m_DestRect = DestRect;
	scRedispList colRedisp;
...
//////////////////////////////////////////////////////////////////////
void CAGText::DrawCaret(HDC hDC, const CAGMatrix& Matrix, const CRect& rect)
{
	POINT pt[2];
...
//////////////////////////////////////////////////////////////////////
void CAGText::DrawSelection(HDC hDC, const CAGMatrix& Matrix, const CRect& rect)
{
	if (m_bAutoSelect)
		return;
...
void CAGText::RestoreSelection()
{
	...
	//m_SavedSelectionMark.fLetterSpace += 30;
	//m_SavedSelectionPoint.fLetterSpace += 30;
	if (pStream)
...
//////////////////////////////////////////////////////////////////////
HANDLE CAGText::GetClipboardData(bool bCut)
{
...
{
	HANDLE hMemory = GetClipboardData(false/*bCut*/);
	if (!hMemory)
		return "";
...
//////////////////////////////////////////////////////////////////////
void CAGText::SetLineSpacing(int iUnits)
{
	CAGSpec NewSpecTemplate;
	NewSpecTemplate.m_sLineLeadPct = iUnits;
	SpecChanger(CAGSpec::CallbackChangeLineSpacing, &NewSpecTemplate);
}

//////////////////////////////////////////////////////////////////////
void CAGText::SetCharSpacing(int iUnits)
{
	CAGSpec NewSpecTemplate;
	NewSpecTemplate.m_sCharSpacePct += iUnits;
	SpecChanger(CAGSpec::CallbackChangeCharSpacing, &NewSpecTemplate);
	////if (!m_pSelection)
	////	return;
	...
	////scTextline* txl; // = cslist.fStream->GetFirstline();
	////for ( txl = pStream->GetFirstline(); txl != NULL; txl = LNNext( txl ) ) 
	////{
	////	TypeSpec ts = txl->GetMaxLeadSpec();
	////	
	////	//txl->
	////	//MicroPoint mp = txl->GetLSP();
	////	//txl->SetLSP(mp + iUnits);
	////}
...
status TSGetStyle(TypeSpec& ts, scStyle& st)
{
	...
	int iCharSpacing = ((pSpec->m_sCharSpacePct <= 0) ? 0 : dtoi(((double)tm.tmHeight * pSpec->m_sCharSpacePct) / 1000.0));
	style.SetMinLSP( iCharSpacing ); //0; minimum letter spacing
	style.SetOptLSP( iCharSpacing ); //0; optimum letter spacing including tracking
	style.SetMaxLSP( iCharSpacing ); //0; maximum letter spacing
