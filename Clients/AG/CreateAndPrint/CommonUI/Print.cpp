...
bool CPrint::Start()
{
	...
	if (nCopies <= 0)
	{
		...
		if (m_bLaunchEnvWizard)
		{
			m_bLaunchEnvWizard = false;
			SendMessage(m_pDocWindow->m_hWnd, WM_COMMAND, IDC_DOC_ENVELOPE_WIZ, -1);
		}
		return false;
	}
	...
	do // A convenient block we can break out of
	{
		...
		CAddressbookData* pAddressbookData = m_pCtp->GetAddressbookData();
		if (m_PrintDocType == DOC_ENVELOPE && pAddressbookData && pAddressbookData->IsMailMergeRequested())
			bDoneDialog = DoMailMerge(bPortraitPaper);
		else
		if (m_PrintDocType == DOC_AVERYPOSTER)
			bDoneDialog = DoMultiplePasses(bPortraitPaper);
		else
	...
	if (bDoneDialog)
	{
		...
		if (bShowConfirmationDialog)
		{
			...
			if (m_pAGDoc->FindSymbolAnywhereByID(ID_ENVELOPE_SEND, 0/*dwFlags*/))
			{
				strSrcURL += "&";
			...
			CString szCaInfo;
			m_pAGDoc->GetClipArtProductInfo(szCaInfo);
			strSrcURL += "&";
			strSrcURL += "cainfo=" + szCaInfo;

			CHtmlDialog dlg(strSrcURL, false/*bRes*/, m_pDocWindow->m_hWnd);
			dlg.SetWindowSize(610, 520);// (580+30, 530+70) JHC
		...
		if (!m_pCtp->AutoPrintEnabled())
			m_pDocWindow->LaunchEMDlg(m_PrintDocType, true/*bAtPrintTime*/);
	}
...
bool CPrint::StartPreview()
{
	...
	// Make sure that we are not already in preview mode
	if (::IsWindow(m_PreviewFrame.m_hWnd))
		m_PreviewFrame.DestroyWindow();

    // Create the main window
    if (!m_PreviewFrame.CreateEx())
        return false;       

	CRect FrameRect;
	m_pDocWindow->GetWindowRect(&FrameRect);
	m_PreviewFrame.SetWindowText(_T("Create & Print Preview"));
	HICON hIcon = ::LoadIcon(_AtlBaseModule.GetResourceInstance(), MAKEINTRESOURCE(IDI_AGICON));
	m_PreviewFrame.SetIcon(hIcon);

	// Need to set the doc type before we determine the number of pages.
	// TFT Commented for task#3
	//m_PrintDocType = m_pAGDoc->GetDocType();
	m_iPaperType = 0;
	m_bForceDuplexTest = false;

	// Need to determine the number of preview pages
	int nPages = DetermineNumPreviewPages();

	// Create the preview window view and assign it to the preview frame.
	m_PreviewWnd.Create(m_PreviewFrame.m_hWnd, FrameRect, NULL, 0, WS_EX_CLIENTEDGE);
	m_PreviewWnd.Init(m_PrintDocType == DOC_ENVELOPE);

	CPrinter Printer;
	Printer.OpenDefaultPrinter();
	m_PreviewWnd.SetPrintPreviewInfo(Printer.Detach(), m_pDevMode, this, 0, nPages);
	m_PreviewWnd.SetPage(0);
	m_PreviewFrame.m_hWndClient = m_PreviewWnd;

	// We will need to access the things later
	m_PreviewFrame.SetPreviewWindow(&m_PreviewWnd);
	m_PreviewFrame.SetMainHwnd(m_pDocWindow->m_hWnd);

	m_PreviewWnd.ShowWindow(SW_SHOW);
	m_PreviewWnd.UpdateWindow();	

	m_PreviewFrame.UpdateLayout();

    // We should maximize the preview window
	WINDOWPLACEMENT WP;
	m_PreviewFrame.GetWindowPlacement(&WP);

	WP.showCmd = SW_MAXIMIZE;
 	WP.length = sizeof(WINDOWPLACEMENT);

    m_PreviewFrame.ShowWindow(SW_SHOW);
    m_PreviewFrame.UpdateWindow();
	m_PreviewFrame.SetWindowPlacement(&WP);
	m_PreviewFrame.SetWindowPos(HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE|SWP_NOSIZE);
...
bool CPrint::ContinuePrinting(bool bPortraitPaper)
{
	...
	// extract the gif for the html
	strTempFile = strTempDir;
	strTempFile += "header.gif";
	WriteResourceToFile(hInstance, IDR_PRINTFINALSTEP_HEADER, "GIF", strTempFile);
	...
	if (bPortrait)
	{
		if (bTopFeed)
		{
			strAngle1 = bToward ? "180" : "0";
			strAngle2 = bToward ? "180" : "0";
		}
		else
		{
			strAngle1 = !bToward ? "180" : "0";
	...
	CHtmlDialog dlg(strURL, false, m_pDocWindow->m_hWnd);
	dlg.SetWindowSize(580, 520);
	dlg.SetResizable(true);
	dlg.SetScrollbars(false);
	dlg.SetValue("title", g_szAppName);
	dlg.SetValue("face", pFace);
	dlg.SetValue("direction", pDirection);
	dlg.SetValue("angle1", strAngle1);
	dlg.SetValue("angle2", strAngle2);
	dlg.DoModal();
...
//////////////////////////////////////////////////////////////////////
bool CPrint::MailMergePrint(CAGPrintDC* pDC, long lIndex, bool bPreview, bool& bPortraitPaper)
{
	CAddressbookData* pAddressbookData = m_pCtp->GetAddressbookData();
	if (!pAddressbookData)
		return false;

	long lTotalEntries = pAddressbookData->GetSize();
	if (lIndex >= lTotalEntries)
		return false;

	CString szAddressEntry;
	int iResults = pAddressbookData->GetMailingAddressByIndex(lIndex, szAddressEntry);
	if ((iResults & MAILLIST_STATUS_ERROR) || (iResults & MAILLIST_STATUS_EMPTY))
		return false;

	if (szAddressEntry.IsEmpty())
		return false;

	CAGSymText* pText = (CAGSymText*)m_pAGDoc->FindSymbolAnywhereByID(ID_ENVELOPE_SEND, PROCESS_HIDDEN);
	if (!pText)
		return false;

	SetAddressText(pText, szAddressEntry, false, CLR_NONE, false);
	m_pDocWindow->SymbolInvalidate(pText);
	bool bSuccess = DoPass(pDC, 1 + lIndex, bPortraitPaper, bPreview);

	return bSuccess;
}
//////////////////////////////////////////////////////////////////////
bool CPrint::DoMailMerge(bool& bPortraitPaper)
{
	...
	bool bSuccess = true;
	long lIndex = 0;
	while (bSuccess && lIndex < lTotalEntries)
	{
		bSuccess = MailMergePrint(NULL/*pPrintDC*/, lIndex, false/*bPreview*/, bPortraitPaper);
		lIndex++;
	}

	CEMDlg* pEMDlg = m_pDocWindow->GetEMDlg();
	if (pEMDlg)
		pEMDlg->UpdateMailingAddress();

	return bSuccess;
}

//////////////////////////////////////////////////////////////////////
bool CPrint::DoMultiplePasses(bool& bPortraitPaper)
{
	bool bRetVal;

	bRetVal = DoPass(NULL/*pPrintDC*/, 1, bPortraitPaper);
	if (bRetVal)
		bRetVal = DoPass(NULL/*pPrintDC*/, 2, bPortraitPaper);
	if (bRetVal)
		bRetVal = DoPass(NULL/*pPrintDC*/, 3, bPortraitPaper);
	if (bRetVal)
		bRetVal = DoPass(NULL/*pPrintDC*/, 4, bPortraitPaper);

	return bRetVal;
}

//////////////////////////////////////////////////////////////////////
bool CPrint::DoPass(CAGPrintDC* pPrintDC, int iPass, bool& bPortraitPaper, bool bPreview)
{
	return DoPass(pPrintDC, 0, iPass, bPortraitPaper, bPreview);
}

bool CPrint::DoPass(CAGPrintDC* pPrintDC,  int iStart, int iPass, bool& bPortraitPaper, bool bPreview)
{
	// All possible page orders: negative value means we rotate it 180 degrees
	const int QuarterSideFoldOrder[] = {-1, -4, 2, 3};
	const int QuarterSideFoldOrderPreview[] = {-3, -2, 4, 1};
	const int QuarterTopFoldOrder[] = {-1, 2, 4, 3};
	const int QuarterTopFoldOrderPreview[] = {-3, -4, -2, 1};

	const int HalfSideFoldPass1Order[] = {-1, -4};
	const int HalfSideFoldPass1OrderPreview[] = {4, 1};
	const int HalfSideFoldPass2Order[] = {-3, -2};
	const int HalfSideFoldPass2OrderPreview[] = {2, 3};
	const int HalfTopFoldPass1Order[] = {-1, 4};
	const int HalfTopFoldPass1OrderPreview[] = {-4, 1};
	const int HalfTopFoldPass2Order[] = {-3, -2};
	const int HalfTopFoldPass2OrderPreview[] = {2, 3};
	...
	const int TrifoldPortraitPass1OrderPreview[] = {5, 6, 1};
	const int TrifoldPortraitPass2Order[] = {-4, -3, -2};
	const int TrifoldPortraitPass2OrderPreview[] = {2, 3, 4};
	const int TrifoldLandscapePass1Order[] = {-1, -6, -5};
	const int TrifoldLandscapePass1OrderPreview[] = {5, 6, 1};
	const int TrifoldLandscapePass2Order[] = {-4, -3, -2};
	const int TrifoldLandscapePass2OrderPreview[] = {2, 3, 4};
	...
	PRINT_QUADRANTS dwQuadrant = QUADRANT_NONE;
	...
	int nPanelsPerPage = pPaperTemplate->nRows * pPaperTemplate->nCols;

	int nPreviewCopies = nPanelsPerPage;
	...
	switch (m_PrintDocType)
	{
		...
		case DOC_AVERYPOSTER:
		{
			...
			{
			case 1:
				dwQuadrant = QUADRANT_ONE;
				break;
			case 2:
				dwQuadrant = QUADRANT_TWO;
				break;
			case 3:
				dwQuadrant = QUADRANT_THREE;
				break;
			case 4:
				dwQuadrant = QUADRANT_FOUR;
				break;
			};
		...
		case DOC_HALFCARD:
		{
			...
			{
				if (bPreview)
					pPageOrder = (bSideFold ? &HalfSideFoldPass1OrderPreview[0] : &HalfTopFoldPass1OrderPreview[0]);
				else
			...
			{
				if (bPreview)
					pPageOrder = (bSideFold ? &HalfSideFoldPass2OrderPreview[0] : &HalfTopFoldPass2OrderPreview[0]);
				else
		...
		case DOC_QUARTERCARD:
		{
			...
			{
				if (bPreview)
					pPageOrder = (bSideFold ? &QuarterSideFoldOrderPreview[0] : &QuarterTopFoldOrderPreview[0]);
				else
		...
		case DOC_TRIFOLD:
		{
			...
			{
				if (bPreview)
					pPageOrder = (bPortrait ? &TrifoldPortraitPass1OrderPreview[0] : &TrifoldLandscapePass1OrderPreview[0]);
				else
			...
			{
				if (bPreview)
					pPageOrder = (bPortrait ? &TrifoldPortraitPass2OrderPreview[0] : &TrifoldLandscapePass2OrderPreview[0]);
				else
	...
	*m_pDevMode = DevModeSaved; // Restore the DevMode before we leave
	...
	bool bSent = Send(pPageOrder, iStart, nPages, *pPaperTemplate, bPortraitPaper, bMirror, bCenterOnly, nCopies, dwQuadrant, pDC, bPreview);
	if (!pDC->EndDoc())
		pDC->AbortDoc();
...
//////////////////////////////////////////////////////////////////////
bool CPrint::Send(const int* pPageOrder, int iStart, int nPages, const PaperTemplate& PaperTemplate, bool bPortraitPaper, bool bMirror, bool bCenterOnly, int nCopies, PRINT_QUADRANTS dwQuadrant, CAGPrintDC* pDC, bool bPreview)
{
	...
	CAGDIBSectionDC* pMirrorDC = NULL;
	...
	if (bMirror)
	{
		// TFT Changes: moved the contents of CreateMirroDib routine 
		// to over here so that the local variable bi is not moved out of scope
		// while it is saved as pointer in the m_pDIB member variable

		// Because mirrroring the output consumes so much memory,
		// protect against an unreasonably high print resolution
		const int kMaxResolution = 300;

		if (DeviceInfo.m_iResolutionX > kMaxResolution)
		{
			// TFT Change
			if (!bPreview && iStart == 0)
		...
		pMirrorDC = new CAGDIBSectionDC(bi, DIB_RGB_COLORS, pDC->GetHDC());
		if (!pMirrorDC)
		{
	...
	while (nImpressions < nTotalImpressions)
	{
		...
		for (int r = 0; r < nRows && nImpressions < nTotalImpressions; r++)
		{
			for (int c = 0; c < nCols && nImpressions < nTotalImpressions; c++)
			{
				...
				if (!bTemplateRotated)
				{
					//SetDestRect(c, r, dwQuadrant, PaperTemplate, DestRect);
				...
				CRect PageRect(0, 0, PageSize.cx, PageSize.cy);
				...
				else
				{
					...
					case QUADRANT_ONE:
						{
							xOffset = INCHES(0.375);
							yOffset = INCHES(0.375) + INCHES(0.0625);
							ViewMatrix.Translate(xOffset, yOffset);
						}
						break;
					case QUADRANT_TWO:
						{
							xOffset = (((PageSize.cx - INCHES(0.25))/2) - INCHES(0.125)) - INCHES(0.375) + INCHES(0.125);
							yOffset = INCHES(0.375) + INCHES(0.0625);
							ViewMatrix.Translate(-xOffset, yOffset);
						}
						break;
					case QUADRANT_THREE:
						{
							xOffset = INCHES(0.375);
							yOffset = (((PageSize.cy - INCHES(0.25))/2) - INCHES(0.125)) - INCHES(0.375) + INCHES(0.0625);
							ViewMatrix.Translate(xOffset, -yOffset);
						}
						break;
					case QUADRANT_FOUR:
				...
				if (pMirrorDC)
				{
					...
					{
						CDib DibSrc = pDIB;
						BITMAPINFOHEADER* pDIBNew = DibSrc.Orient(0/*iRotateDirection*/, true/*bFlipX*/, false/*bFlipY*/, pBits);
						if (!pDIBNew)
				...
				{
					if (bPreview && m_PrintDocType == DOC_ENVELOPE)
					{
						CSize PageSize(0, 0);
						pPage->GetPageSize(PageSize);
						CRect DestRect(0, 0, PageSize.cx-1, PageSize.cy-1);

						CAGMatrix Matrix = pDC->GetViewToDeviceMatrix();
						CPoint pPoints[4];
						Matrix.TransformRectToPolygon(DestRect, pPoints);

						COLORREF FillColor = RGB(255,255,255);
						COLORREF LineColor = CLR_NONE; // RGB(0,0,0)
						pDC->DrawRectangle(pPoints, LineColor, LT_None, FillColor); // LT_Hairline
					}
...
void CPrint::OrientEnvelope(bool bPortraitPaper, RECT& DestRect, int iDeviceWidth, int iDeviceHeight, bool& bRotate180)
{
    ...
    if (bTopFeed)
    {
        ...
        else if (iPos  == IDC_PAPERFEED_POS_RIGHT)	
...
// TFT Changes ...moved the contents to CPrint::Send() below //
...
int CPrint::ShowDialog(PRINTDLG* pPrintDlg)
{
	...
	if (!::PrintDlg(&pd) || m_bLaunchEnvWizard)
	{
	...
	*m_pDevMode = DevModeSaved; // Restore the DevMode before we leave
	...
	do // A convenient block we can break out of
	{
		...
		if (bFirstTest)
		{
			...
			CRect MsgRect(0, DeviceInfo.m_iResolutionY * 3, DeviceInfo.m_Size.cx, DeviceInfo.m_iResolutionY * 6);
...
//////////////////////////////////////////////////////////////////////
void InitializeLabel(CLabel& Label, HWND hWnd, bool bStyle1)
{
    Label.InitDialog(hWnd);
    Label.SetFontBold(bStyle1 ? true : false);
    Label.SetFontSize(bStyle1 ? 13 : 12);
    Label.SetTextColor(bStyle1 ? RGB(0,0,255) : RGB(0x64,0x96,0xC8));
	Label.SetFontName("Microsoft Sans Serif");
	Label.SetBackgroundColor(CLR_NONE);
}
...
void CPrint::InitWindow(HWND hWnd)
{
	...
	if (m_pCtp)
		m_pCtp->SetWindowIcon(hWnd);

	CSize PageSize(0, 0);
	m_pAGDoc->GetPageSize(PageSize);

	::EnableWindow(::GetDlgItem(hWnd, IDC_PAPERTYPE), (m_PrintDocType!=DOC_ENVELOPE));
	::ShowWindow(::GetDlgItem(hWnd, IDC_ENVELOPE_HELP), ((m_PrintDocType==DOC_ENVELOPE) ? SW_SHOW : SW_HIDE));
	::ShowWindow(::GetDlgItem(hWnd, IDC_BTN_ENVWIZ), ((m_PrintDocType==DOC_ENVELOPE) ? SW_SHOW : SW_HIDE));
	...
	{
		CString strDocLabel = (m_PrintDocType == DOC_ENVELOPE)? "Envelope Size" : "Paper Type";
		...
		::SetWindowText(::GetDlgItem(hWnd, IDC_PAPERTYPE_LABEL), strDocLabel);
	}
	...
	InitializeLabel(m_lblSelectPrintGroup, ::GetDlgItem(hWnd, IDC_SELECTPRINTER_GROUP), true);
	InitializeLabel(m_lblCopiesGroup, ::GetDlgItem(hWnd, IDC_COPIES_GROUP), true);
	InitializeLabel(m_lblPaperTypeGroup, ::GetDlgItem(hWnd, IDC_PAPERTYPE_LABEL), true);
//	InitializeLabel(m_lblEnvelopeHelp, ::GetDlgItem(hWnd, IDC_ENVELOPE_HELP), true);
	InitializeLabel(m_lblEnvelopePreviewGroup, ::GetDlgItem(hWnd, IDC_ENVELOPEPREVIEW_GROUP), true);
	InitializeLabel(m_lblPrinterSettingsGroup, ::GetDlgItem(hWnd, IDC_PRINTSETTINGS_GROUP), true);
	InitializeLabel(m_lblPaperLoadHeader, ::GetDlgItem(hWnd, IDC_PAPERLOAD_STATIC), false);
	InitializeLabel(m_lblPaperAlignHeader, ::GetDlgItem(hWnd, IDC_PAPERALIGN_STATIC), false);
	InitializeLabel(m_lblEnvelopeLoadHeader, ::GetDlgItem(hWnd, IDC_ENVELOPELOADS_STATIC), false);
	InitializeLabel(m_lblEnvelopeEdgeHeader, ::GetDlgItem(hWnd, IDC_ENVELOPEEDGE_STATIC), false);
...
bool CPrint::InitializeOrientation(HWND hWnd)
{
    ...
    int nShow = SW_HIDE;
    if (m_PrintDocType == DOC_ENVELOPE || IsCustomPaper())
        nShow = SW_SHOW;
	...
	switch (msg)
	{
		...
		case WM_COMMAND:
		{
			...
			switch (wID)
			{
				case IDC_BTN_ENVWIZ:
				{
					m_bLaunchEnvWizard = true;
					EndDialog(hWnd, IDCANCEL);
					break;
				}
				case IDC_BUTTON_PRINTPREVIEW:
				{
...
//////////////////////////////////////////////////////////////////////
// CPrintJobInfo override - used during print preview
bool CPrint::PrintPage(UINT nPage, HDC hDC)
{
	CAGPrintDC dc(hDC, true/*bPreview*/);
	...
	if (m_PrintDocType == DOC_ENVELOPE && pAddressbookData && pAddressbookData->IsMailMergeRequested())
	{
		MailMergePrint(&dc, nPage, true/*bPreview*/, bPortraitPaper);
		CEMDlg* pEMDlg = m_pDocWindow->GetEMDlg();
		if (pEMDlg)
			pEMDlg->UpdateMailingAddress();
	}
	else
		DoPass(&dc, nPage, 1 + nPage, bPortraitPaper, true/*bPreview*/);
...
//////////////////////////////////////////////////////////////////////
// This was stupidly copied from DoPass()
int CPrint::DetermineNumPreviewPages()
{
#ifndef USED
	CSize PageSize(0, 0);
	m_pAGDoc->GetPageSize(PageSize);
	...
	switch (m_PrintDocType)
	{
		...
		{
			CAddressbookData* pAddressbookData = m_pCtp->GetAddressbookData();
			if (pAddressbookData && pAddressbookData->IsMailMergeRequested())
				nPages = pAddressbookData->GetSize();
			break;
		}
		//case DOC_BROCHURE:
		case DOC_QUARTERCARD:
		//case DOC_POSTCARD:
		// TFT Change do not assign 1 to nPages
		{
			nPages = 1; 
			break;
		}
	...
	bool bReverseOrientation = (m_PrintDocType != DOC_ENVELOPE ? pPaperTemplate->bReverseOrientation : (CPaperFeed::GetEdge(m_iPaperFeedCode) == IDC_PAPERFEED_EDGE_TOP));
	bool bPortraitPaper = (bReverseOrientation ? !bPortrait : bPortrait); // This is returned to the caller
