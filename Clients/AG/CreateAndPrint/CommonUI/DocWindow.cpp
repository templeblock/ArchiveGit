...
LRESULT CDocWindow::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    // Setup MRU
    m_MenuMru = m_popupFile.GetSubMenu(9);
	m_mruList.SetMenuHandle(m_MenuMru);
	m_mruList.SetMaxEntries(6);
...
//j	maybe select the m_pClientDC's palette into hPaintDC
	...
	if (!m_pAGDoc)
	{
		::FillRect(hPaintDC, &PaintStruct.rcPaint, CAGBrush(BROWSER_BACKGROUND_COLOR));
...
LRESULT CDocWindow::OnRButtonUp(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	bool bAllowNewPages = (m_pAGDoc ? (m_pAGDoc->AllowNewPages() | (bSpecialsEnabled ? m_pAGDoc->NeedsCardBack() : false)) : false);
	bool bSymSelected = m_Select.SymSelected();
	bool bClipArtSelected = bSymSelected && m_Select.SymIsClipArt();
	bool bIsAmericanGreetings = m_pCtp->IsAmericanGreetings();

	Popup.EnableMenuItem(0/*Image Editor*/,     MF_BYPOSITION | (m_pAGDoc && m_Select.SymIsImage() ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(6/*Select*/,           MF_BYPOSITION | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(7/*Layers*/,           MF_BYPOSITION | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(8/*Add*/,              MF_BYPOSITION | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(9/*Zoom*/,             MF_BYPOSITION | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(10/*Position*/,        MF_BYPOSITION | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(11/*Transform*/,       MF_BYPOSITION | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(12/*Size*/,            MF_BYPOSITION | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(13/*Page*/,            MF_BYPOSITION | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	...
	Popup.EnableMenuItem(ID_IME_CROP,			MF_BYCOMMAND | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(ID_IME_FLIPANDROTATE,  MF_BYCOMMAND | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(ID_IME_CONTRAST,		MF_BYCOMMAND | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(ID_IME_COLORTINT,      MF_BYCOMMAND | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(ID_IME_REDEYE,			MF_BYCOMMAND | (m_pAGDoc && !bClipArtSelected ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(ID_IME_ADDBORDER,      MF_BYCOMMAND | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(ID_IME_SPECIALEFFECTS, MF_BYCOMMAND | (m_pAGDoc ? MF_ENABLED : MF_GRAYED));
	...
	Popup.EnableMenuItem(IDC_DOC_FLIP,          MF_BYCOMMAND | (m_pAGDoc && bSymSelected && !(m_Select.SymIsText() || m_Select.SymIsCalendar()) ? MF_ENABLED : MF_GRAYED));
	Popup.EnableMenuItem(IDC_DOC_MIRROR,        MF_BYCOMMAND | (m_pAGDoc && bSymSelected && !(m_Select.SymIsText() || m_Select.SymIsCalendar()) ? MF_ENABLED : MF_GRAYED));
	...
	Popup.EnableMenuItem(IDC_DOC_TEXTROTATE,    MF_BYCOMMAND | (m_pAGDoc && bSymSelected ? MF_ENABLED : MF_GRAYED));
	...
	else
	{
		...
		if (pSym->IsImage())
		{
			...
			if (pDIB)
			{
				...
				CString strSourceType = pImage->GetSourceType();
...
void CDocWindow::SymbolInvalidate(CAGSym* pSym)
{
	...
	if (m_pClientDC->GetCoverAllowed() && pSym->IsImage() && ((CAGSymImage*)pSym)->GetCoverDraw())
	{
...
void CDocWindow::SetupPageView(bool bSetScrollbars, bool bOffset)
{
	...
	m_pClientDC->SetClipToView(PageSize, &m_PageRect, true/*bIncludeRawDC*/);
...
bool CDocWindow::DoCommand(const CString& strCommand)
{
	...
	else if (strCommand == "PRINTPREVIEW")
		lResult = SendMessage(WM_COMMAND, IDC_DOC_PRINT_PREVIEW);
...
void AdjustArrowKeyByTransform(const CAGSym* pSym, UINT& nChar, bool bInverse)
{
	...
	pSym->GetMatrix().Translate(iX, iY);*/
...
/////////////////////////////////////////////////////////////////////////////
bool CDocWindow::SwitchToEnvelope(bool bAtPrintTime, UINT iTabID)
{
    if (!m_pAGDoc)
		return false;

    if (!m_pAGDoc->AllowEMDlg())
		return false;
    ...
    if (DOC_ENVELOPE != DocType)
    {
        ...
        if (!m_LinkedDoc.szEnvelopeFilePath.IsEmpty() && !bAtPrintTime)
        {
			...
			if (m_pEMDlg && ::IsWindow(m_pEMDlg->m_hWnd))
			{
				m_pEMDlg->SetEnvelopeDoc(m_pAGDoc);
				m_pEMDlg->SetCurrentTab(iTabID);
				m_pEMDlg->ShowWindow(SW_SHOW);
			}
        }
        else if (m_LinkedDoc.szEnvelopeFilePath.IsEmpty())
        {
            ...
            CEMDlg* pEMDlg = new CEMDlg(m_hWnd, this, m_pAGDoc, DocType, iTabID);
	        if (!pEMDlg)
		        return false;
            ...
            m_pEMDlg = pEMDlg;
            m_pEMDlg->CreateEnvelope(&m_pAGDoc);
            Refresh(_T("Untitled Envelope"));

	        RECT EnvRect;
	        m_pEMDlg->GetWindowRect(&EnvRect);
	        RECT DocRect;
	        ::GetWindowRect(m_pCtp->GetBrowser(), &DocRect);
	        int nYOffset = (((DocRect.bottom - DocRect.top) - (EnvRect.bottom - EnvRect.top))/2);
	        m_pEMDlg->SetWindowPos(HWND_NOTOPMOST, (DocRect.right - (EnvRect.right - EnvRect.left)), (DocRect.top + nYOffset), 0, 0, SWP_NOSIZE);
	        m_pEMDlg->ShowWindow(SW_SHOW);
        }
    }
    else // DOC_ENVELOPE == DocType
    {
        ...
        if (!bAtPrintTime)
        {
			...
			{
				delete m_pEMDlg;
				m_pEMDlg = new CEMDlg(m_hWnd, this, m_pAGDoc, DocType, iTabID);
				if (m_pEMDlg)
				{
					m_pEMDlg->SetEnvelopeDoc(m_pAGDoc);
					RECT EnvRect;
...
bool CDocWindow::SwitchToCard()
{
	...
	// If Envelope Wizard is up and running then clear reference to envelope doc
	// since the envelope has been closed and is no longer valid. 
	// Note: The envelope doc reference is set in SwitchToEnvelope() after it is 
	// either created or opened. 
	if (m_pEMDlg)
	{
		m_pEMDlg->SetEnvelopeDoc(NULL);
	}
...
//j	bool bDlgMode	= !!(wFlags & KF_DLGMODE);
	...
	if (bKeyDown)
	{
		...
		if (bControl)
		{
			if (nChar == 'Q' && !bRepeat)		idMessage = IDC_LOOSEN_LINE;		else
			if (nChar == 'W' && !bRepeat)		idMessage = IDC_TIGHTEN_LINES;		else
			if (nChar == 'E' && !bRepeat)		idMessage = IDC_LOOSEN_CHARS;		else
			if (nChar == 'R' && !bRepeat)		idMessage = IDC_TIGHTEN_CHARS;		else
	...
	switch (nChar)
	{
        ...
        case VK_F3: // Text Rotate
		{
			int bHandled = true;
			if (bRepeat)
				return true;
			if (bKeyDown)
				g_pDocWindow->SendMessage(WM_COMMAND, IDC_DOC_TEXTROTATE);
			return true;
		}
		...
		case VK_F5: // Select Next/Previous
		{
			int bHandled = true;
			if (bRepeat)
				return true;
			if (bKeyDown && SHIFT)
				g_pDocWindow->SendMessage(WM_COMMAND, IDC_DOC_SELECTPREV);
			else if (bKeyDown)
