...
LRESULT CCtp::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	bool bHpUI = IsHpUI();
	if (bHpUI)
		m_pAutoUpdate = new CHpAutoUpdate(this);
	else
		m_pAutoUpdate = new CAutoUpdate(this);

	g_szAppName.Format(bHpUI ? IDS_MSGBOXTITLE2 : IDS_MSGBOXTITLE);
	...
	CMessageBox::SetDefaults(hWndBrowser, g_szAppName, (bHpUI ? "MessageBox_hp.htm" : "MessageBox.htm"));
...
//j	if (hWndBrowser)
//j	{
//j		CComPtr<IWebBrowser2> spBrowser = GetWebBrowserInterface();
//j		if (spBrowser) // Suppress Drag/drop
//j			spBrowser->put_RegisterAsDropTarget(VARIANT_FALSE);
//j	}

	CWindow(hWndBrowser).DragAcceptFiles(false);
...
void CCtp::Free()
{
	if (m_pArtGalleryDialog)
	{
		delete m_pArtGalleryDialog;
		m_pArtGalleryDialog = NULL;
	}

	if (m_pAddressbookData)
	{
		delete m_pAddressbookData;
		m_pAddressbookData = NULL;
	}
...
void CCtp::CreateBackPage()
{
	...
	// The Copyright text
	long lWidth = (bHpUI ? INCHES(2.75) : INCHES(2.5));
	long lHeight = (bHpUI ? INCHES(0.185) : INCHES(0.375));
	...
	RECT LogoRect;
	long lSize = (bHpUI ? INCHES(1.375) : INCHES(1.75));
	::SetRect(&ImageRect, 0, 0, lSize, lSize);
	SYMID idSiteLogo = (bHpUI ? ID_HPAGLOGO : (bBlueMountain ? ID_BMLOGO : ID_AGLOGO));
	CAGSymImage* pSymLogo = GetImageFromResource(idSiteLogo, &ImageRect);
	if (pSymLogo)
	{
		pSymLogo->SetID(ID_AGLOGO);
	...
	if (bHpUI)
	{
		::SetRect(&ImageRect, 0, 0, INCHES(1.25), INCHES(1.25));
		pSymSponsorLogo = GetImageFromResource(ID_HPLOGO, &ImageRect);
		if (pSymSponsorLogo)
		{
			ImageRect = pSymSponsorLogo->GetDestRect();
			RECT Rect = {0, 0, WIDTH(ImageRect), HEIGHT(ImageRect)};
			SponsorLogoRect = Rect;
		}
	}
	
	int iMessageFontHeight = (bHpUI ? POINTUNITS(10) : POINTUNITS(14));
	int iCopyrightFontHeight = (bHpUI ? POINTUNITS(8) : POINTUNITS(9));
	...
	if (pText)
	{
		pText->SetID(ID_CARDBACK_COPYRIGHT);
		...
		CString pRawText = (bHpUI ? "© Hewlett-Packard Company, " : strURL + "\n");
		pRawText += "© AG.com, Inc.";
	...
	if (pText)
	{
		pText->SetID(ID_CARDBACK_MESSAGE);
...
/////////////////////////////////////////////////////////////////////////////
CAGSymImage* CCtp::GetImageFromResource(SYMID nID, const RECT* pMaxBounds)
{
	CAGDoc* pAGDoc = (!m_pDocWindow ? NULL : m_pDocWindow->GetDoc());
	if (!pAGDoc)
		return NULL;
	...
	CImage Image(_AtlBaseModule.GetResourceInstance(), SYMIDToResourceIdTable[nID], "GIF");
...
HWND CCtp::GetBrowser()
{
	...
	if (iFirstSeparator >= 0)
	{
		...
		::SetWindowText(m_hWndBrowser, strTitle); // Temp - JHC
		
	}

	// Change the browser's icons to our logo icon
	SetWindowIcon(m_hWndBrowser);

	return m_hWndBrowser;
}
/////////////////////////////////////////////////////////////////////////////
bool CCtp::SetWindowIcon(HWND hwnd)
{
	if (!::IsWindow(hwnd))
		return false;

	bool bBlueMountain = IsBlueMountain();
	int idSiteIcon = (IsHpUI() ? IDI_HPICON : (bBlueMountain ? IDI_BMICON : IDI_AGICON));
...
/////////////////////////////////////////////////////////////////////////////
HRESULT CCtp::get_Version(BSTR* bstrVersion)
{
	*bstrVersion = CComBSTR(VER_PRODUCT_VERSION_STR).Copy();
	return S_OK;
}
...
#ifdef FYS
	if (m_pDocWindow)
		m_pDocWindow->CloseFYSRemoteDlg();
...
/////////////////////////////////////////////////////////////////////////////
HRESULT CCtp::FileDownload(BSTR bstrSrcURL, BSTR bstrInfo, BSTR bstrOptionString)
{ // The graphics file download function called by the Art Gallery's HTML dialog
	CString strSrcURL(bstrSrcURL);
	if (strSrcURL.IsEmpty() || NULL == m_pArtGalleryDialog)
		return S_OK;

	m_pArtGalleryDialog->InitClipArtDownload(CString(bstrInfo), CString(bstrOptionString));
...
/////////////////////////////////////////////////////////////////////////////
HRESULT CCtp::ProcessAddressbook(VARIANT varData)
{
	if (NULL == m_pAddressbookData)
		return S_OK;

	if (m_pAddressbookData->LoadXML(CString(varData.bstrVal)))
	{
		if (m_pDocWindow->m_pEMDlg)
			m_pDocWindow->m_pEMDlg->UpdateMailingAddress();
	}

	return S_OK;
}
/////////////////////////////////////////////////////////////////////////////
HRESULT CCtp::SetAddressbookUrl(VARIANT varUrl)
{
	CString szTemp = varUrl.bstrVal;
	szTemp.MakeLower();
	if (szTemp.Find(_T("http")) == -1 || NULL == m_pAddressbookData)
		return S_OK;

	m_pAddressbookData->SetAddressbookUrl(CString(varUrl.bstrVal)); 

	return S_OK;
}
/////////////////////////////////////////////////////////////////////////////
HRESULT CCtp::ShowAddressbookPopup(VARIANT varShow)
{
	if (NULL == m_pAddressbookData)
		return S_OK;

	if (0 == varShow.intVal)
		m_pAddressbookData->SetWindowShowCommand(SW_HIDE);
	else
		m_pAddressbookData->SetWindowShowCommand(SW_SHOW);

	return S_OK;
}
/////////////////////////////////////////////////////////////////////////////
HRESULT CCtp::ResizeAddressbookPopup(VARIANT varWidth, VARIANT varHeight)
{
	if (NULL == m_pAddressbookData)
		return S_OK;

	int iWidth = varWidth.intVal;
	int iHeight = varHeight.intVal;

	if (iWidth > 10 && iHeight > 10)
		m_pAddressbookData->SetWindowSize(iWidth, iHeight);

	return S_OK;
}
/////////////////////////////////////////////////////////////////////////////
HRESULT CCtp::CloseAddressbookPopup()
{
	if (NULL == m_pAddressbookData)
		return S_OK;

	m_pAddressbookData->ClosePopup();
	
	return S_OK;
}
/////////////////////////////////////////////////////////////////////////////
HRESULT CCtp::SetClipArtUrl(VARIANT varUrl)
{
	if (NULL == m_pArtGalleryDialog)
		return S_OK;

	CString szVal = varUrl.bstrVal;
	m_pArtGalleryDialog->SetURL(szVal);
	return S_OK;
}
/////////////////////////////////////////////////////////////////////////////
HRESULT CCtp::ShowClipArtPopup(VARIANT varShow)
{
	if (NULL == m_pArtGalleryDialog)
		return S_OK;

	if (0 == varShow.intVal)
		m_pArtGalleryDialog->SetWindowShowCommand(SW_HIDE);
	else
		m_pArtGalleryDialog->SetWindowShowCommand(SW_SHOW);

	return S_OK;
}
/////////////////////////////////////////////////////////////////////////////
HRESULT CCtp::ResizeClipArtPopup(VARIANT varWidth, VARIANT varHeight)
{
	if (NULL == m_pArtGalleryDialog)
		return S_OK;

	m_pArtGalleryDialog->SetWindowSize(varWidth.intVal, varHeight.intVal);
	return S_OK;
}
/////////////////////////////////////////////////////////////////////////////
HRESULT CCtp::CloseClipArtPopup()
{
	if (NULL == m_pArtGalleryDialog)
		return S_OK;

	m_pArtGalleryDialog->ClosePopup();
	return S_OK;
}
/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
bool CCtp::LaunchAddressbook(HWND hwndParent)
{
	CWaitCursor Wait;
	bool bSuccess = false;
	if (m_pAddressbookData)
	...
	try
	{
		CComPtr<IWebBrowser2> spBrowser = GetWebBrowserInterface();
		if (!spBrowser)
			return false;

		m_pAddressbookData = new CAddressbookData(this);
		m_pAddressbookData->SetDispatch(this);
		m_pAddressbookData->RequestAddressbookSetup(spBrowser);
		bSuccess = m_pAddressbookData->LaunchAddressbook(hwndParent);
		CEMDlg* pEMDlg = GetDocWindow()->GetEMDlg();
		if (pEMDlg)
...
/////////////////////////////////////////////////////////////////////////////
bool CCtp::LaunchClipArtPopup(HWND hwndParent, PFNARTGALLERYCALLBACK pfnArtGalleryCallback, LPARAM lParam, LPCTSTR szClient)
{
	CWaitCursor Wait;
	bool bSuccess = false;

	CAGDoc* pAGDoc = m_pDocWindow->GetDoc();
	if (NULL == pAGDoc)
		return false;
	...
	try
	{
		...
		m_pArtGalleryDialog = new CArtGalleryDialog(this);
		m_pArtGalleryDialog->SetDispatch(this);
		
		CString szTemp(szClient);
		if (szTemp.CompareNoCase(CLIENT_CTP) == 0)
		{
			m_pArtGalleryDialog->CallJavaScript(_T("SetupStandardClipArt"), _T(""), spBrowser);
		}
		else if (szTemp.CompareNoCase(CLIENT_IME) == 0)
		{
			m_pArtGalleryDialog->CallJavaScript(_T("SetupIMEClipArt"), _T(""), spBrowser);
		}

		m_pArtGalleryDialog->RegisterCallback(pfnArtGalleryCallback, lParam);
		bSuccess = m_pArtGalleryDialog->LaunchClipArtPopup(hwndParent, szClient);
	}
	catch(...)
	{
		return false;
...
/////////////////////////////////////////////////////////////////////////////
void CALLBACK CCtp::MyArtSetupCallback(HWND hwnd, PFNARTGALLERYCALLBACK pfnArtGalleryCallback, LPARAM lArtGalParam, LPARAM lParam, LPCTSTR szValue)
{
	CCtp * pCtp = (CCtp*)lParam;
	if (!pCtp)
		return;

	pCtp->LaunchClipArtPopup(hwnd, pfnArtGalleryCallback, lArtGalParam, szValue);
}
/////////////////////////////////////////////////////////////////////////////
void CALLBACK CCtp::MyArtGalleryCallback(HWND hwnd, UINT uMsgId, LPARAM lParam, LPARAM lArtGalleryPtr, LPCTSTR szValue)
{
	CCtp * pCtp = (CCtp*)lParam;
	if (!pCtp)
		return;
	...
	{
		CString szFileName(szValue);
		CString szResults = _T("error");
		if (!szFileName.IsEmpty())
		{
			HRESULT hr = pCtp->GetDocWindow()->AddImage((LPCSTR)szFileName, 0, 0, 0, false);
			if (SUCCEEDED(hr))
				szResults = _T("success");
		}

		pCtp->GetArtGalleryDialog()->ArtAddComplete(szResults);
	}
	else if (WM_INITDIALOG == uMsgId)
	{
		pCtp->SetWindowIcon(hwnd);
	}
...
/////////////////////////////////////////////////////////////////////////////
CComPtr<IWebBrowser2> CCtp::GetWebBrowserInterface()
{
	CComQIPtr<IServiceProvider> spServiceProvider(m_spClientSite);
...
HRESULT CCtp::ProcessImage(BSTR bstrFilePath, int nMaxKB, int nQuality, BSTR* bstrFilePathNew)
{
	...
	*bstrFilePathNew = CComBSTR(bstrFilePath).Copy();
	...
	BITMAPINFOHEADER* pDibDst = DibAlloc(DibSize(&DibDst));
	if (!pDibDst)
		return S_OK;
...
#ifdef NETSCAPE
/////////////////////////////////////////////////////////////////////////////
void CCtp::SetNPPInstance(NPP pInstance)
{
...
STDMETHODIMP CCtp::Drop(IDataObject* pDataObject, DWORD grfKeyState, POINTL pt, DWORD* pdwEffect)
{
	...
	if (m_pDocWindow->m_pEMDlg && m_pDocWindow->m_pEMDlg->IsWindowVisible())
	{
		m_pDocWindow->m_pEMDlg->FinishImageDrop(true/*bDrop*/, pt);
		return S_OK;
	}
...
//j	::MessageBox(NULL, "QueryContinueDrag", "", MB_OK);
	...
	if (fEscapePressed)
	{
		if (m_pDocWindow && m_pDocWindow->m_pEMDlg && m_pDocWindow->m_pEMDlg->IsWindowVisible())
		{
			//Clean up from drag
			POINTL pt;
			m_pDocWindow->m_pEMDlg->FinishImageDrop(false/*bDrop*/, pt);
		}
