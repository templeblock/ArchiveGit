...
bool CFYSPrintDoc::DumpImageFile(CAGSymImage* pSymImage, CString& strImagePath)
{
	...
	if (hMemory)
	{
		...
		{
			fwrite(pMemory, sizeof(BYTE), dwSize, imgout);
			fclose(imgout);
		}
	...
	if (!hMemory || bError)
		bError = DibWrite(pSymImage->GetDib(), strImageFile);
...
void CFYSPrintDoc::UnpackFiles()
{
	...
	HGLOBAL hMemory = ::GlobalAlloc(GMEM_MOVEABLE, sizeof(BYTE));
