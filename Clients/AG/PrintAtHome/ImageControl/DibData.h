...
public:
	...
	// Bitmap update
	//BOOL SetDIBits(BITMAPINFOHEADER* pbmpDest);

	// Pixel set/get
	DWORD GetPixelValue(int x, int y) const;
	BOOL SetPixelValue(int x, int y, DWORD dwValue);
	...
	// Line drawing
	BOOL DrawLineH(int x1, int x2, int y, DWORD dwValue);
	...
	//// Convert to DIB compatible with source DIB
	//CDibData* GetCompatibleDIB(CDibData& dibSrc);
