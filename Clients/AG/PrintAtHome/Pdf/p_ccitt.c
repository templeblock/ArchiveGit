...
/* $Id: p_ccitt.c,v 1.14.2.4 2002/01/26 18:32:18 tm Exp $
 *
...
pdf_data_source_CCITT_fill(PDF *p, PDF_data_source *src)
{
    ...
    src->bytes_available =
	fread(src->buffer_start, 1, CCITT_BUFSIZE, image->fp);
		...
		int BitReverse, int K, int BlackIs1)
{
    ...
    PDF_TRACE(("%s\t(pdf[%p], \"%s\", %d, %d, %d, %d, %d);\n",
    	fn, (void *) p, filename, width, height, BitReverse, K, BlackIs1));
	...
	if (p->debug['i']) {
		...
		"Couldn't open CCITT file '%s'", filename);
	}
        return -1;
    }
