...
/* $Id: p_afm.c,v 1.16.2.2 2002/01/07 18:26:29 tm Exp $
 *
 * PDFlib AFM parsing routines
 *
 * This source is based on Adobe's AFM parser. It has been adapted
...
enum parseKey {
  ASCENDER, CHARBBOX, CODE, COMPCHAR, CAPHEIGHT, CHARACTERS, CHARACTERSET,
  CHARWIDTH,
  COMMENT, DESCENDER, ENCODINGSCHEME, ENDCHARMETRICS, ENDCOMPOSITES, 
  ENDFONTMETRICS, ENDKERNDATA, ENDKERNPAIRS, ENDTRACKKERN, 
  FAMILYNAME, FONTBBOX, FONTNAME, FULLNAME, ISFIXEDPITCH, 
...
static const char *keyStrings[] = {
  "Ascender", "B", "C", "CC", "CapHeight", "Characters", "CharacterSet",
  "CharWidth",
  "Comment", "Descender", "EncodingScheme", "EndCharMetrics", "EndComposites", 
  "EndFontMetrics", "EndKernData", "EndKernPairs", "EndTrackKern", 
  "FamilyName", "FontBBox", "FontName", "FullName", "IsFixedPitch", 
...
parseGlobals(PDF *p, FILE *fp, pdf_font *font, char *ident)
{
	...
	switch(recognize(keyword)) {
		...
		break;
	    /* New AFM 4.1 keyword "CharacterSet" */
	    case CHARACTERSET:
		(void) token(fp, ident);	/* eat character set name */
		break;
		...
		keyword = token(fp, ident);
		/*
		 * "AdobeStandardEncoding" is correct, but some AFMs don't
		 * bother and use "StandardEncoding" instead. Fix this now.
		 */
		if (!strcmp(keyword, "StandardEncoding"))
		    keyword = "AdobeStandardEncoding";

		font->encodingScheme = pdf_strdup(p, keyword);
		break; 
	    case FULLNAME:
		/* discard -- no longer used */
		(void) linetoken(fp, ident);
		break; 
	    case FAMILYNAME:           
		...
		keyword = linetoken(fp, ident);
		weight = pdf_strdup(p, keyword);
		weight_to_be_freed = 1;
		break;
	    case ITALICANGLE:
		keyword = token(fp, ident);
...
parseCharMetrics(PDF *p, FILE *fp, pdf_font *font, char *ident)
{
	...
	pdf_error(p, PDF_NonfatalError,
	    "Bad StartCharMetrics entry (%d) in AFM file of font '%s'",
	    font->numOfChars, font->name);
	*/
        error = parseError;
    }
...
#define PDF_DEFAULT_CHAR	"space"
	...
	if (!strcmp(cmi->name, PDF_DEFAULT_CHAR)) {
	    default_width = cmi->wx;
	    break;
	}
    }
