...
/* ==================================================================== */
void scSelection::GetCharSpecListEx( scSpecLocList& csList )
{
	long		offset1,
				offset2;
	scContUnit* p;
	...
	if ( p1 == p2 )
	{
		scSpecLocation specLoc( p1->GetCount(), -1, p1->GetDefaultSpec() );
		specLoc.offset().fCharOffset = -9999;
		csList.Append( specLoc );
...
/* ==================================================================== */
/* called from a continue click or after a appmouse down, hilite the new area
 */
							...
							scRedispList*	redispList )
{
	...
	*pbOverflow = (refevent == eOverflowGeometry);
...
static void GetLocationInfo( TextMarker& tm, scStreamLocation& pl )
{
	...
	if ( pl.fChSpec.ptr() ) {
		...
		memset( &pl.fFont, 0, sizeof( APPFont ) );
		pl.fPointSize		= 0;
	}
...
/* ==================================================================== */
status scSelection::Iter( SubstituteFunc	func,
						   scRedispList*	redispList )
{	
	scSelection 	validatedSelection;
	scContUnit	*para,
	...
	else
	{
		...
		for ( para = startParaH->GetNext(); para != endParaH; para = para->GetNext() )
		{
			...
			if (para->Iter( func, 0, endLocation ) == scUserAbort)
			{
				stat = scUserAbort;
				break;
			}
