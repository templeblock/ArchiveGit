...
void scStream::CopyAPPText( stTextImportExport& appText )
{
	...
	for (scContUnit* para = First(); para; para = para->GetNext( ) )
...
/* ==================================================================== */
status scStream::Iter( SubstituteFunc func,
					 scRedispList*	damage )
{
	status stat = scSuccess;
	for (scContUnit* para = First(); para; para = para->GetNext( ) )
	{
		long endOffset = PARAChSize( para );
		if (para->Iter( func, 0, endOffset) == scUserAbort)
		{
			stat = scUserAbort;
			break;
		}
