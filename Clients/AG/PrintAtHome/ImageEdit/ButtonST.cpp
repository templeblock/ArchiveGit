procedure TMyListView.WMCtlColorEdit(var Message: TWMCtlColorEdit);
begin
  Message.Result := DefWindowProc(Handle, Message.Msg, Message.ChildDC,
    Message.ChildWnd);
end;
...
DWORD CButtonST::SetURL(LPCTSTR lpszURL)
{
	...
	{
		// Store the URL
		int iLength = min(sizeof(m_szURL), _tcslen(lpszURL) + 1);
		memcpy(m_szURL, lpszURL, iLength);
	} // if
...
DWORD CButtonST::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
	...
	if (lpszSound)
	{
		...
		{
			int iLength = min(sizeof(m_csSounds[byIndex].szSound), _tcslen(lpszSound) + 1);
			memcpy(m_csSounds[byIndex].szSound, lpszSound, iLength);
