...
typedef bool (CALLBACK * FNDOWNLOADCALLBACK)(LPARAM lParam, void* pDownload, long lError);
...
//a	}
	...
	//	if (!pClientSite)
	//		::MessageBox(NULL, String("Error downloading %s.  No client site.", CString(bstrSrcURL)), "", MB_OK);
		...
		::MessageBox(NULL, String("%s (0x%lx)\nError while trying to download %s.", strError, hResult, CString(bstrSrcURL)), "", MB_OK);
		return hResult;
	}
