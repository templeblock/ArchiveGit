...
protected:
	...
	bool SetDevModeWidthAndHeight(const SIZE& PaperSize);
	bool VerifyPaperSize(SIZE& PageSize);
	...
	bool DoPass(CAGPrintDC* pPrintDC, int iPass, bool& bPortraitPaper, bool fPrintPreview = false);
	bool Send(const int* pPageOrder, int iStart, int nPages, const PaperTemplate& PaperTemplate, bool bPortraitPaper, bool bMirror, bool bCenterOnly, int nCopies, PRINT_QUADRANTS dwQuadrant, CAGPrintDC* pDC, bool fPrintPreview);
...
protected:
	...
	CPrintPreviewWindowEx m_PrintPreviewWnd;
	CPrinter m_printer;
	CDevMode m_devmode;
	HDC m_hDC;

	bool m_fPrintPreview;
};
