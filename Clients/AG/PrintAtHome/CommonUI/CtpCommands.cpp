...
#import "progid:PAHContentMgr.Pack"
using namespace PAHContentMgr;
...
/////////////////////////////////////////////////////////////////////////////
LRESULT CDocWindow::OnTest(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	CComPtr<IPack> spPack;
	HRESULT hr = spPack.CoCreateInstance(CComBSTR("PAHContentMgr.Pack"), NULL, CLSCTX_INPROC_SERVER);
	if (SUCCEEDED(hr))
		hr = spPack->ProcessPurchase("");
...
LRESULT CDocWindow::OnDocNew(UINT code, UINT id, HWND hWnd, BOOL& bHandled)
{
	...
	if (fabs(pNewDocData->m_fStdHeight - pNewDocData->m_fHeight) > 0.05 ||
		fabs(pNewDocData->m_fStdWidth - pNewDocData->m_fWidth) > 0.05)
	{
		m_pAGDoc->SetCustomLayout(true);
	}
...
LRESULT CDocWindow::LaunchEnvelopeWizard(AGDOCTYPE DocType, bool bAtPrintTime)
{
    ...
    if (!SwitchToEnvelope(bAtPrintTime))
        return E_FAIL;
...
CString CDocWindow::GetDocFileName()
{
	...
	ofx.of.lpstrFilter = "Project Files (*.pah)\0*.pah\0Create&Print (*.ctp)\0*.*ctp\0\0";
	CString strTitle = "Open a saved " + g_szAppName + " project";
	ofx.of.lpstrTitle = strTitle;
	ofx.of.lpstrDefExt = "pah";
...
LRESULT CDocWindow::Close(bool bExit, bool bSilent)
{
	...
	if (m_pEnvelopeWizard)
	{
		if (bSilent)
        {
            m_pEnvelopeWizard->ShowWindow(SW_HIDE);
        }
        else
        {
...
LRESULT CDocWindow::SaveAs(bool bSilent, bool bExit)
{
	...
	if (!bSilent)
	{
		...
		ofx.of.lpfnHook = CDocWindow::OpenFileHookProc;
		ofx.of.nMaxFile = MAX_PATH;
		ofx.of.lpstrFilter = "Project Files (*.pah)\0*.*pah\0\0";
...
{
	HRESULT hr = S_OK;
	hr = (m_Print.StartPreview() ? S_OK : E_FAIL);
