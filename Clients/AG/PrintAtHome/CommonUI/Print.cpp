...
CPrint::CPrint(CDocWindow* pDocWindow)
{
	...
	m_bPaperSearchRequested	 = false;
	m_fPrintPreview = false;
...
bool CPrint::IsCustomPaper(AGDOCTYPE PrintDocType)
{
	...
	bool bCustom = (PrintDocType == DOC_HOLIDAYCARD	||
				   PrintDocType == DOC_NOTECARD		||
				   PrintDocType == DOC_ENVELOPE		||
				   PrintDocType == DOC_PHOTOCARD);
				   

	return (bCustom || m_pAGDoc->IsCustomLayout());
}

//////////////////////////////////////////////////////////////////////
bool CPrint::IsAveryPoster()
{
	return (m_PrintDocType == DOC_AVERYPOSTER);
}

//////////////////////////////////////////////////////////////////////
bool CPrint::ShowDuplexTest()
{
	return (IsDocDuplex() || (IsCustomPaper() && m_PrintDocType != DOC_ENVELOPE));
}
...
bool CPrint::Start()
{
	...
	if (nCopies <= 0)
	{
		...
		{
			m_bLaunchEnvWizard = false;
			SendMessage(m_pDocWindow->m_hWnd, WM_COMMAND, IDC_DOC_ENVELOPE_WIZ, 0);
		}
		return false;
	}
...
bool CPrint::StartPreview()
{
	...
	if (m_fPrintPreview && ::IsWindow(m_PreviewFrame.m_hWnd))
	{
    ...
    if ( NULL == m_PreviewFrame.CreateEx() )
        return false;       

	RECT rect;
	m_pDocWindow->GetWindowRect(&rect);
	m_PreviewFrame.SetWindowText(_T("Print Preview"));
	...
	m_PrintPreviewWnd.Create(m_PreviewFrame.m_hWnd, rect, NULL, 0, WS_EX_CLIENTEDGE);
	m_PrintPreviewWnd.SetPrintPreviewInfo(m_printer, m_pDevMode, this, 0, nPages);
	m_PrintPreviewWnd.SetPage(0);
	m_PreviewFrame.m_hWndClient = m_PrintPreviewWnd;

	//We will need to access the things later
	m_PreviewFrame.SetPreviewWindow(&m_PrintPreviewWnd);
	m_PreviewFrame.SetMainHwnd(m_pDocWindow->m_hWnd);

	m_PrintPreviewWnd.ShowWindow(SW_SHOW);
	m_PrintPreviewWnd.UpdateWindow();	
...
bool CPrint::DoMultiplePasses(bool& bPortraitPaper)
{
	...
	bRetVal = DoPass(NULL, 1, bPortraitPaper);
	if (bRetVal)
		bRetVal = DoPass(NULL, 2, bPortraitPaper);
	if (bRetVal)
		bRetVal = DoPass(NULL, 3, bPortraitPaper);
	if (bRetVal)
		bRetVal = DoPass(NULL, 4, bPortraitPaper);

	return bRetVal;
}

//////////////////////////////////////////////////////////////////////
bool CPrint::DoPass(CAGPrintDC* pPrintDC, int iPass, bool& bPortraitPaper, bool fPrintPreview)
{
	...
	SetPaperSize(pPaperTemplate, m_pAGDoc, false/*bSilent*/); //jhc
	...
	*m_pDevMode = DevModeSaved; // Restore the DevMode before we leave
	...
	bool bSent = Send(pPageOrder, iStart, nPages, *pPaperTemplate, bPortraitPaper, bMirror, bCenterOnly, nCopies, dwQuadrant, pDC, fPrintPreview);
	if (!pDC->EndDoc())
		pDC->AbortDoc();
...
//////////////////////////////////////////////////////////////////////
bool CPrint::Send(const int* pPageOrder, int iStart, int nPages, const PaperTemplate& PaperTemplate, bool bPortraitPaper, bool bMirror, bool bCenterOnly, int nCopies, PRINT_QUADRANTS dwQuadrant, CAGPrintDC* pDC, bool fPrintPreview)
{
	...
	{
		if (!fPrintPreview && !pDC->StartPage())
		{
		...
		for (int r = 0; r < nRows && nImpressions < nTotalImpressions; r++)
		{
			for (int c = 0; c < nCols && nImpressions < nTotalImpressions; c++)
			{
				...
				// If printing a doc where pages have a custom layout (i.e. custom width and height)
				// then override paper template size with actual page size.
				double fWidthWithGap = PaperTemplate.fWidthWithGap;
				double fHeightWithGap = PaperTemplate.fHeightWithGap;
				double fWidth = PaperTemplate.fWidth;
				double fHeight = PaperTemplate.fHeight;;
				double fCustomWidth = PaperTemplate.fCustomWidth;
				double fCustomHeight = PaperTemplate.fCustomHeight;
				SIZE SizeOfPage = {0,0};
				if (m_pAGDoc->IsCustomLayout())
				{
					pPage->GetPageSize(SizeOfPage);
					fWidth		= DINCHES(SizeOfPage.cx);
					fHeight		= DINCHES(SizeOfPage.cy);
					fWidthWithGap	= fWidth;
					fHeightWithGap	= fHeight;
					fCustomWidth	= fWidth;
					fCustomHeight	= fHeight; 
				}
				...
				if (!bTemplateRotated)
				{
					...
					double fLeft = PaperTemplate.fLeft + (c * fWidthWithGap);
					double fTop  = PaperTemplate.fTop + (r * fHeightWithGap);
					DestRect.left   = INCHES(fLeft);
					DestRect.top    = INCHES(fTop);
					DestRect.right  = INCHES(fLeft + fWidth);
					DestRect.bottom = INCHES(fTop + fHeight);
				}
				else
				{
					double fLeft = PaperTemplate.fTop + (c * fHeightWithGap);
					double fTop  = PaperTemplate.fLeft + (r * fWidthWithGap);
					DestRect.left   = INCHES(fLeft);
					DestRect.top    = INCHES(fTop);
					DestRect.right  = INCHES(fLeft + fHeight);
					DestRect.bottom = INCHES(fTop + fWidth);
				}
				...
				if (!bMatch)
				{
					...
					if (IsCustomPaper())
						OrientCustomPaper(bPortraitPaper, DestRect, iDeviceWidth, iDeviceHeight, INCHES(fCustomWidth), INCHES(fCustomHeight));
				}
...
bool CPrint::SetPaperSize(const PaperTemplate* pPaperTemplate, CAGDoc* pAGDoc, bool bSilent)
{
	...
	// Set the devmode width and height directly unless 
	// overriden by user via m_bPaperSearchRequested.
	// Setting width/height works for majority of printers, 
	// however to handle cases where its not supported the 
	// user may override behavior with m_bPaperSearchRequested
	// which will look for best paper size match.
	bool bPaperVerified = false;
	if (!m_bPaperSearchRequested)
	{
		SIZE CustomPaperSize = {0,0};
		pAGDoc->GetPaperSize(CustomPaperSize, 0.0, 0.0);
		SetDevModeWidthAndHeight(CustomPaperSize);
		bPaperVerified = VerifyPaperSize(CustomPaperSize);
	}
	...
	bool bDoPaperSearch = bBorderless || m_bPaperSearchRequested || !bPaperVerified;
	if (!bDoPaperSearch)
	{
		delete [] pn;
		if (bPaperVerified)
...
//////////////////////////////////////////////////////////////////////
bool CPrint::SetDevModeWidthAndHeight(const SIZE& PaperSize)
{
	m_pDevMode->dmPaperSize = 0;// Most printers require this to be 0 (i.e.DMPAPER_USER does not always work);
	m_pDevMode->dmFields |= DM_PAPERSIZE;
...
bool CPrint::SetDevModeSizeNT(LPSTR pstrDevice, const SIZE& PaperSize)
{
	...
	strcpy_s((LPSTR)m_pDevMode->dmFormName, _countof(m_pDevMode->dmFormName), pFormName);
	m_pDevMode->dmFields |= DM_FORMNAME;
	m_pDevMode->dmFields &= ~DM_PAPERSIZE;
	m_pDevMode->dmFields &= ~DM_PAPERWIDTH;
	m_pDevMode->dmFields &= ~DM_PAPERLENGTH;
...
void CPrint::GetMargins(CAGDoc* pAGDoc, RECT& Margins)
{
	...
	if (IsCustomPaper(PrintDocType) && m_PrintDocType != DOC_ENVELOPE)
	{
		int iPos = CPaperFeed::GetPos(m_iPaperFeedCode);
...
////////////////////////////////////////////////////////////////////////
bool CPrint::VerifyPaperSize(SIZE& PageSize)
{
	HDC hDC = ::CreateIC(m_strDriver, m_strDevice, m_strPort, m_pDevMode);
	if (!hDC)
		return false;
	...
	int iResolutionX = ::GetDeviceCaps(hDC, LOGPIXELSX);
	int iResolutionY = ::GetDeviceCaps(hDC, LOGPIXELSY);
	int iWidth	= ::GetDeviceCaps(hDC, PHYSICALWIDTH) * 254 / iResolutionX;
	int iPageHeight	 = ::GetDeviceCaps(hDC, PHYSICALHEIGHT) * 254 / iResolutionY;

	::DeleteDC(hDC);

	int xDevSize = min(iWidth, iPageHeight);
	int yDevSize = max(iWidth, iPageHeight); 
	int xPaperSize = min(PageSize.cx, PageSize.cy);
	int yPaperSize = max(PageSize.cx, PageSize.cy);

	if (abs(xPaperSize - xDevSize) > 20 || abs(yPaperSize - yDevSize) > 20)
		return false;
...
void CPrint::InitWindow(HWND hWnd)
{
	...
	::CheckDlgButton(hWnd, IDC_CHECK_PAPERSEARCH, BST_UNCHECKED);
	...
	else
	{
		...
		CPaperTemplates::StuffCombo(m_PrintDocType, 0, NULL, GetDlgItem(hWnd, IDC_PAPERTYPE), PageSize.cx, PageSize.cy, false/*bIgnoreCompetitors*/);
    ...
    m_lblEnvelopeHelp.InitDialog(::GetDlgItem(hWnd, IDC_ENVELOPE_HELP));
    m_lblEnvelopeHelp.SetFontBold(false);
    m_lblEnvelopeHelp.SetFontSize(12);
    m_lblEnvelopeHelp.SetTextColor(RGB(0x00,0x00,0xFF));
	m_lblEnvelopeHelp.SetFontName("Microsoft Sans Serif");
	m_lblEnvelopeHelp.SetBackgroundColor(CLR_NONE);
		...
		case WM_COMMAND:
		{
			...
			switch (wID)
			{
				...
				case IDOK:
				{
					...
					m_bPaperSearchRequested	 = !!::IsDlgButtonChecked(hWnd, IDC_CHECK_PAPERSEARCH);
...
//////////////////////////////////////////////////////////////////////
void CPrint::PrePrintPage(UINT nPage, HDC hDC)
{
	m_hDC = hDC;
}
...
{
	CAGPrintPreviewDC* pDC = new CAGPrintPreviewDC(m_hDC);
	if (pDC == NULL)
		return false;
	...
	bool bPortraitPaper = true;
	DoPass(pDC, 1 + nPage, bPortraitPaper, true/*fPrintPreview*/);

	return true;
}

//////////////////////////////////////////////////////////////////////
void CPrint::PostPrintPage(UINT nPage, HDC hDC)
{
}
