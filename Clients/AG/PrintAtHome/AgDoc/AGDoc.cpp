...
/////////////////////////////////////////////////////
//
// Bit Field format for Fileheader.Ver BYTE
//
//        _____________________________ unused
//       |  ___________________________ unused
//       | |    _______________________ Custom Layout
//       | |   |	 __________________ Restrict Context
//	     | |   |	|	  _____________ Version
//       | |   |	|	 |		  |
//   [   7 6   5    4 ][ 3        0 ]
//
/////////////////////////////////////////////////////
const BYTE VERSION_MASK			= 0x0F; // bits 0 - 3 : represents header version.
const BYTE RESTRICTCONTEXT_MASK = 0x10; // bit 4 : determines if envelope wizard suppresses images, i.e. Disney
const BYTE CUSTOMLAYOUT_MASK	= 0x20; // bit 5 : determines if doc has custom layout
...
{ // return the paper size required by the document (in tenths of a millimeter)
	if (!fWidth && !fHeight)
	{
		if (m_AGDocType == DOC_BANNER || m_AGDocType == DOC_ENVELOPE || m_bCustomLayout)
		{
			SIZE PageSize;
...
bool CAGDoc::Read(istream& input, bool& bAdjusted, bool bAllowAdjustment)
{
	...
	// Extract custom layout bit flag
	m_bCustomLayout = ((FileHeader.Ver & CUSTOMLAYOUT_MASK) != 0);
	...
	if (nVersion <= 3)
	{
		...
		if (!bAllowAdjustment || m_bCustomLayout)
			return true;
...
bool CAGDoc::Write(ostream& output) const
{
	...
	// If custom layout then set Custom Layout bit.
	if (m_bCustomLayout)
		nVerInfo |= CUSTOMLAYOUT_MASK;
