...
IPicture* CImage::NativeImageToPictureWithGdiPlus(IStream* pStream)
{
	...
	PFNGdipCreateBitmapFromStream pfnGdipCreateBitmapFromStream = NULL;
	pfnGdipCreateBitmapFromStream = (PFNGdipCreateBitmapFromStream)::GetProcAddress(hGdiPlusLib, "GdipCreateBitmapFromStream");

	typedef GpStatus (WINGDIPAPI *PFNGdipCreateHBITMAPFromBitmap)(GpBitmap* bitmap, HBITMAP* hbmReturn, ARGB background);
	PFNGdipCreateHBITMAPFromBitmap pfnGdipCreateHBITMAPFromBitmap = NULL;
	pfnGdipCreateHBITMAPFromBitmap = (PFNGdipCreateHBITMAPFromBitmap)::GetProcAddress(hGdiPlusLib, "GdipCreateHBITMAPFromBitmap");

	typedef GpStatus (WINGDIPAPI *PFNGdipDisposeImage)(GpImage* image);
	PFNGdipDisposeImage pfnGdipDisposeImage = NULL;
	pfnGdipDisposeImage = (PFNGdipDisposeImage)::GetProcAddress(hGdiPlusLib, "GdipDisposeImage");

	typedef GpStatus (WINAPI *PFNGdiplusStartup)(ULONG_PTR* token, const GdiplusStartupInput* input, GdiplusStartupOutput* output);
	PFNGdiplusStartup pfnGdiplusStartup = NULL;
	pfnGdiplusStartup = (PFNGdiplusStartup)::GetProcAddress(hGdiPlusLib, "GdiplusStartup");

	typedef void (WINAPI *PFNGdiplusShutdown)(ULONG_PTR token);
	PFNGdiplusShutdown pfnGdiplusShutdown = NULL;
	pfnGdiplusShutdown = (PFNGdiplusShutdown)::GetProcAddress(hGdiPlusLib, "GdiplusShutdown");

	if (!pfnGdipCreateBitmapFromStream || !pfnGdipCreateHBITMAPFromBitmap || !pfnGdipDisposeImage || !pfnGdiplusStartup || !pfnGdiplusShutdown)
	{
	...
	// Initialize GDI+
	GpStatus status;
	ULONG_PTR dwToken = NULL;
	GdiplusStartupInput input;
	GdiplusStartupOutput output;
	status = pfnGdiplusStartup(&dwToken, &input, &output);
	if (status != Ok)
	{
		::FreeLibrary(hGdiPlusLib);
		return NULL;
	...
	status = pfnGdipCreateBitmapFromStream(pStream, &pBitmap);
	if (status != Ok || !pBitmap)
	{
		pfnGdiplusShutdown(dwToken);
		::FreeLibrary(hGdiPlusLib);
		return NULL;
	}
	...
	status = pfnGdipCreateHBITMAPFromBitmap(pBitmap, &hBitmap, 0);
