...
/* $Id: p_gif.c,v 1.11.2.4 2002/01/26 18:32:18 tm Exp $
 *
    ...
    int intparam)
{
	...
	"GIF images not supported in this configuration");
...
#define PDF_STRING_89a	"\070\071\141"
	...
	if (c == PDF_EXCLAM) { 	/* Extension */
		...
		if (p->debug['i']) {
		    pdf_error(p, PDF_NonfatalError,
			"Read error on extension code in GIF file '%s'",
			filename);
		}
		return -1;
	    }
	...
	image->components	= 1;
	image->bpc		= 8;
	image->width		= (float) LM_to_uint(buf[4],buf[5]);
	image->height		= (float) LM_to_uint(buf[6],buf[7]);
...
#define INTERLACE		0x40
	if (BitSet(buf[8], INTERLACE)) {
	    ...
	    if (p->debug['i']) {
		pdf_error(p, PDF_NonfatalError,
		    "Interlaced GIF image '%s' not supported", filename);
	    }
	    return -1;
	}
	...
	*/
	...
	if (buf[0] != 8) {
	    ...
	    if (p->debug['i']) {
		pdf_error(p, PDF_NonfatalError,
	"LZW minimum code size other than 8 bit not supported in GIF file '%s'",
		    filename);
	    }
	    return -1;
	}
