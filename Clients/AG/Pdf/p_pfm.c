...
/* $Id: p_pfm.c,v 1.10.2.4 2002/01/26 18:32:18 tm Exp $
 *
...
#define PFM_DWORD(offset) \
	((unsigned long) ((PFM_WORD(offset+2)<<16) + PFM_WORD(offset)))
...
#define PDF_FIXED_PITCH		0x01  /* Fixed width font -- unused */
...
static pdf_bool
pdf_parse_pfm(PDF *p, FILE *fp, pdf_font *font, const char *filename)
{
    size_t length;
    long file_length;
    unsigned char *pfm;
	...
	ext_dfExtentTable+2 * (header_dfLastChar-header_dfFirstChar)+1 >length){
	    p->free(p, pfm);
	    pdf_error(p, PDF_RuntimeError, "Corrupt PFM file '%s'", filename);
    }
    ...
    const char *filename)
{
    ...
    /* Read PFM metrics */
    if (pdf_parse_pfm(p, pfmfile, font, filename) == pdf_false) {
	fclose(pfmfile);
