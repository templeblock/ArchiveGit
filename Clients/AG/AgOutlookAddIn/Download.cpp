...
#define MESSAGE_BOX_TITLE "Creata Mail"
...
void CDownload::AddFile(CString& strSrcURL, CString& strDstPath, int iData)
{
	...
	//j Temporary
	if (m_iAppType != CLIENT_TYPE_IE && strSrcPath.Find("stage.") >= 0)
...
bool CDownload::Next()
{
	...
	if (m_iAppType == CLIENT_TYPE_EXPRESS)
		CBindStatusCallback2<COEAddin>::Download(CComBSTR(strSrcURL), this, m_pClientSite);
	else if (m_iAppType == CLIENT_TYPE_OUTLOOK)
		CBindStatusCallback2<CMsAddIn>::Download(CComBSTR(strSrcURL), this, m_pClientSite);
	else if (m_iAppType == CLIENT_TYPE_IE)
		CBindStatusCallback2<CJMExplorerBand>::Download(CComBSTR(strSrcURL), this, m_pClientSite);
	else if (m_iAppType == CLIENT_TYPE_AOL)
		CBindStatusCallback2<CAOLAddin, BINDF_ASYNCHRONOUS | BINDF_ASYNCSTORAGE | BINDF_GETNEWESTVERSION /*| BINDF_NOWRITECACHE *//*| BINDF_PULLDATA*/>::Download(CComBSTR(strSrcURL), this, m_pClientSite);
	else if (m_iAppType == CLIENT_TYPE_UPDATE)
		CBindStatusCallback2<CUpdateObj>::Download(CComBSTR(strSrcURL), this, m_pClientSite);
