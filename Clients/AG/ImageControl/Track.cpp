...
void CTrack::ZeroData()
{
	...
	m_BoundViewRect.SetRectEmpty();
	m_StartRect.SetRectEmpty();
...
//////////////////////////////////////////////////////////////////////
void CTrack::Init(CAGDC* pDC, int iWhatCanDo, DRAWPROC lpDrawProc, void* pData, CRect& DestRect, CRect& CropRect, CRect& PageViewRect, CMatrix& ViewToDeviceMatrix, CMatrix& Matrix, CGrid& Grid)
{
	ZeroData();
	...
	m_Matrix = Matrix;
	m_Grid = Grid;
	m_PageViewRect = PageViewRect;

	if (m_iWhatCanDo & TR_BOUNDTOSYMBOL)
	{
		if (!CropRect.IsRectEmpty())
			m_Matrix.StretchToFit(CropRect, DestRect);

		m_BoundRect = DestRect;
		m_OrigMatrix.Transform(m_BoundRect);
		m_BoundViewRect = m_BoundRect;
		m_ViewToDeviceMatrix.Transform(m_BoundViewRect);
	}
	else
		m_BoundViewRect = m_PageViewRect;

	m_Distort.Rect = DestRect;
...
void CTrack::Mode(int iWhatCanDo, bool fDisplay)
{
	...
	m_bDrawBars			= !!(iWhatCanDo & TR_DRAWBARS);
	m_bDisabled			= !!(iWhatCanDo == 0);
...
//////////////////////////////////////////////////////////////////////
void CTrack::GetExcessRect(CRect* pRect)
{
	if (!pRect)
		return;

	pRect->UnionRect(&m_hHandleRects[H_ROTATE], &m_hHandleRects[H_CENTER]);
	pRect->InflateRect(H_SIZE, H_SIZE);
}

//////////////////////////////////////////////////////////////////////
CMatrix CTrack::GetChangeMatrix()
{
	CMatrix ChangeMatrix = m_Matrix * m_OrigMatrix.Inverse();
	return ChangeMatrix;
}
...
bool CTrack::SetCursor(int x, int y)
{
	...
	return SetCursor(x, y, true/*bInsideMoveAllowed*/, bPtInBorder, bPtInHandle);
}

//////////////////////////////////////////////////////////////////////
bool CTrack::PtInBorder(CPoint pt)
{
	CMatrix Matrix = m_Matrix * m_ViewToDeviceMatrix;
...
bool CTrack::Move(int x, int y)
{
	...
	x = bound(x, m_BoundViewRect.left, m_BoundViewRect.right);
	y = bound(y, m_BoundViewRect.top, m_BoundViewRect.bottom);
...
void CTrack::Delta(int dx, int dy)
{
	...
	CMatrix CombinedMatrix = m_Matrix * m_ViewToDeviceMatrix;
...
void CTrack::Highlight(bool bOn, HDC hDC, bool bHash)
{
	...
	m_pAGDC->SetClipRect(rClipRect, false/*bCombine*/);
...
//////////////////////////////////////////////////////////////////////
void CTrack::DrawLine(HDC hDC, int x1, int y1, int x2, int y2, bool bOn)
{
...
{
	HBITMAP hBitmap = NULL; //j ::LoadBitmap(_AtlBaseModule.GetResourceInstance(), MAKEINTRESOURCE(IDB_HATCH_BORDER_1));
	LOGBRUSH lb;
...
//////////////////////////////////////////////////////////////////////
void CTrack::DrawEllipse(HDC hDC, CRect* pRect, bool bOn)
{
...
//////////////////////////////////////////////////////////////////////
void CTrack::DrawRect(HDC hDC, CRect* pRect, bool bOn)
{
	int OldROP = ::SetROP2(hDC, R2_NOT);
	CRect rect(pRect->left, pRect->top, pRect->right+1, pRect->bottom+1);
...
//////////////////////////////////////////////////////////////////////
void CTrack::IntersectSegments(CPoint A0, CPoint A1, CPoint B0, CPoint B1, CPoint* lpPt)
