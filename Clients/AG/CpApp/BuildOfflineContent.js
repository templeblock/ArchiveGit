...
function WriteXML(fso, FileName, PRODUCT_NUMBER, REPORT_TYPE, TITLE, PRODUCT_SEQUENCE, PATHNUMBER, SMALL_X, SMALL_Y, LARGE_X, LARGE_Y, INSIDEVERSE)
{
	try
	{
		if (!fso.FileExists(FileName))
		{
			...
			tso.WriteLine('<!--Copyright 2005 AmericanGreetings.com-->');
			tso.WriteLine('<thumbs title="Category ' + PATHNUMBER + '">');
			tso.WriteLine('	<custom-parameters>');
			tso.WriteLine('		<param name="root-directory" value=".." />');
			tso.WriteLine('	</custom-parameters>');
		...
		{ // Holiday Cards
			...
			'<br/><br/>' + 
			'<span style=\'color:#1A4C8F;\'><b>Printing Borderless Designs:</b></span>  ' + 
			'Click on the Print button, click the Preferences button, and within your specific printer driver, find and check the Borderless Printing option. Paper Size - Check in print preferences to see if you have a 4x8 Borderless Paper size option. If yes, then select it. If not, select the Borderless Printing checkbox. (Note: not all printers support borderless printing.  See <a href=\'#\' onclick=\'Help(0);\' >Help</a> for details).<br/><br/>' +
			'<b>Tips:</b>  ' + 
			'Print your cards on plain paper first to check the alignment. spelling. and overall look of the project. <br/><br/>';
		}
		else
...
function ProcessCSV(FileName)
{
	...
	var ContentFolder = RootFolder + 'html';
	...
	var ThumbslistFolder = ContentFolder + '\\paths';
	if (!DeleteFolder(fso, ThumbslistFolder))
		return;
	if (!MakeFolder(fso, ThumbslistFolder))
		return;

	var ProductFolder = ContentFolder + '\\content';
