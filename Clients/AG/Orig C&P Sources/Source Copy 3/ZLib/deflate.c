	...
	int length;
{
	...
	{
		Trace((str, " start %u, match %u, length %d\n", start, match, length));
		do { Trace((str, "%c%c", s->window[match++], s->window[start++])); } while (--length != 0);
		Assert(0, "invalid match");
	}

	Tracevv((str, "\\[%d,%d]", start-match, length));
	#ifdef DEBUG_EXTRA
	...
	int flush;
{
	...
	for (;;) {
...
#endif
		{
			...
			Tracevv((str, "%c", s->window[s->strstart]));
	...
	int flush;
{
	...
	for (;;) {
		...
		if (s->prev_length >= MIN_MATCH && s->match_length <= s->prev_length) {
			 ...
			 */
			Tracevv((str, "%c", s->window[s->strstart-1]));
