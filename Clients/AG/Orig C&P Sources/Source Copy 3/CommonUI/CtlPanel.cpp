...
{
	HKEY hKey;
	if (::RegOpenKeyEx(HKEY_LOCAL_MACHINE, DUPLEX_SUBKEY, 0, KEY_WRITE, &hKey) == ERROR_SUCCESS)
	{
...
{
	HKEY hKey;
	if (::RegOpenKeyEx(HKEY_LOCAL_MACHINE, DUPLEX_SUBKEY, 0, KEY_QUERY_VALUE, &hKey) != ERROR_SUCCESS)
		return false;
	...
	bool bFound = false;
	if (::RegQueryValueEx(hKey, pszDevice, 0, &dwType, (BYTE*)szDuplex, &dwSize) == ERROR_SUCCESS)
	{
		if (dwType == REG_SZ)
		{
...
static void SetRegistryDuplex(const char* pszDevice, int nDuplex)
{
	HKEY hKey;
	DWORD dw;
	if (::RegCreateKeyEx(HKEY_LOCAL_MACHINE, DUPLEX_SUBKEY, 0, REG_NONE, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, &dw) != ERROR_SUCCESS)
		return;
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
static UINT APIENTRY PrintHookProc(HWND hWnd, UINT msg, UINT wParam, LONG lParam)
{
	...
	switch (msg)
	{
		case WM_INITDIALOG:
		{
			...
			if (*pbSingleFold)
				::SendDlgItemMessage(hWnd, IDC_SINGLEFOLD, BM_SETCHECK, BST_CHECKED, 0);
			else
				::SendDlgItemMessage(hWnd, IDC_QUARTERFOLD, BM_SETCHECK, BST_CHECKED, 0);

			int n = ::SendDlgItemMessage(hWnd, cmb4, CB_GETCURSEL, 0, 0);
			if (n >= 0)
			{
				char szDevice[100];
				::SendDlgItemMessage(hWnd, cmb4, CB_GETLBTEXT, n, (LPARAM)szDevice);

				int nDuplex;
				::ShowWindow(::GetDlgItem(hWnd, IDC_DBLSIDE), GetRegistryDuplex(szDevice, nDuplex));
			}
			break;
		}
		...
		case WM_COMMAND:
		{
			switch (LOWORD(wParam))
			{
				case IDOK:
				{
					if (::SendDlgItemMessage(hWnd, IDC_SINGLEFOLD, BM_GETCHECK, 0, 0) == BST_CHECKED)
						*pbSingleFold = true;
					else
						*pbSingleFold = false;
					...
					if (::IsWindowVisible(::GetDlgItem(hWnd, IDC_DBLSIDE)) &&
					    ...
					    ::SendDlgItemMessage(hWnd, IDC_DBLSIDE, BM_GETCHECK, 0, 0) == BST_CHECKED)
					{
						int n = ::SendDlgItemMessage(hWnd, cmb4, CB_GETCURSEL, 0, 0);
						if (n >= 0)
						{
...
HWND CCtlPanel::Create(HWND hWndParent)
{
	...
	if (nLogPixelsY > 96)
	{
		...
		HRSRC hRsrc = ::FindResource(hInst, MAKEINTRESOURCE(CCtlPanel::IDD), RT_DIALOG);
		if (hRsrc)
		{
			...
			if (m_hDlg)
			{
				...
				hWnd = ::CreateDialogIndirectParam(hInst, (DLGTEMPLATE*)pNew, hWndParent, (DLGPROC)CCtlPanel::StartDialogProc, NULL);
	...
	{
		hWnd = ::CreateDialogParam(_Module.GetResourceInstance(), MAKEINTRESOURCE(CCtlPanel::IDD), hWndParent, (DLGPROC)CCtlPanel::StartDialogProc, NULL);
	}
...
LRESULT CCtlPanel::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	{
		char szPtSize[10];
		SendDlgItemMessage(IDC_PTSIZE, CB_ADDSTRING, 0, (LPARAM)_itoa(i, szPtSize, 10));
	}
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtlPanel::OnCtlColorDlg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	bHandled = true;
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtlPanel::OnCtlColorStatic(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	bHandled = true;
	SetBkMode((HDC)wParam, TRANSPARENT);
	return (LRESULT)GetStockObject(NULL_BRUSH);
...
LRESULT CCtlPanel::OnDrawItem(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (!lpdis || (idCtl != IDC_COLOR && idCtl != IDC_FONT))
		return false;
	...
	if (idCtl == IDC_COLOR)
	{
		...
		{
			COLORREF clr = (COLORREF)::SendMessage(lpdis->hwndItem, CB_GETITEMDATA, lpdis->itemID, 0);
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtlPanel::OnPage(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	int nPage = wID - IDC_PAGE1;
	if (m_pAGDoc && nPage >= 0 && nPage <= 3)
	{
		m_pAGDoc->SetCurrentPage(m_PageMap[nPage]);
		m_pMainWnd->NewPage();
	}
...
void CCtlPanel::SetDoc(CAGDoc* pAGDoc)
{
	...
	for (int nPage = 1, nMap = 0; nPage <= nPages; nPage++)
	{
		...
		if (pPage && (!pPage->IsEmpty() || (bCardType && nPage == 4)))
		{
...
void CCtlPanel::UpdateControls(const CAGText* pText)
{
	...
	if (bFont)
	{
		...
		{
			int index = SendDlgItemMessage(IDC_FONT, CB_FINDSTRINGEXACT, (WPARAM)-1, (LPARAM)FontArray[i].szFullName);
	...
	if (bPtSize)
	{
		...
		int index = SendDlgItemMessage(IDC_PTSIZE, CB_FINDSTRINGEXACT, (WPARAM)-1, (LPARAM)_itoa(nPtSize, szPtSize, 10));
		if (index == CB_ERR)
		{
			...
			{
				char szTemp[20];
				SendDlgItemMessage(IDC_PTSIZE, CB_GETLBTEXT, i, (LPARAM)szTemp);
				if (nPtSize < atoi(szTemp))
				{
					index = SendDlgItemMessage(IDC_PTSIZE, CB_INSERTSTRING, i, (LPARAM)szPtSize);
					break;
				}
			}
			if (i >= nItems)
			{
				index = SendDlgItemMessage(IDC_PTSIZE, CB_ADDSTRING, 0, (LPARAM)szPtSize);
			}
		}
	...
	if (bColor)
	{
		...
		for (int i = 0; i < nItems; i++)
		{
			COLORREF Color = (COLORREF)SendDlgItemMessage(IDC_COLOR, CB_GETITEMDATA, i, 0);
			if (Color == SpecColor)
				break;
		}
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtlPanel::OnHorzJust(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtlPanel::OnPtSize(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	if (wNotifyCode != CBN_SELCHANGE)
	{
		if (wNotifyCode == CBN_CLOSEUP)
	...
	if (nItem >= 0)
	{
		...
		{
			char szPtSize[20];
			SendDlgItemMessage(IDC_PTSIZE, CB_GETLBTEXT, nItem, (LPARAM)szPtSize);
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtlPanel::OnColor(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	if (wNotifyCode != CBN_SELCHANGE)
	{
		if (wNotifyCode == CBN_CLOSEUP)
	...
	if (nItem >= 0)
	{
		...
		if (pText)
		{
			COLORREF Color = (COLORREF)SendDlgItemMessage(IDC_COLOR, CB_GETITEMDATA, nItem, 0);
...
void CCtlPanel::SetFonts()
{
	...
	for (int i = 0; i < nFonts; i++)
	{
		if (SendDlgItemMessage(IDC_FONT, CB_FINDSTRINGEXACT, (WPARAM)-1, (LPARAM)FontArray[i].szFullName) == CB_ERR)
		{
			int index = SendDlgItemMessage(IDC_FONT, CB_ADDSTRING, 0, (LPARAM)FontArray[i].szFullName);
