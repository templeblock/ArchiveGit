...
void CCtp::CreateBackPage()
{
	...
	pAGSymText->SetText(szCopyright, lstrlen(szCopyright), 1, &CopyrightSpec, &nSpecOffset);
	pAGLayer->AppendSymbol(pAGSymText);
}
...
void CCtp::DrawEditRect(CAGDC* pDC)
{
	...
	HPEN hOldPen = (HPEN) ::SelectObject(pDC->GetHDC(), ::GetStockObject(BLACK_PEN));
...
void CCtp::FontData(const char* pszFontFile, BYTE* pBytes, DWORD dwLen)
{
	...
	for (int i = 0; i < nDownload; i++)
	{
		if (lstrcmpi(pszFontFile, m_FontDownloadArray[i].szFontFile))
			continue;
...
{
	HRSRC hResource = ::FindResource(_Module.GetResourceInstance(), MAKEINTRESOURCE(nID), "AGIMAGE");
	if (!hResource)
		return NULL;

	HGLOBAL hSymImage = ::LoadResource(_Module.GetResourceInstance(), hResource);
	if (!hSymImage)
		return NULL;

	CAGSymImage* pAGSymImage = new CAGSymImage();
	if (!pAGSymImage)
		return NULL;
	
	istrstream input((char*)::LockResource(hSymImage), ::SizeofResource(_Module.GetResourceInstance(), hResource));
...
LRESULT CCtp::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	g_hHook = ::SetWindowsHookEx(WH_KEYBOARD, reinterpret_cast<HOOKPROC> (CtpKeyboardProc), NULL, GetCurrentThreadId());
	g_pThis = this;
...
HRESULT CCtp::OnDraw(ATL_DRAWINFO &di)
{
	...
	else
	{
		...
		::DrawText(di.hdcDraw, "Preparing to edit card.  Please Wait...", -1, &r, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
	}
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtp::OnChar(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	UINT nChar = wParam;
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtp::OnKeyDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	UINT nChar = wParam;
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtp::OnKeyUp(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	UINT nChar = wParam;
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtp::OnLButtonDblClk(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if (m_pText && m_pText->IsEditing())
	{
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtp::OnLButtonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if (!m_pAGDoc)
		return 0;
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtp::OnLButtonUp(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if (m_pText && m_pText->IsEditing())
	{
		POINT pt = { LOWORD(lParam), HIWORD(lParam) };
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtp::OnMouseMove(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if (m_pText && m_pText->IsEditing())
	{
		POINT pt = { LOWORD(lParam), HIWORD(lParam) };
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtp::OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if (m_pText && m_pText->IsEditing())
	{
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtp::OnSetFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if (m_pText && m_pText->IsEditing())
	{
		if (m_pText->GetSelection()->IsSliverCursor())
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
LRESULT CCtp::OnTimer(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
