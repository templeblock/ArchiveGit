...
{
	return ((RGBQUAD*)(((BYTE*) pbih) + pbih->biSize + (pbih->biCompression == BI_BITFIELDS ? sizeof(DWORD) * 3 : 0)));
}

inline DWORD DibNumColors(const BITMAPINFOHEADER* pbih)
{
	return ((pbih->biClrUsed == 0 && pbih->biBitCount <= 8) ? (1 << pbih->biBitCount) : pbih->biClrUsed);
}
...
{
	return (pbih->biSizeImage != 0 ? pbih->biSizeImage : DibWidthBytes(pbih) * pbih->biHeight);
}

inline DWORD DibSize(const BITMAPINFOHEADER* pbih)
{
	return (pbih->biSize + DibSizeImage(pbih) + DibPaletteSize(pbih) + (pbih->biCompression == BI_BITFIELDS ? sizeof(DWORD) * 3 : 0));
}
