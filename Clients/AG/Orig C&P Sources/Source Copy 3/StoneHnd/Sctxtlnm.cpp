/****************************************************************************
	...
	$Header: /Projects/Toolbox/ct/Sctxtlnm.cpp 6	 5/30/97 8:45a Wmanis $
...
{	
	...
	fDrawOrigin 	= fCurPos;
	fDropCapFlag	= fOffset == 0 && fChRec->flags.IsDropCap();
...
/* ==================================================================== */
/* call the spec sub-system drop cap routine and make sense of it */

Bool DCCompute( DropCapInfo&	dcInfo,
				TypeSpec&		pspec,
				TypeSpec&		charspec,
				MicroPoint		lineOrg,
				MicroPoint	baseline,
				UCS2			ch )
{
	if ( !ch || !CTIsDropCapable( ch ) )
		return false;

	scRLURect	rluRect;
	REAL		ptConv,
				setConv;
	DCPosition& dcPos = dcInfo.dcPosition;
	
	if ( TSdropCap( pspec, charspec, dcPos, ch ) ) {
		FIgetRLUExtents( scCachedStyle::GetParaStyle().GetSpec(), ch, rluRect );

		ptConv	= (REAL)dcPos.dcPtSize / scBaseRLUsystem;
		setConv = (REAL)dcPos.dcSetSize / scBaseRLUsystem;
			
			// compute character extents in micro points
		dcInfo.dcMinX = scRoundMP( (REAL)setConv * rluRect.rluLeft );
		dcInfo.dcMaxX = scRoundMP( (REAL)setConv * rluRect.rluRight );
			
			// correct the coordinate systems
		dcInfo.dcMinY = scRoundMP( (REAL)ptConv * -rluRect.rluTop );
		dcInfo.dcMaxY = scRoundMP( (REAL)ptConv *	-rluRect.rluBottom);

			// this value is the horz offset for the character from 
			// the recomputed origin 
		dcInfo.dcHChange = dcInfo.dcMaxX + dcPos.dcHBorder; 	 

			// until a baseline exceeds this value reoring lines
		dcInfo.dcVMax = baseline + dcPos.dcVOffset + dcInfo.dcMaxY + dcPos.dcVBorder;
		
			// the recomputed line orgin
		dcInfo.dcLineOrgChange = lineOrg + dcPos.dcHOffset + dcInfo.dcMaxX + dcPos.dcHBorder;

			// now transform character extents into column coordinates
		dcInfo.dcMinX += ( lineOrg + dcPos.dcHOffset );
		dcInfo.dcMaxX += ( lineOrg + dcPos.dcHOffset ); 				  

		dcInfo.dcMinY += ( baseline + dcPos.dcVOffset );
		dcInfo.dcMaxY += ( baseline + dcPos.dcVOffset );
...
/* ==================================================================== */
// draw the drop characters

size_t scDrawLine::DrawDropChar( )
{
	DropCapInfo 	dcInfo;
	scGlyphInfo 	charInfo;
	short			chCount = 1;
	scMuPoint		origin( fCurPos );

	DCCompute( dcInfo, 
			   scCachedStyle::GetParaStyle().GetSpec(), 
			   scCachedStyle::GetCurrentCache().GetSpec(), 
			   0L, 0L, fChRec->character );

	fChArray[0].fGlyphID	= fChRec->character;

#ifdef _RUBI_SUPPORT
	fChArray[0].flags.ClearFlags( );
#endif

	fChArray[0].vEscapement = MPtoGS( dcInfo.dcMaxY );
	fChArray[0].hEscapement = MPtoGS( dcInfo.dcMaxX );
	
	origin.Translate( -dcInfo.dcHChange, ( dcInfo.dcPosition.dcVOffset - scCachedStyle::GetParaStyle().GetBaseline() ) );

	charInfo.fontname		= dcInfo.dcPosition.dcFont;
	charInfo.emphasis_		= scCachedStyle::GetParaStyle().GetDropEmphasis();
	charInfo.fontRender 	= scCachedStyle::GetParaStyle().GetDropRender();
	charInfo.typespec		= scCachedStyle::GetParaStyle().GetSpec();
	charInfo.color			= dcInfo.dcPosition.dcColor;
	charInfo.pointsize		= dcInfo.dcPosition.dcPtSize;
	charInfo.setsize		= dcInfo.dcPosition.dcSetSize;
	charInfo.oblique		= scCachedStyle::GetParaStyle().GetHorzOblique();
	charInfo.rotation		= 0;

	if ( fFlowDir.IsHorizontal() )
		charInfo.fBaseline		= kRomanBaseline;
	else
		charInfo.fBaseline		= kCenterBaseline;

	charInfo.flowDir		= fFlowDir;
	
	APPDrawString( fDrawContext, fChArray, chCount, origin.x, origin.y, charInfo );
...
void scDrawLine::SetCharData(  )
{
	...
	fGlyphInfo.oblique		= scCachedStyle::GetCurrentCache().GetHorzOblique();
...
static void getannotation( stUnivString&	ustr,
							APPColumn		col,
							scStream*		stream,
							uint8			id,
							TypeSpec&		spec )
...
void scDrawLine::DrawField()
{
	...
	getannotation( ustr, 
				   txl_.GetColumn()->GetAPPName(),
				   txl_.GetColumn()->GetStream(),				   
				   fChRec->flags.GetField(),
				   fSpec );
...
void scDrawLine::Draw()
{
	...
	SetCharData( );
		...
		// draw the drop char if there is one	
	if ( !fOffset && scCachedStyle::GetParaStyle().GetNumbered() )
		DrawBullet();

	if ( fDropCapFlag ) 
		fOffset = DrawDropChar( );
...
#endif
		{  
			// if we are using small caps and there is a case change dump 
			// the buffer and set up a new text graphics context
			if ( cachedStyle.GetSmallCaps() ) {
				if ( CTIsUpperCase( fChRec->character ) )
					curCase = upperCase;
				else if ( CTIsLowerCase( fChRec->character ) )
					curCase = lowerCase;
				else
					curCase = noCase;
				if ( curCase != prevCase )
					SetCaseCharData( curCase );
			}
			...
			switch ( fChRec->character ) {
				...
				default:
					if (ch)
					{
						chRec.Set( CMctToAPP( cachedStyle.GetCorrectedGlyph( ch ) ), fChRec->flags, fChRec->escapement );
						 ...
						 Bool&				endOfLine )
{
	...
	DropCapInfo 	dcInfo;
	if ( offset == 0 && 
		 scCachedStyle::GetParaStyle().GetDropCap() &&
		 ::DCCompute( dcInfo,
					  scCachedStyle::GetParaStyle().GetSpec(), 
					  scCachedStyle::GetCurrentCache().GetSpec(),									   
					  0L, 0L, chRec->character ) ) {

		if ( ( curPos.x - ( dcInfo.dcLineOrgChange / 2	) ) < hitPt.x ) {
				/* if the selection point is greater than the computed line
				 * origin continue searching down the line
				 */
			offset++, chRec++;
		}
		else {
				/* the drop char has been selected */
			curPos.x -= dcInfo.dcLineOrgChange;
			searchDone = true;
		}
	}
							   ...
							   eContentMovement cursDirection )
{
	...
	if ( offset == 0 && scCachedStyle::GetParaStyle().GetDropCap() && chRec->flags.IsDropCap() ) {
		DropCapInfo 	dcInfo;
		if ( chOffset == 0 &&  ::DCCompute( dcInfo,
										   scCachedStyle::GetParaStyle().GetSpec(), 
										   scCachedStyle::GetCurrentCache().GetSpec(),									   
										   0L, 0L,	chRec->character ) ) {
			chEscapement = dcInfo.dcHChange + dcInfo.dcPosition.dcHOffset - dcInfo.dcMinX;
