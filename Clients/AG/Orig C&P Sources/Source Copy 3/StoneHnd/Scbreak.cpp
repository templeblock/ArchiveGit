									...
									Hyphen* 	hyphens )
{
	...
	if ( HYFWord( hyphWord, hyphArray ) ) {
		...
		for ( j = 0, charCount = 1; j < hLen; charCount++, theChar++ ) {
			...
			if ( ch == hyphWord[j] ) {
				if ( hyphArray[j] && j >= ( scCachedStyle::GetCurrentCache().GetPreHyph() - 1 ) && j < ( hLen - scCachedStyle::GetCurrentCache().GetPostHyph() ) ) {
	...
	if ( gbrS.cB.spaceCount ) {
		...
		if ( gbrS.justSpace < 0 ) {
			...
			if ( gbrS.letterSpaceAdj < scCachedStyle::GetCurrentCache().GetMinLSP() ) {
				...
				adjustableSpace = MPtoGS( glueSpace - scRoundMP( (REAL)gbrS.letterSpaceAdj * lspSpaces ) );
