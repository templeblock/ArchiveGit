							...
							eDoVJ		flag )
{
	...
	if ( flag == eVJCenter ) {
		...
		if ( ( txl = col->GetFirstline() ) != NULL ) {
			...
			FIgetRLUFontExtents( scCachedStyle::GetCurrentCache().GetSpec(), capHiteRlu, d1, d2, d3, rect );

			capHeight = scRoundMP( (REAL)scCachedStyle::GetCurrentCache().GetPtSize() / scBaseRLUsystem * capHiteRlu );
