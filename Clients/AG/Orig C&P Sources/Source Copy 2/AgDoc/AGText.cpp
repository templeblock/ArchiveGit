...
void FIgetRLUFontExtents(const scFontRender &fp, RLU &capHite, RLU &xHite,
							RLU &ascenderHite, RLU &descenderDepth,
							scRLURect &maxFont)
{
	capHite = 0;
	xHite = 0;
...
//--------------------------------------------------------------------------//
//--------------------------------------------------------------------------//
void FIgetDEVFontExtents(const scFontRender &fp, MicroPoint &capHite,
						MicroPoint &xHite, MicroPoint &ascenderHite,
						MicroPoint &descenderDepth, scXRect &maxFont)
{
	CAGSpec *pSpec = (CAGSpec *)fp.ptr();
	CAGIC TextIC("DISPLAY");
	TextIC.SetTransformMode(false);
