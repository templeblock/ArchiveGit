/****************************************************************************
	...
	$Header: /Projects/Toolbox/ct/Sctypes.h 2	  5/30/97 8:45a Wmanis $

	Contains:	This contains the basic preprocessor defines to
				account for platfrom behavior as well as the data
				types that are commonly used by the client and the
				toolbox.
...
/* ==================================================================== */
	...
	// these are here to deal with the huge pointer problem in 16 bit code
#define SCmemset	memset
#define SCmemmove	memmove
#define SCmemcpy	memcpy
#define SCmemcmp	memcmp
	...
	#if defined( _MSC_VER )
		#define MSVCBUG_1A		// used for fix in scexcept.h

		#pragma warning(disable:4121)		// disable - wingdi makes noise about this
		#pragma warning(disable:4711)		// disable - the compiler may inline this function/method
//		#pragma warning(disable:4244)		// disable - int conversion
		#pragma warning(disable:4100)		// disable - unused formal parameters
	#endif

		#include <windows.h>
		#include <tchar.h>

	#ifdef _CPPUNWIND			// C++ exception handling defined
		#define useCPLUSEXCEPTIONS
	#endif
	...
	#define scStrcpy		_tcscpy
	#define scStrcat		_tcscat
	#define scStrcmp		_tcscmp
	...
	#ifdef scIMPL_DLL	
		#ifndef scCLIENT_IMPLEMENTOR
			#define scIMPL_EXPORT __declspec(dllexport)
			#define scIMPL_IMPORT __declspec(dllimport)
		#else
			#define scIMPL_IMPORT __declspec(dllexport)
			#define scIMPL_EXPORT __declspec(dllimport)
		#endif
	#else
		#define scIMPL_EXPORT 
		#define scIMPL_IMPORT 
	#endif
	...
	// GMNOTE: is the following correct??? - no since there is
	// no native unicode support on the mac now, you may want
	// to just define the _tcsxxx as the strxxx funcs in string.h
	#define scChar			char
	#define scString		(char *)
	#define scStrcpy		_tcscpy
	#define scStrcat		_tcscat
	#define scStrcmp		_tcscmp
	#define scStrlen		_tcslen
...
/* ==================================================================== */
...
#ifdef useSMARTHEAP
	#undef	useSMARTHEAP
	#define useSMARTHEAP 1

	#define IFSMARTHEAP( x )	x
#else
	#define IFSMARTHEAP( x )
#endif
	...
	typedef MEM_HANDLE scMemHandle;
	inline void*	scMemDeref( scMemHandle h ) 	
						{ 
							return MEM_REFERENCE( (h) ); 
						}
...
// @type HRgnHandle | A handle to a device independent region.
typedef scMemHandle 		HRgnHandle;


/* ==================================================================== */
/* ==================================================================== */

#define PUBLISHED	public			// these are C++ class member functions
									// that are safe for the client to use
...
typedef uchar		Char;
typedef ushort		UCS2;				// @type UCS2 | A 16 bit Unicode character.
typedef ulong		UCS4;				// @type UCS4 | A 32 bit Unicode character.
...
#ifndef SCMACINTOSH
	scTicks SCTickCount( void );
	void	SCSysBeep( long );
#else
	#define SCTickCount()		((scTicks)TickCount())
	#define SCSysBeep( x )		SysBeep(x)
#endif
...
/* ==================================================================== */
...
#if defined( USE_TWIPS )

		// this will use twips * 16
		// we will use a corrupted point 1/72 inch
	extern long scLogUnitsPerPixel;
	#define one_point				(scLogUnitsPerPixel)
	...
	inline int MPToTwips( MicroPoint mp )
	{
		...
		// a printers' point - 1/72.27 of an inch
	#define one_point				65291L
	#define one_didot				61506L			/* this needs to be checked */
	#define one_inch				4718592L
	#define one_millimeter			185771L
...
#if SCDEBUG > 1
#define scDebugAssert( exp )	scFuncAssert( (int)(exp), scString( #exp ), scString( __FILE__ ), __LINE__ )
#else
...
//	Replacement for __FILE__ macro that won't use heap space

#ifdef SCWINDOWS
	static const char *SC_THIS_FILE = __FILE__;
#else
	#define SC_THIS_FILE __FILE__
#endif
...
#if SCDEBUG > 1 
inline void TestStatus( status stat )
{
	if ( stat != scSuccess ) 
		SCDebugBreak();
}
#else
void TestStatus( status stat );
#endif
...
#if defined( SCWINDOWS )
typedef HANDLE	APPSYSHandle;
#elif defined( SCMACINTOSH )
typedef Handle	APPSYSHandle;
#endif


struct SystemMemoryObject {
					SystemMemoryObject();
					~SystemMemoryObject();
...
#if defined( scUSE_MATRIX )
			
class scMatrix : public scMuPoint {
public:
			scMatrix( MicroPoint xx = 0, MicroPoint yy = 0 ) :
					scMuPoint( xx, yy )
					{ fMat[0] = 1.0, fMat[1] = 0.0, fMat[2] = 0.0, fMat[3] = 1.0; }
					
			scMatrix( const scMatrix& );
			
	scMatrix&	operator=( const scMatrix& );
	scMatrix	operator*( const scMatrix& ) const;
			
		// this only tests the matrix and not any of its superclass stuff
	int 	operator==( const scMatrix& ) const;
	int 	operator!=( const scMatrix& ) const;
	
		// tests equality of kernal matrix only, not the translation component
	int 	IsKernalEqual( const scMatrix& ) const;
	
	void	XFormPt( scMuPoint& pt ) const;
	
	Bool	IsRotated( void ) const;
	REAL	GetHorzScale( void ) const;
	REAL	GetVertScale( void ) const;

	void	Invert( void );
	void	SetRotation( REAL );
	void	SetShear( REAL, REAL );
...
/* ==================================================================== */
// drop cap position information
// Used in callback TSdropCap, the values are to be filled in by
// the client. It is at the clients discretion whether to use the
// Spec Manager and the deltas for drop caps.
...
public:
	...
	short		dcNumChars;
	GlyphSize	dcPtSize,	/* drop cap character point size			*/
				dcSetSize,	/* drop cap character set size				*/
				dcHOffset,	/* horz character offset from line origin	*/
				dcVOffset,	/* vert character offset from baseline		*/
				dcHBorder,	/* horz border from right extent of char to start
							 * normal character setting
							 */
				dcVBorder;	/* vertical border from bottom extent of char
							 * to return to normal line placement
							 */
	APPFont 	dcFont;
	APPColor	dcColor;
