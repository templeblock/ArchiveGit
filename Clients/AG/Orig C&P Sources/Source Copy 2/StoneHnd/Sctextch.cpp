/****************************************************************************
	...
	$Header: /Projects/Toolbox/ct/SCTEXTCH.CPP 2	 5/30/97 8:45a Wmanis $

	Contains:

	This module handles the array of characters forming a paragraph.
	The array is stored as part of a paragraph. These routines handle
	creation, copying, deleting, editing, etc. of this array.


	Written by: Manis

	Copyright (c) 1989,1990,1991 Stonehand Inc., of Cambridge, MA.	
	All rights reserved.
...
/* 'repair' the text array, rehypehate, rekern and recompute widths */
...
//		ch =  cachedStyle->GetCorrectedGlyph( chRec->character );
		ch = chRec->character;
		if ( prevChRec )
...
MicroPoint	UnivStringWidth( stUnivString&	ustr, 
						   ...
						   long 		end )
{
	...
	if	(end >= 0L) {
		...
		ch =  chRec->character ;
		if ( ch != scEndStream && CTIsSelectable( ch ) ) {
						  ...
						  long				len )
{
	...
	SCmemmove( dstCh + offset + len,
			 dstCh + offset,
			 ( oldNumItems - offset ) * fElemSize );

	SCmemcpy( dstCh + offset, srcCh, len * fElemSize );
...
int scCharArray::Insert( const stUnivString& str,
						  int32 			  start,
						  int32 			  end )
{
	long	oldNumItems = fNumItems;
...
void scCharArray::CopyChars( CharRecordP dstCh, long offset, long len )
{
	...
	SCmemcpy( dstCh, srcCh + offset, len * sizeof( CharRecord ) );
}
							   ...
							   Bool 		textCleared )
{
	...
	if ( offset > 0 ) {
		...
		if ( chRec->flags.IsDiscHyphen() && chRec->flags.IsLineBreak() ) {
			...
			SCmemmove( chRec, chRec + 1, (GetNumItems() - offset) * sizeof(CharRecord) );
								   ...
								   Bool 		textCleared )
{
	...
	if ( offset < GetContentSize() ) {
		...
		if ( !textCleared ) {
			...
			SCmemmove( chRec, chRec + 1, (GetContentSize() - offset) * sizeof(CharRecord) );
			streamSize	= -1;
							  ...
							  Bool			textCleared )
{
	...
	if ( chRec->character == keyRec.keycode() ) {
		...
		if ( ((chRec-1)->escapement + keyRec.escapement()) == 0 ) {
				// if the resulting value is to be zero (ie no kern) remove the kern character */
			SCmemmove( chRec - 1, chRec, (GetNumItems() - offset + 1) * sizeof(CharRecord) );
			cursorSize = -1;
			streamSize = -1;
		}
		else
							   ...
							   Bool 		textCleared )
{
	...
	if ( keyRec.type() == scKeyRecord::overstrike && chRec->character ) {
		...
		SCmemmove( chRec + 1, chRec, (GetNumItems() - offset) * sizeof(CharRecord) );
		streamSize	= 1;
							 ...
							 int			charset )
{
	...
	try {
		for ( ; chRec->character; chRec++ ) {
			...
			scAPPWriteCharacter( ctxPtr, writeFunc, &chRec->character, nullSpec );
		}
...
int scCharArray::GetToken( stUnivString& ustr, 
						   int32			start, 
						   int32			end ) const
{
...
int scCharArray::ReplaceToken( const stUnivString& ustr,
							   int32 start,
							   int32& end )
{
...
int scCharArray::FindString( const stUnivString&	ustr,
