...
class ATL_NO_VTABLE CPlugin : 
	...
	public IDispatchImpl<IPlugin, &IID_IPlugin, &LIBID_VDCrusherLib, /*wMajor =*/ 1, /*wMinor =*/ 0>,
...
// IPlugin
	...
	STDMETHOD(UploadVideo)(BSTR bstrURL, BSTR bstrForm, BSTR bstrOptions, int bSilent, BSTR* pbstrResult);
	STDMETHOD(PrepareVideo)(BSTR bstrFilePath, BSTR bstrOptions, int bSilent, BSTR* pbstrFilePathNew);


private:
	HRESULT UploadForm(const CString& strURL, const CString& strFormDataPairs, const CString& strOptions, int bSilent, CString& strReturn);
	HGLOBAL PrepareFormData(const CString& strFormDataPairs, const CString& strOptions, int bSilent);
	HRESULT ScanForVideoLength(const CString& strFilePathLog, int bSilent);
...
public:
	HRESULT OnDraw(ATL_DRAWINFO& di)
	{
		...
		LPCTSTR pszText = _T("VDCrusher Plugin");
		TextOut(di.hdcDraw, 
			(rc.left + rc.right) / 2, 
			(rc.top + rc.bottom) / 2, 
			pszText, 
