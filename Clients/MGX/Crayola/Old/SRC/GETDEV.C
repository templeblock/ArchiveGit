...
// Static prototypes
static  LPSTR GetNextParm(LPSTR lpString);
...
SendMessage( (HWND)0xFFFF, WM_WININICHANGE, 0, (long)(LPSTR)"windows" );
...
SendMessage( (HWND)0xFFFF, WM_WININICHANGE, 0, (long)(LPSTR)"devices" );
}
...
typedef LRESULT (CALLBACK* DEVICEPROC)(HWND, HINSTANCE, LPSTR, LPSTR);
DEVICEPROC lpDeviceModes;
...
if (Result = (hDriver = LoadLibrary( szDriver )) > (HINSTANCE)HINSTANCE_ERROR)
	{ // replace 13L with "DEVICEMODE"
	if ( lpDeviceModes = (DEVICEPROC)GetProcAddress( hDriver, (LPSTR)13L ) )
		(*lpDeviceModes)( hWndAstral, hDriver, lpName, lpPort );
	FreeLibrary( hDriver );
	}
