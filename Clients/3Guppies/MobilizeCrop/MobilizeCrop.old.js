...
//			'http://www.facetofacesoftware.com/mobilize/images/sample.jpg', // the user's image filename
...
//			<td valign="middle" style="width:400px; height:300px;">
//				<div id="idCropDisplayContainer">
...
//				</div>
//				<br />
//				<div style="width:128px; height:160px; text-align:left; vertical-align:top; background-image:url(images/GenericPhone.jpg);">
	...
	xPhoneDisplay, yPhoneDisplay)
{
	...
	var m_dxCropDisplay = dxCropDisplay - (2 * m_iHandleSize);
	var m_dyCropDisplay = dyCropDisplay - (2 * m_iHandleSize);
	...
	function CreateElements()
	{
		...
		if (m_bStretchToFit)
		{
			fxScale = Scale(m_dxUserImage, m_dxCropDisplay);
			fyScale = Scale(m_dyUserImage, m_dyCropDisplay);
		}
		else
			fxScale = fyScale = ScaleToFit(m_dxUserImage, m_dyUserImage, m_dxCropDisplay, m_dyCropDisplay, true/*bUseSmallerFactor*/);
		...
		m_oPhoneDisplayContainer = document.getElementById('idPhoneDisplayContainer'); // Set size in UpdatePhoneDisplay()
		m_oPhoneDisplayContainer.style.left = m_xPhoneDisplay + 'px';
		m_oPhoneDisplayContainer.style.top = m_yPhoneDisplay + 'px';
		...
		var xc = Math.floor((xr + xl) / 2);
		var yt = -m_iHandleSize;
		var yb = m_dyCropDisplay;
		var yc = Math.floor((yb + yt) / 2);
	...
	function UpdatePhoneDisplay()
	{
		...
		m_oPhoneDisplayContainer.style.left = m_xPhoneDisplay + Math.floor((m_dxPhoneDisplay - dxPhoneDisplayContainer) / 2) + 'px';
		m_oPhoneDisplayContainer.style.top = m_yPhoneDisplay + Math.floor((m_dyPhoneDisplay - dyPhoneDisplayContainer) / 2) + 'px';
