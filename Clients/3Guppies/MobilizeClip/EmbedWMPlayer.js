...
function CEmbedWMPlayer(sAudioUrl, bUseObjectTagAlways, iWidth, iHeight, Version)
{
	// private
...
//j		m_bInstalled = VersionInstalled.IsNewer(Version6065);
	...
	{
		var sMimeType = 'application/x-ms-wmp'; // x-mplayer2
		var sClassId = '6BF52A52-394A-11d3-B153-00C04F79FAA6';
		var sPluginsPage = 'http://www.microsoft.com/windows/windowsmedia/download/';
		var sCodebase = 'http://activex.microsoft.com/activex/controls/mplayer/en/nsmp2inf.cab#Version=7,0,0,0';
		var sAudioUrlParamName = 'url';
	...
	function GetVersionInstalled()
	{
		try
		{
			...
			if (window.ActiveXObject)
				oPlugin = new ActiveXObject('WMPlayer.OCX');
			else
			//if (window.GeckoActiveXObject)
			//	oPlugin = new GeckoActiveXObject('WMPlayer.OCX');
			//else
			if (navigator.mimeTypes && navigator.mimeTypes.length)
			{
				var mimeType = navigator.mimeTypes['application/x-ms-wmp']; // x-mplayer2
				if (mimeType)
				{
			...
			if (oPlugin)
			{
				...
				try { sVersion = oPlugin.versionInfo; } 
				catch(e) {}
