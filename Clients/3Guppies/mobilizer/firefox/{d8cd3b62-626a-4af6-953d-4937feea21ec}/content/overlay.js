...
function Mixxer_Mobilizer()
{
	...
	this.openNewExternalMobilizerTab=function(selected)
	{
		...
		var newTab = browser;
		var tabUrl = this.base_url + 'wShowAll/true/module/OpenLocker/action/ExternalMobilize?extcmp=FFXP00330&src=ffplugin&sUrl=';
	...
	this.openNewMobilizerTab=function(url,selected)
	{
		var browser = window.getBrowser();
		var newTab = browser;
		var tabUrl = this.base_url + 'module/OpenLocker/action/Mobilize?extcmp=FFXP00330&src=ffplugin&Url=';
		tabUrl += encodeURIComponent(url);
		newTab = newTab.addTab(tabUrl);
		if ( selected )
			browser.selectedTab = newTab;
	...
	this.handleSelectedText=function()
	{
		...
		if ( t )
		{
			var url = this.base_url + 'no_border/1/module/Pager?extcmp=FFXP00330&src=ffplugin&Postfix=' + encodeURIComponent(t);
	...
	this.doObjectOverlay=function(mw)
	{
		...
		{
			var wmode = rawEmbeds[ t ].getAttribute( 'wmode' );
			if( !wmode || wmode != 'transparent' )
			{
				var par = rawEmbeds[ t ].parentNode;
				var e = rawEmbeds[ t ];
			...
			if ( url.hostname == 'www.mixxer.com' )
				return;
			this.makeObjectFrame(rawEmbeds[t], url);
		}
