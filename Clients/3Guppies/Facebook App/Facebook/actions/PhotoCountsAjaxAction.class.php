...
/**
 * PhotoCountsAjax action for the Facebook application
 *
 *
 * @author  Jim McCurdy
 */
class PhotoCountsAjaxAction extends Action
{
	/**
	...
	/**
	 * Executes the PhotoCountsAjax Action
	 *
	 * <br/><br/>
	 *
	 * <note>
	...
	function execute (&$controller, &$request, &$user)
	{
		...
		$userid = Param('user');
		if (!$userid)
			$userid = Param('id', $facebookUser); // From the profile-action link

		$arrUserids = explode(',', $userid);
		$arrUsers = GetViewUsersPhotoCounts($arrUserids);
...
//j		$request->setParameter('no_header_footer', 1);
		...
		// returning index view because this is the only view for the PhotoCountsAjax page
		return VIEW_INDEX;
	}
