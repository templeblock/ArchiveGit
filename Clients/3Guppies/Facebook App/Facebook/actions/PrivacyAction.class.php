...
/**
 * Privacy action for the Facebook application
 *
 *
 * @author  Jim McCurdy
 */
class PrivacyAction extends Action
{
	...
	function execute (&$controller, &$request, &$user)
	{
		...
		$nameLoggedIn = GetUserInfo($facebookUser, 'first_name');
		$userid = (int)Param('user', $facebookUser);
		$userName = GetUserInfo($userid, 'first_name');
		
		// set the request attributes for presenting by the view
		$request->setAttribute( 'fbUserName', $userName );
		
		// returning index view because this is the only view for the Privacy page
		return VIEW_INDEX;
	}
