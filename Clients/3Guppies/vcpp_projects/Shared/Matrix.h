...
public:
	CMatrix();
	CMatrix(double ax, double ay, double bx, double by, double cx = 0, double cy = 0)
	...
	void operator*= (const CMatrix& Matrix)
		{ *this = *this * Matrix; }
	int operator!= (const CMatrix& Matrix) const
	...
	void Center(const CRect& DestRect, const CRect& SrcRect);
	void ScaleToFit(const CRect& DestRect, const CRect& SrcRect, bool bUseSmallerFactor = true);
	void StretchToFit(const CRect& DestRect, const CRect& SrcRect);
	...
	void TransformRectToPolygon(const CRect& Rect, POINT pt[4]) const;
