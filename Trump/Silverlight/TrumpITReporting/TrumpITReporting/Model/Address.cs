...
{
	public class AddressCollection : ObservableCollection<Address>, IDisposable
	{
		public void Dispose()
		{
		...
		{
			AddressSqlQuery query = new AddressSqlQuery(addressID);
			query.Result += delegate(Collection<Address> collection)
			{
				base.Clear();
				if (collection == null)
					return;

				foreach (Address item in collection)
					base.Add(item);
			};
	...
	internal class AddressSqlQuery : DBServiceBase<Address>
	{
		private string _addressID;

		internal AddressSqlQuery(string addressID)
		{
		...
		{
			IEnumerable<Address> query = from item in xmlDoc.Root.Descendants()
			select new Address
			{
	...
	public partial class Address : NotifyPropertyChanges
	{
