			...
			//	fso.GetFileName(filepath)			Filename with extension
				try
				{
					...
					if (fp1 != fp2)
						fso.CopyFile(filepath, newfilepath, false/*bOverwrite*/);
					return newfilepath;
				}
				catch(error)
				{
			...
			function OpenSaveDialog(bSave, Filter, DefaultExt, InitDir, DefFileName)
			{
				...
				//j	dialog = new ActiveXObject('ccrFileDialogs6.ccrpFileDialogs');
				//j	dialog = document.all.admin;
				//j	dialog = new ActiveXObject('SAFRCFileDlg.FileSave');
					if (!dialog)
						throw '';
				}
				catch (error)
			...
			function RenameFile(fso, Source, Destination)
			{
				try
				{
					if (fso.FileExists(Destination))
				...
				{
					var FsoError = 'Could not rename "' + Source + '" to "' + Destination + '"';
					alert(error.description + '\n\n' + FsoError);
					return false;
				}
			...
			function ProcessCSV(FileName)
			{
				...
				var SourceRoot = 'V:\\websitesimg\\american\\html\\product\\full\\ap\\';
				var OutputRoot = SourceRoot;
			//j	OutputRoot = strFolder + 'fixes' + '\\';
				if (!MakeFolder(fso, OutputRoot))
					return;
				...
				{  
					...
					var OutputFolder = OutputRoot + PRODUCT_NUMBER;
					if (!MakeFolder(fso, OutputFolder))
						break;

					var SourceFolder = SourceRoot + PRODUCT_NUMBER;
					var SettingsPath = SourceFolder + '\\' + DefaultSettingsFileName;

					// Make a copy of the settings file for debug purposes
					var SettingsCopyPath = OutputFolder + '\\' + "settings.orig.txt";
					if (fso.FileExists(SettingsPath) && !fso.FileExists(SettingsCopyPath))
						RenameFile(fso, SettingsPath, SettingsCopyPath);

					if (Open(SettingsPath))
					{
						SettingsPath = OutputFolder + '\\' + DefaultSettingsFileName;
						Export(SettingsPath)
					}

					iCount++;

				//j	break; // For debug purposes - just do one
				}
